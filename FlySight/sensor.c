...
typedef enum { MODE_INACTIVE, MODE_ACTIVE } Mode_t;

static volatile Mode_t mode = MODE_INACTIVE;
static volatile bool busy = false;

typedef enum { Operation_Read, Operation_Write, Operation_Recieve, Operation_Transmit } Operation_t;
...
void FS_Sensor_TransmitAsync(uint8_t addr, uint8_t* pData, uint16_t size,
                             void (*Callback)(HAL_StatusTypeDef)) {
...
void FS_Sensor_ReceiveAsync(uint8_t addr, uint8_t* pData, uint16_t size,
                            void (*Callback)(HAL_StatusTypeDef)) {
    Handler_t* h = &handlerBuf[handlerWrI % HANDLER_COUNT];
    uint32_t primask_bit;
    bool begin;
...
void FS_Sensor_WriteAsync(uint8_t addr, uint16_t reg, uint8_t* pData, uint16_t size,
                          void (*Callback)(HAL_StatusTypeDef)) {
    Handler_t* h = &handlerBuf[handlerWrI % HANDLER_COUNT];
    uint32_t primask_bit;
    bool begin;
...
void FS_Sensor_ReadAsync(uint8_t addr, uint16_t reg, uint8_t* pData, uint16_t size,
                         void (*Callback)(HAL_StatusTypeDef)) {
    Handler_t* h = &handlerBuf[handlerWrI % HANDLER_COUNT];
    uint32_t primask_bit;
    bool begin;
