...
static const int16_t sineTable[] = {
    ...
    18204,  19519,  20787,  22004,  23169,  24278,  25329,  26318,  27244,  28105,  28897,  29621,
    30272,  30851,  31356,  31785,  32137,  32412,  32609,  32727,  32767,  32727,  32609,  32412,
    32137,  31785,  31356,  30851,  30272,  29621,  28897,  28105,  27244,  26318,  25329,  24278,
    23169,  22004,  20787,  19519,  18204,  16845,  15446,  14009,  12539,  11038,  9511,   7961,
    6392,   4807,   3211,   1607,   0,      -1608,  -3212,  -4808,  -6393,  -7962,  -9512,  -11039,
    -12540, -14010, -15447, -16846, -18205, -19520, -20788, -22005, -23170, -24279, -25330, -26319,
    -27245, -28106, -28898, -29622, -30273, -30852, -31357, -31786, -32138, -32413, -32610, -32728,
    -32767, -32728, -32610, -32413, -32138, -31786, -31357, -30852, -30273, -29622, -28898, -28106,
    -27245, -26319, -25330, -24279, -23170, -22005, -20788, -19520, -18205, -16846, -15447, -14010,
...
static void FS_Audio_SetVolume(uint8_t volume) {
    if (audioVolume != volume) {
        ...
        /* Set volume */
        buf[0] = volume;
        if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_VOLUME, 1, buf, 1, HAL_MAX_DELAY) !=
            HAL_OK) {
            Error_Handler();
        }
...
void FS_Audio_Init(void) {
    ...
    /* Set stereo mode */
    buf[0] = 0;
    if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_GENERAL, 1, buf, 1, HAL_MAX_DELAY) !=
        HAL_OK) {
        Error_Handler();
    }
    ...
    /* Set IC = 0x0 (SF = 1) */
    buf[0] = (0x0 << 2);
    if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_CLOCK, 1, buf, 1, HAL_MAX_DELAY) !=
        HAL_OK) {
        Error_Handler();
    }
    ...
    /* Set CP = 0x09 */
    buf[0] = (0x09 << 0);
    if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_CHARGE_PUMP, 1, buf, 1,
    ...
    /* Set INT = 1 */
    buf[0] = (1 << 7);
    if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_LRCLK_MSB, 1, buf, 1, HAL_MAX_DELAY) !=
        HAL_OK) {
        Error_Handler();
    }
    ...
    /* Set LSB = 32 */
    buf[0] = 32;
    if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_LRCLK_LSB, 1, buf, 1, HAL_MAX_DELAY) !=
        HAL_OK) {
        Error_Handler();
    }
    ...
    if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_DIGITAL_AUDIO, 1, buf, 1,
                          HAL_MAX_DELAY) != HAL_OK) {
        Error_Handler();
    }
    ...
    if (HAL_I2C_Mem_Write(&hi2c1, MAX9850_ADDR, MAX9850_REG_ENABLE, 1, buf, 1, HAL_MAX_DELAY) !=
        HAL_OK) {
        Error_Handler();
    }
...
static void FS_Audio_InitTransfer(uint32_t bytes) {
    // Initialize state
    frameCount = 0;
    ...
    // Begin DMA transfer
    HAL_SAI_Transmit_DMA(&hsai_BlockA1, (uint8_t*)(audioBuffer + readPos),
                         AUDIO_FRAME_LEN - readPos);
}

void FS_Audio_Beep(uint32_t startFrequency, uint32_t endFrequency, uint32_t duration,
                   uint8_t volume) {
    ...
    if (end_step > audioStep)
        audioChirp = (end_step - audioStep) / audioLen;
    else
        audioChirp = 0 - (audioStep - end_step) / audioLen;
...
static bool FS_Audio_PlayFile(const char* filename) {
    f_chdir("/audio");
