    ...
    while (HAL_GetTick() < ms + timeout) {
        if (gnssRxIndex != GNSS_RX_BUF_LEN - huart1.hdmarx->Instance->CNDTR) {
            if (FS_GNSS_HandleByte(FS_GNSS_GetChar())) {
                if (gnssMsgClass == UBX_ACK && gnssMsgId == UBX_ACK_ACK) {
                    if (gnssPayload.ackAck.clsID == msg_class &&
                        gnssPayload.ackAck.msgID == msg_id) {
                        return 1; // ACK
                    }
                }
                else if (gnssMsgClass == UBX_ACK && gnssMsgId == UBX_ACK_NAK) {
...
static void FS_GNSS_InitMessages(void) {
    ...
    const ubxCfgMsg_t cfgMsg[] = {
        { UBX_NMEA, UBX_NMEA_GPGGA, 0 }, { UBX_NMEA, UBX_NMEA_GPGLL, 0 },
        { UBX_NMEA, UBX_NMEA_GPGSA, 0 }, { UBX_NMEA, UBX_NMEA_GPGSV, 0 },
        { UBX_NMEA, UBX_NMEA_GPRMC, 0 }, { UBX_NMEA, UBX_NMEA_GPVTG, 0 },
        { UBX_NAV, UBX_NAV_VELNED, 1 },  { UBX_NAV, UBX_NAV_PVT, 1 },
        { UBX_TIM, UBX_TIM_TP, 1 },      { UBX_SEC, UBX_SEC_ECSIGN, 10 }
    };

    const ubxCfgMsg_t cfgMsgRaw[] = { { UBX_MON, UBX_MON_TXBUF, 1 },
    ...
    const ubxCfgNav5_t cfgNav5 = { .mask = 0x0001, // Apply dynamic model settings
    ...
    ubxCfgValset_t cfgValset = {
        .version = 0x00,
        .layers = 0x01,
        .cfgData = { UBX_CFG_SEC_ECCFGSESSIONID0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
