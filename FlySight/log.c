...
static void FS_Log_Update(void) {
    ...
    while ((HAL_GetTick() < msStart + LOG_TIMEOUT) && FS_Config_Get()->enable_raw &&
           (rawRdI != rawIndex)) {
        FS_Log_UpdateRaw();
    }

    // Write GNSS log entries
    while ((HAL_GetTick() < msStart + LOG_TIMEOUT) && (gnssRdI != gnssIndex)) {
        FS_Log_UpdateGNSS();
    }
...
static FRESULT delete_node(TCHAR* path,  /* Path name buffer with the sub-directory to delete */
...
static void FS_Log_AdjustDateTime(uint16_t* year, uint8_t* month, uint8_t* day, uint8_t* hour,
