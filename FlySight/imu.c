...
typedef enum { ACCEL_FS_2 = 0, ACCEL_FS_16 = 1, ACCEL_FS_4 = 2, ACCEL_FS_8 = 3 } FS_IMU_AccelFS_t;
...
void FS_IMU_Start(void) {
    ...
    switch (config->accel_fs) {
        case ACCEL_FS_2:
            accelFactor = 2 * 100000;
            break;
        case ACCEL_FS_4:
            accelFactor = 4 * 100000;
            break;
        case ACCEL_FS_8:
            accelFactor = 8 * 100000;
            break;
        case ACCEL_FS_16:
            accelFactor = 16 * 100000;
            break;
    }
    ...
    switch (config->gyro_fs) {
        ...
        case GYRO_FS_500:
            ...
            break;
        case GYRO_FS_1000:
            gyroFactor = 1000 * 1000;
            break;
        case GYRO_FS_2000:
            gyroFactor = 2000 * 1000;
            break;
    }
...
static void FS_IMU_Read_Callback(HAL_StatusTypeDef result) {
    ...
    if (result == HAL_OK) {
        ...
        imuData.wx =
            -(((int16_t)((dataBuf[6] << 8) | dataBuf[5])) * (gyroFactor / 8)) / (32768 / 8);
        imuData.wz = (((int16_t)((dataBuf[8] << 8) | dataBuf[7])) * (gyroFactor / 8)) / (32768 / 8);

        imuData.ay =
            (((int16_t)((dataBuf[10] << 8) | dataBuf[9])) * (accelFactor / 8)) / (32768 / 8);
        imuData.ax =
            -(((int16_t)((dataBuf[12] << 8) | dataBuf[11])) * (accelFactor / 8)) / (32768 / 8);
        imuData.az =
            (((int16_t)((dataBuf[14] << 8) | dataBuf[13])) * (accelFactor / 8)) / (32768 / 8);
