...
static FS_CRS_StateFunc_t* const state_table[FS_CRS_STATE_COUNT] = {
    FS_CRS_State_Idle, FS_CRS_State_Read, FS_CRS_State_Write, FS_CRS_State_Dir
};
            ...
            switch (packet->data[0]) {
                case FS_CRS_COMMAND_CREATE:
                    ...
                    if (FS_ResourceManager_RequestResource(FS_RESOURCE_FATFS) ==
                        ...
                        // Create file
                        if (f_open(&file, (TCHAR*)&(packet->data[1]), FA_WRITE | FA_CREATE_NEW) ==
                            FR_OK) {
...
static FS_CRS_State_t FS_CRS_State_Read(void) {
    ...
    while ((next_state == FS_CRS_STATE_READ) && (packet = Custom_CRS_GetNextTxPacket()) &&
           (next_packet < next_ack + FS_CRS_WINDOW_LENGTH) && (next_packet < last_packet)) {
        buffer[0] = (next_packet & 0xff);
