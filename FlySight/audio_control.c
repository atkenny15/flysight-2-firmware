...
static const uint16_t sas_table[] = { 1024, 1077, 1135, 1197, 1265, 1338,
                                      1418, 1505, 1600, 1704, 1818, 1944 };
...
static void setTone(FS_Config_Data_t* config, int32_t val_1, int32_t min_1, int32_t max_1,
                    int32_t val_2, int32_t min_2, int32_t max_2) {
#define UNDER(val, min, max) ((min < max) ? (val <= min) : (val >= min))
#define OVER(val, min, max) ((min < max) ? (val >= max) : (val <= max))

    if (val_1 != INVALID_VALUE && val_2 != INVALID_VALUE) {
        if (UNDER(val_2, min_2, max_2)) {
            ...
            setRate(config->min_rate +
                    (config->max_rate - config->min_rate) * (val_2 - min_2) / (max_2 - min_2));
        }
        ...
        if (UNDER(val_1, min_1, max_1)) {
            ...
            setPitch(TONE_MIN_PITCH +
                     (TONE_MAX_PITCH - TONE_MIN_PITCH) * (val_1 - min_1) / (max_1 - min_1));
            setChirp(0);
        }
    }
    else {
...
static void getValues(FS_GNSS_Data_t* current, FS_Config_Data_t* config, uint8_t mode, int32_t* val,
    ...
    switch (mode) {
        ...
        case FS_CONFIG_MODE_DIRECTION_TO_DESTINATION:
            ...
            if ((calcDistance(current->lat, current->lon, config->lat, config->lon) <
                 config->max_dist) ||
                (config->max_dist == 0)) {
                // check if above height tone should be silenced
                if ((current->hMSL > (config->end_nav + config->dz_elev)) ||
                    (config->end_nav == 0)) {
                    tVal = calcDirection(current->lat, current->lon, config->lat, config->lon,
                                         current->heading);
                    // check if heading not within UBX_min_angle deg of bearing or tones needed for
                    // other measurement
                    if ((ABS(tVal) > config->min_angle) ||
                        (config->mode_2 != FS_CONFIG_MODE_DIRECTION_TO_DESTINATION) ||
                        (config->min_angle == 0)) {
                        *min = -180;
                        *max = 180;
        ...
        case FS_CONFIG_MODE_DIRECTION_TO_BEARING: // Direction to bearing
            ...
            if ((current->hMSL > (config->end_nav + config->dz_elev)) || (config->end_nav == 0)) {
                ...
                // other measurement
                if ((ABS(tVal) > config->min_angle) ||
                    (config->mode_2 != FS_CONFIG_MODE_DIRECTION_TO_BEARING) ||
                    (config->min_angle == 0)) {
                    *min = -180;
                    *max = 180;
                    // manipulate tone so biggest change is at desired heading
...
static char* numberToSpeech(int32_t number, char* ptr) {
    // Adapted from
...
static void speakValue(FS_Config_Data_t* config, FS_GNSS_Data_t* current) {
    uint16_t speed_mul = 1024;
    ...
    switch (config->speech[cur_speech].mode) {
        ...
        case FS_CONFIG_MODE_GLIDE_RATIO:
            if (current->velD != 0) {
                speech_ptr = writeInt32ToBuf(
                    speech_ptr, 100 * (int32_t)current->gSpeed / current->velD, 2, 1, 0);
            }
            else {
                *(--speech_ptr) = '\0';
            }
        ...
        case FS_CONFIG_MODE_INVERSE_GLIDE_RATIO:
            if (current->gSpeed != 0) {
                speech_ptr = writeInt32ToBuf(
                    speech_ptr, 100 * (int32_t)current->velD / current->gSpeed, 2, 1, 0);
            }
            else {
                *(--speech_ptr) = '\0';
            }
        ...
        case FS_CONFIG_MODE_DISTANCE_TO_DESTINATION:
            ...
            tVal = calcDistance(current->lat, current->lon, config->lat,
                                config->lon); // returns metres
            switch (config->speech[cur_speech].units) {
        ...
        case FS_CONFIG_MODE_DIVE_ANGLE:
            speech_ptr = writeInt32ToBuf(
                speech_ptr, 100 * atan2(current->velD, current->gSpeed) / M_PI * 180, 2, 1, 0);
            break;
    ...
    if (config->speech[cur_speech].mode != FS_CONFIG_MODE_DIRECTION_TO_DESTINATION) {
        if (config->speech[cur_speech].decimals == 0)
            ...
            end_ptr -= 3 - config->speech[cur_speech].decimals;
    }
...
static void updateAlarms(FS_Config_Data_t* config, FS_GNSS_Data_t* current) {
    ...
    if (prev_flags & FLAG_HAS_FIX) {
        ...
        if ((config->alt_step > 0) && (i == config->num_alarms) &&
            (prevHMSL - config->dz_elev >= ALT_MIN * 1000) && (*speech_ptr == 0) &&
            !(flags & FLAG_SAY_ALTITUDE) && !suppress_alt) {
            if ((step_elev >= min && step_elev < max) && ABS(current->velD) >= config->threshold &&
...
static void updateTones(FS_Config_Data_t* config, FS_GNSS_Data_t* current) {
    ...
    {
        if (config->mode ==
            FS_CONFIG_MODE_DIRECTION_TO_DESTINATION) // no need to re-calculate direction
        {
            val_2 = ABS(val_1);
        }
        else {
            val_2 = ABS(calcDirection(current->lat, current->lon, config->lat, config->lon,
                                      current->heading));
    ...
    {
        if (config->mode ==
            FS_CONFIG_MODE_DIRECTION_TO_BEARING) // no need to re-calculate direction
        {
            val_2 = ABS(val_1);
        }
        else {
        ...
        if (x0 != INVALID_VALUE && x1 != INVALID_VALUE && x2 != INVALID_VALUE && max_1 != min_1) {
    ...
    if (!g_suppress_tone) {
        if (ABS(current->velD) >= config->threshold && current->gSpeed >= config->hThreshold) {
            setTone(config, val_1, min_1, max_1, val_2, min_2, max_2);

            if (config->sp_rate != 0 && config->num_speech != 0 && sp_counter >= config->sp_rate &&
                (*speech_ptr == 0) && !(flags & FLAG_SAY_ALTITUDE)) {
                for (i = 0; i < config->num_speech; ++i) {
                    if ((config->speech[cur_speech].mode !=
                         FS_CONFIG_MODE_DIRECTION_TO_DESTINATION) ||
                        (current->hMSL - config->dz_elev >= ALT_MIN * 1000)) {
...
static void consumerTask(void) {
    ...
    if (*speech_ptr) {
        ...
        if ((flags & FLAG_SAY_ALTITUDE) && (flags & FLAG_HAS_FIX) && (flags & FLAG_VERTICAL_ACC) &&
            FS_Audio_IsIdle()) {
...
void FS_AudioControl_Init(void) {
    ...
    if (config->init_mode == 1) {
        ...
        if (strlen(config->init_filename)) {
            ...
            strncat(speech_buf, config->init_filename, sizeof(speech_buf) - strlen(speech_buf) - 1);
            strncat(speech_buf, ".wav", sizeof(speech_buf) - strlen(speech_buf) - 1);
        }
    }
}
