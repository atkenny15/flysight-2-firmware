...
#if (BLE_CFG_SVC_MAX_NBR_CB > 0)
    SVCCTL_EvtHandler.SVCCTL__SvcHandlerTab[SVCCTL_EvtHandler.NbreOfRegisteredHandler] =
        pfBLE_SVC_Service_Event_Handler;
...
#if (BLE_CFG_CLT_MAX_NBR_CB > 0)
    SVCCTL_CltHandler.SVCCTL_CltHandlerTable[SVCCTL_CltHandler.NbreOfRegisteredHandler] =
...
__WEAK SVCCTL_UserEvtFlowStatus_t SVCCTL_UserEvtRx(void* pckt) {
            ...
            switch ((blecore_evt->ecode) & SVCCTL_EGID_EVT_MASK) {
                case SVCCTL_GATT_EVT_TYPE:
                    ...
                    for (index = 0; index < SVCCTL_EvtHandler.NbreOfRegisteredHandler; index++) {
                        ...
                        /**
                         * When a GATT event has been acknowledged by a Service, there is no need to
                         * call the other registered handlers a GATT event is relevant for only one
                         * Service
                         */
                    ...
                    for (index = 0; index < SVCCTL_CltHandler.NbreOfRegisteredHandler; index++) {
                        ...
                        /**
                         * When a GATT event has been acknowledged by a Client, there is no need to
                         * call the other registered handlers a GATT event is relevant for only one
                         * Client
                         */
                        if (event_notification_status != SVCCTL_EvtNotAck) {
    ...
    /**
     * When no registered handlers (either Service or Client) has acknowledged the GATT event, it is
     * reported to the application a GAP event is always reported to the application.
     */
