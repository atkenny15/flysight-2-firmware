...
uint8_t SHCI_C2_FUS_GetState(SHCI_FUS_GetState_ErrorCode_t* p_error_code) {
    ...
    if (p_error_code != 0) {
        *p_error_code =
            (SHCI_FUS_GetState_ErrorCode_t)(((TL_CcEvt_t*)(p_rsp->evtserial.evt.payload))
                                                ->payload[1]);
    }
...
SHCI_CmdStatus_t SHCI_C2_FUS_FwUpgrade(uint32_t fw_src_add, uint32_t fw_dest_add) {
    /**
     * TL_BLEEVT_CC_BUFFER_SIZE is 16 bytes so it is large enough to hold the 8 bytes of command
     * parameters Buffer is large enough to hold command complete without payload
     */
...
SHCI_CmdStatus_t SHCI_C2_FUS_UpdateAuthKey(SHCI_C2_FUS_UpdateAuthKey_Cmd_Param_t* pParam) {
    ...
    shci_send(SHCI_OPCODE_C2_FUS_UPDATE_AUTH_KEY, sizeof(SHCI_C2_FUS_UpdateAuthKey_Cmd_Param_t),
              (uint8_t*)pParam, p_rsp);
...
SHCI_CmdStatus_t SHCI_C2_CONCURRENT_GetNextBleEvtTime(
    SHCI_C2_CONCURRENT_GetNextBleEvtTime_Param_t* pParam) {
    /**
     * Buffer is large enough to hold command complete with payload
     */
    ...
    memcpy((void*)&(pParam->relative_time),
           (void*)&((TL_CcEvt_t*)(p_rsp->evtserial.evt.payload))->payload[1],
...
SHCI_CmdStatus_t SHCI_C2_ExtpaConfig(uint32_t gpio_port, uint16_t gpio_pin_number,
...
SHCI_CmdStatus_t SHCI_C2_SetFlashActivityControl(
    SHCI_C2_SET_FLASH_ACTIVITY_CONTROL_Source_t Source) {
    /**
     * TL_BLEEVT_CC_BUFFER_SIZE is 16 bytes so it is large enough to hold the 1 byte of command
     * parameter Buffer is large enough to hold command complete without payload
     */
    uint8_t local_buffer[TL_BLEEVT_CC_BUFFER_SIZE];
    TL_EvtPacket_t* p_rsp;
...
SHCI_CmdStatus_t SHCI_C2_Config(SHCI_C2_CONFIG_Cmd_Param_t* pCmdPacket) {
    ...
    shci_send(SHCI_OPCODE_C2_CONFIG, sizeof(SHCI_C2_CONFIG_Cmd_Param_t), (uint8_t*)pCmdPacket,
              p_rsp);
...
SHCI_CmdStatus_t SHCI_GetWirelessFwInfo(WirelessFwInfo_t* pWirelessInfo) {
     ...
     * If the FUS is running on CPU2, FUS_DEVICE_INFO_TABLE_VALIDITY_KEYWORD shall be written in the
     * table. Otherwise, it means the Wireless Firmware is running on the CPU2
     */
    p_fus_device_info_table =
        (MB_FUS_DeviceInfoTable_t*)(*(uint32_t*)((ipccdba << 2) + SRAM2A_BASE));
    ...
    if (p_fus_device_info_table->DeviceInfoTableState == FUS_DEVICE_INFO_TABLE_VALIDITY_KEYWORD) {
        ...
        wireless_firmware_memorySize =
            p_RefTable->p_device_info_table->WirelessFwInfoTable.MemorySize;
        wireless_firmware_infoStack =
            p_RefTable->p_device_info_table->WirelessFwInfoTable.InfoStack;
    ...
    pWirelessInfo->FusMemorySizeFlash =
