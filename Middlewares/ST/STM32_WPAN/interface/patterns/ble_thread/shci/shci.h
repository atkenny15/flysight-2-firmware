 ...
 *
 * These errors can be generated by all layers (low level driver, stack, framework infrastructure,
 * etc..)
 */
typedef enum {
    ...
    ERR_THREAD_UNKNOWN_CMD =
        126, /* The command send by the CPU1 to control the Thread stack is unknown */
    ERR_ZIGBEE_UNKNOWN_CMD =
        200, /* The command send by the CPU1 to control the Zigbee stack is unknown */
 ...
 * It reports as well which firmware is running on CPU2 : The wireless stack of the FUS (previously
 * named RSS)
 */
...
/** Response parameters*/
/** It responds a 1 byte value holding FUS State error code when the FUS State value is 0xFF
...
enum {
    ...
    FUS_STATE_VALUE_FW_UPGRD_ONGOING_END =
        0x1F, /* All values between 0x10 and 0x1F has the same meaning */
    FUS_STATE_VALUE_FUS_UPGRD_ONGOING = 0x20,
    FUS_STATE_VALUE_FUS_UPGRD_ONGOING_END =
        0x2F, /* All values between 0x20 and 0x2F has the same meaning */
    FUS_STATE_VALUE_SERVICE_ONGOING = 0x30,
    FUS_STATE_VALUE_SERVICE_ONGOING_END =
        0x3F, /* All values between 0x30 and 0x3F has the same meaning */
...
#    define SHCI_OPCODE_C2_FUS_ACTIVATE_ANTIROLLBACK \
        ((SHCI_OGF << 10) + SHCI_OCF_C2_FUS_ACTIVATE_ANTIROLLBACK)
/** No command parameters */
/** No response parameters*/
...
typedef PACKED_STRUCT {
     ...
     * extended property. The total calculated value must be increased by 9, due to the records
     * related to the standard attribute profile and GAP service characteristics, and automatically
     * added when initializing GATT and GAP layers
    ...
    /**
     * NumAttrServ
     * Defines the maximum number of services that can be stored in the GATT database. Note that the
     * GAP and GATT services are automatically added at initialization so this parameter must be the
     * number of user services increased by two.
    ...
    /**
     * AttrValueArrSize
     * NOTE: This parameter is ignored by the CPU2 when the parameter "Options" is set to "LL_only"
    ...
    /**
     * MblockCount
     * NOTE: This parameter is overwritten by the CPU2 with an hardcoded optimal value when the
     * parameter "Options" is set to "LL_only" ( see Options description in that structure )
     *
    ...
    /**
     * SlaveSca
     * The sleep clock accuracy (ppm value) that used in BLE connected slave mode to calculate the
     * window widening (in combination with the sleep clock accuracy sent by master in CONNECT_REQ
     * PDU), refer to BLE 5.0 specifications - Vol 6 - Part B - chap 4.5.7 and 4.2.2
     *     - Min value: 0
    ...
    /**
     * MasterSca
     * The sleep clock accuracy handled in master mode. It is used to determine the connection and
     * advertising events timing. It is transmitted to the slave in CONNEC_REQ PDU used by the slave
     * to calculate the window widening, see SlaveSca and Bluetooth Core Specification v5.0 Vol 6 -
     * Part B - chap 4.5.7 and 4.2.2 Possible values:
     ...
     * - bit 0:   1: Calibration for the RF system wakeup clock source   0: No calibration for the
     * RF system wakeup clock source
     * - bit 1:   1: STM32W5M Module device                              0: Other devices as
     * STM32WBxx SOC, STM32WB1M module
     ...
     * duration is reached the slave closes the current connections event (whatever is the CE_length
     * parameter specified by the master in HCI_CREATE_CONNECTION HCI command), expressed in units
     * of 625/256 µs (~2.44 µs)
     *    - Min value: 0 (if 0 is specified, the master and slave perform only a single TX-RX
     * exchange per connection event)
     *    - Max value: 1638400 (4000 ms). A higher value can be specified (max 0xFFFFFFFF) but
     * results in a maximum connection time of 4000 ms as specified. In this case the parameter is
     * not applied, and the predicted CE length calculated on slave is not shortened
     */
     ...
     * (~2.44 µs).
     *    - Min value: 0
     *    - Max value:  820 (~2 ms). A higher value can be specified, but the value that implemented
     * in stack is forced to ~2 ms
     */
     ...
     * This parameter is linked to max_adv_data_len such as both compliant with allocated Total
     * memory computed with BLE_EXT_ADV_BUFFER_SIZE based on Max Extended advertising configuration
     * supported. This parameter is considered by the CPU2 when Options has
     ...
     * This parameter is linked to max_adv_set_nbr such as both compliant with allocated Total
     * memory computed with BLE_EXT_ADV_BUFFER_SIZE based on Max Extended advertising configuration
     * supported. This parameter is considered by the CPU2 when Options has
     * SHCI_C2_BLE_INIT_OPTIONS_EXT_ADV flag set
     */
...
#    define SHCI_OPCODE_C2_FLASH_ERASE_ACTIVITY \
        ((SHCI_OGF << 10) + SHCI_OCF_C2_FLASH_ERASE_ACTIVITY)
/** Command parameters */
typedef enum {
...
#    define SHCI_OPCODE_C2_RADIO_ALLOW_LOW_POWER \
        ((SHCI_OGF << 10) + SHCI_OCF_C2_RADIO_ALLOW_LOW_POWER)
...
#    define SHCI_OPCODE_C2_SET_FLASH_ACTIVITY_CONTROL \
        ((SHCI_OGF << 10) + SHCI_OCF_C2_SET_FLASH_ACTIVITY_CONTROL)
/** Command parameters */
typedef enum {
...
/*
 *   At startup, the information relative to the wireless binary are stored in RAM through a
 * structure defined by MB_WirelessFwInfoTable_t.This structure contains 4 fields
 * (Version,MemorySize, Stack_info and a reserved part) each of those coded on 32 bits as shown on
 * the table below:
 *
 *
 ...
 * |3 |2 |1 |0 |
 *               -------------------------------------------------------------------------------------------------
 * Version       |   Major version       |    Minor version      |    Sub version        | Branch
 * |ReleaseType|
 *               -------------------------------------------------------------------------------------------------
 * MemorySize    |   SRAM2B (kB)         |    SRAM2A (kB)        |    SRAM1 (kB)         | FLASH
 * (4kb)           |
 *               -------------------------------------------------------------------------------------------------
 * Info stack    |   Reserved            |    Reserved           |    Reserved           | Type
 * (MAC,Thread,BLE) |
 *               -------------------------------------------------------------------------------------------------
 ...
 *               When the wireless firmware receives that command, it responds
 * SHCI_FUS_CMD_NOT_SUPPORTED the first time. When the wireless firmware receives that command a
 * second time, it reboots the full device with the FUS running on CPU2
 *
 ...
 * @brief Request the FUS to enable the AntiRollback feature so that it is not possible to update
 * the wireless firmware with an older version than the current one. Note:
 ...
 *        When the CPU2 receives the command, it controls the Ext PA as requested by the
 * configuration This configures only which IO is used to enable/disable the ExtPA and the
 * associated polarity This command has no effect on the other IO that is used to control the mode
 * of the Ext PA (Rx/Tx)
 *
 * @param gpio_port: GPIOx where x can be (A..F) to select the GPIO peripheral for STM32WBxx family
 * @param gpio_pin_number: This parameter can be one of GPIO_PIN_x (= LL_GPIO_PIN_x)  where x can be
 * (0..15).
 ...
 *                -  FLASH_ACTIVITY_CONTROL_PES : The CPU2 set the PES bit to prevent the CPU1 to
 * either read or write in flash
 *                -  FLASH_ACTIVITY_CONTROL_SEM7 : The CPU2 gets the semaphore 7 to prevent the CPU1
 * to either read or write in flash. This requires the CPU1 to first get semaphore 7 before erasing
 * or writing the flash.
 *
 * @retval Status
 */
 ...
 *                    uint8_t PayloadCmdSize : Size of the payload - shall be
 * SHCI_C2_CONFIG_PAYLOAD_CMD_SIZE uint8_t Config1 :
 *                     - bit0 :  0 - BLE NVM Data  data are flushed in internal secure flash
 *                               1 - BLE NVM Data are written in SRAM cache pointed by
 * BleNvmRamAddress
 *                     - bit1 :  0 - THREAD NVM Data  data are flushed in internal secure flash
 *                               1 - THREAD NVM Data are written in SRAM cache pointed by
 * ThreadNvmRamAddress
 *                     - bit2 to bit7 : Unused, shall be set to 0
 *                    uint8_t EvtMask1 :
 *                            When a bit is set to 0, the event is not reported
 *                            bit0 : Asynchronous Event with Sub Evt Code 0x9201 (=
 * SHCI_SUB_EVT_ERROR_NOTIF)
 *                            ...
 ...
 *                               The size of the buffer shall be BLE_NVM_SRAM_SIZE (number of
 * 32bits) The buffer shall be allocated in SRAM2 uint32_t ThreadNvmRamAddress : Only considered
 * when Config1.bit1 = 1 When set to 0, data are kept in internal SRAM on CPU2 Otherwise, data are
 * copied in the cache pointed by ThreadNvmRamAddress The size of the buffer shall be
 * THREAD_NVM_SRAM_SIZE (number of 32bits) The buffer shall be allocated in SRAM1
 *
 ...
 *                -  SET_SYSTEM_CLOCK_HSE_TO_PLL : CPU2 set system clock to PLL, PLL must be
 * configured and started before.
 *                -  SET_SYSTEM_CLOCK_PLL_ON_TO_HSE : CPU2 set System clock to HSE, PLL is still ON
 * after command exection.
 *                -  SET_SYSTEM_CLOCK_PLL_OFF_TO_HSE : CPU2 set System clock to HSE, PLL is turned
 * OFF after command exection.
 *
 * @retval Status
 */
