...
typedef struct {
    ...
    int32_t (*Receive)(uint8_t*,
                       uint16_t); /**< Pointer to HCI TL function for the IO Bus data reception */
    int32_t (*Send)(uint8_t*,
                    uint16_t); /**< Pointer to HCI TL function for the IO Bus data transmission */
    int32_t (*DataAck)(
        uint8_t*,
        uint16_t* len); /**< Pointer to HCI TL function for the IO Bus data ack reception */
    int32_t (*GetTick)(
        void); /**< Pointer to BSP function for getting the HAL time base timestamp */
} tHciIO;
/**
 * @}
 */
...
/**
 * @brief  This function is called when an ACI/HCI command is sent to the CPU2 and the response is
 * waited. It is called from the same context the HCI command has been sent. It shall not return
 * until the command response notified by hci_cmd_resp_release() is received. A weak implementation
 * is available in hci_tl.c based on polling mechanism The user may re-implement this function in
...
/**
 * @brief  This process shall be called by the scheduler each time it is requested with
 * hci_notify_asynch_evt() This process may send an ACI/HCI command when the svc_ctl.c module is
 * used
 *
 * @param  None
 * @retval None
