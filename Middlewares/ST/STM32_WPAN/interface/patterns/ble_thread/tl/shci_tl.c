...
void shci_user_evt_proc(void) {
    ...
    /**
     * Up to release version v1.2.0, a while loop was implemented to read out events from the queue
     * as long as it is not empty. However, in a bare metal implementation, this leads to calling in
     * a "blocking" mode shci_user_evt_proc() as long as events are received without giving the
     * opportunity to run other tasks in the background. From now, the events are reported one by
     * one. When it is checked there is still an event pending in the queue, a request to the user
     * is made to call again shci_user_evt_proc(). This gives the opportunity to the application to
     * run other background tasks between each event.
     */

    /**
     * It is more secure to use LST_remove_head()/LST_insert_head() compare to
     * LST_get_next_node()/LST_remove_node() in case the user overwrite the header where the
     * next/prev pointers are located
     */
...
void shci_resume_flow(void) {
    ...
    /**
     * It is better to go through the background process as it is not sure from which context this
     * API may be called
     */
    shci_notify_asynch_evt((void*)&SHciAsynchEventQueue);
...
static void TlUserEvtReceived(TL_EvtPacket_t* shcievt) {
    ...
    shci_notify_asynch_evt((void*)&SHciAsynchEventQueue); /**< Notify the application a full HCI
