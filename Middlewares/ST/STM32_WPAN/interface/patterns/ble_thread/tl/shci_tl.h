...
typedef struct {
    ...
    int32_t (*Receive)(uint8_t*,
                       uint16_t); /**< Pointer to SHCI TL function for the IO Bus data reception */
    int32_t (*Send)(uint8_t*,
                    uint16_t); /**< Pointer to SHCI TL function for the IO Bus data transmission */
    int32_t (*DataAck)(
        uint8_t*,
        uint16_t* len); /**< Pointer to SHCI TL function for the IO Bus data ack reception */
    int32_t (*GetTick)(
...
/**
 * @brief  This function is called when an System HCI Command is sent to the CPU2 and the response
 * is waited. It is called from the same context the System HCI command has been sent. It shall not
 * return until the command response notified by shci_cmd_resp_release() is received. A weak
 * implementation is available in shci_tl.c based on polling mechanism The user may re-implement
 * this function in the application to improve performance :
...
/**
 * @brief  This process shall be called each time the shci_notify_asynch_evt notification is
 * received
 *
 * @param  None
 * @retval None
