...
/**
 * Define BD_ADDR type: define proper address. Can only be GAP_PUBLIC_ADDR (0x00) or
 * GAP_STATIC_RANDOM_ADDR (0x01)
 */
 ...
 * GAP_PUBLIC_ADDR or GAP_STATIC_RANDOM_ADDR if CFG_PRIVACY equals PRIVACY_ENABLED,
 * CFG_BLE_ADDRESS_TYPE has 2 allowed values: GAP_RESOLVABLE_PRIVATE_ADDR or
 ...
 * This parameter is ignored by the CPU2 when CFG_BLE_OPTIONS has SHCI_C2_BLE_INIT_OPTIONS_LL_ONLY
 * flag set
 */
 ...
 * This parameter is overwritten by the CPU2 with an hardcoded optimal value when the parameter
 * CFG_BLE_OPTIONS has SHCI_C2_BLE_INIT_OPTIONS_LL_ONLY flag set
 */
 ...
 * - bit 0:   1: Calibration for the RF system wakeup clock source   0: No calibration for the RF
 * system wakeup clock source
 * - bit 1:   1: STM32WB5M Module device                             0: Other devices as STM32WBxx
 * SOC, STM32WB1M module
...
/**
 * Maximum duration of the connection event when the device is in Peripheral mode in units of
 * 625/256 us (~2.44 us)
 */
 ...
 * This parameter is linked to CFG_BLE_MAX_ADV_DATA_LEN such as both compliant with allocated Total
 * memory computed with BLE_EXT_ADV_BUFFER_SIZE based on Max Extended advertising configuration
 * supported. This parameter is considered by the CPU2 when CFG_BLE_OPTIONS has
 * SHCI_C2_BLE_INIT_OPTIONS_EXT_ADV flag set
 */
 ...
 * This parameter is linked to CFG_BLE_MAX_ADV_SET_NBR such as both compliant with allocated Total
 * memory computed with BLE_EXT_ADV_BUFFER_SIZE based on Max Extended advertising configuration
 * supported. This parameter is considered by the CPU2 when CFG_BLE_OPTIONS has
 * SHCI_C2_BLE_INIT_OPTIONS_EXT_ADV flag set
 */
 ...
 * packet payload size (a greater value is a lost of memory as it will never be used) It shall be at
 * least 4 to receive the command status event in one frame. The default value is set to 27 to allow
 * receiving an event of MTU size in a single buffer. This value maybe reduced further depending on
 * the application.
 */
 ...
 *
 *  CFG_RTC_ASYNCH_PRESCALER: This sets the asynchronous prescaler of the RTC. It should as high as
 * possible ( to output clock as low as possible) but the output clock should be equal or higher
 * frequency compare to the clock feeding the wakeup timer. A lower clock speed would impact the
 * accuracy of the timer server.
 *
 *  CFG_RTC_SYNCH_PRESCALER: This sets the synchronous prescaler of the RTC.
 *  When the 1Hz calendar clock is required, it shall be sets according to other settings
 *  When the 1Hz calendar clock is not needed, CFG_RTC_SYNCH_PRESCALER should be set to 0x7FFF (MAX
 * VALUE)
 *
...
/**
 * When CFG_DEBUG_TRACE_FULL is set to 1, the trace are output with the API name, the file name and
 * the line number When CFG_DEBUG_TRACE_LIGHT is set to 1, only the debug message is output
 *
 ...
 * This mechanism allows to implement a generic code in the API TL_BLE_HCI_StatusNot() to comply
 * with the requirement that a HCI/ACI command shall never be sent if there is already one pending
 */
...
typedef enum {
    CFG_FIRST_TASK_ID_WITH_NO_HCICMD =
        CFG_LAST_TASK_ID_WITH_HCICMD - 1, /**< Shall be FIRST in the list */
