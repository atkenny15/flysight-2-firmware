...
/* Private typedef -----------------------------------------------------------*/
typedef enum { TimerID_Free, TimerID_Created, TimerID_Running } TimerIDStatus_t;

typedef enum { SSR_Read_Requested, SSR_Read_Not_Requested } RequestReadSSR_t;
...
static uint16_t linkTimer(uint8_t TimerID) {
    ...
    if (CurrentRunningTimerID == CFG_HW_TS_MAX_NBR_CONCURRENT_TIMER) {
        ...
        if (aTimerContext[CurrentRunningTimerID].CountLeft <= time_left) {
            ...
            while ((next_id != CFG_HW_TS_MAX_NBR_CONCURRENT_TIMER) &&
                   (aTimerContext[next_id].CountLeft <= time_left)) {
...
static void UnlinkTimer(uint8_t TimerID, RequestReadSSR_t RequestReadSSR) {
    ...
    if ((CurrentRunningTimerID == CFG_HW_TS_MAX_NBR_CONCURRENT_TIMER) &&
        (RequestReadSSR == SSR_Read_Requested)) {
        SSRValueOnLastSetup = SSR_FORBIDDEN_VALUE;
    }
 ...
 * @note  The API is writing the counter value so that the value is decreased by one to cope with
 * the fact the interrupt is generated with 1 extra clock cycle (See RefManuel) It assumes all
 * condition are met to be allowed to write the wakeup counter
 * @param  Value: Value to be written in the counter
 * @retval None
 */
static void RestartWakeupCounter(uint16_t Value) {
    /**
     * The wakeuptimer has been disabled in the calling function to reduce the time to poll the
     * WUTWF FLAG when the new value will have to be written
    ...
    if (Value == 0) {
         ...
         * Otherwise, when the timer is periodic with 1 Tick, it may generate an extra interrupt in
         * between due to the autoreload feature
         */
        __HAL_RTC_WAKEUPTIMER_CLEAR_FLAG(&hrtc, RTC_FLAG_WUTF); /**<  Clear flag in RTC module */
        __HAL_RTC_WAKEUPTIMER_EXTI_CLEAR_FLAG();                /**<  Clear flag in EXTI module */
        HAL_NVIC_ClearPendingIRQ(
            CFG_HW_TS_RTC_WAKEUP_HANDLER_ID); /**<  Clear pending bit in NVIC */
...
void HW_TS_RTC_Wakeup_Handler(void) {
    ...
    if (aTimerContext[local_current_running_timer_id].TimerIDStatus == TimerID_Running) {
        ...
        /**
         * It should be good to check whether the TimeElapsed is greater or not than the tick left
         * to be counted However, due to the inaccuracy of the reading of the time elapsed, it may
         * return there is 1 tick to be left whereas the count is over A more secure implementation
         * has been done with a flag to state whereas the full count has been written in the
         * wakeuptimer or not
         */
...
#endif
                HW_TS_Start(local_current_running_timer_id,
                            aTimerContext[local_current_running_timer_id].CounterInit);
         ...
         * However, if due to any bug in the timer server this is the case, the mistake may not
         * impact the user. We could just clean the interrupt flag and get out from this unexpected
         * interrupt
         */
        while (__HAL_RTC_WAKEUPTIMER_GET_FLAG(&hrtc, RTC_FLAG_WUTWF) == RESET)
            ;
...
void HW_TS_Init(HW_TS_InitMode_t TimerInitMode, RTC_HandleTypeDef* phrtc) {
    ...
    AsynchPrescalerUserConfig = (uint8_t)(READ_BIT(RTC->PRER, RTC_PRER_PREDIV_A) >>
     ...
     */
    localmaxwakeuptimersetup = ((((SynchPrescalerUserConfig - 1) * AsynchPrescalerUserConfig) -
    ...
    if (TimerInitMode == hw_ts_InitMode_Full) {
        ...
        CurrentRunningTimerID =
            CFG_HW_TS_MAX_NBR_CONCURRENT_TIMER; /**<  Set ID to non valid value */
...
HW_TS_ReturnStatus_t HW_TS_Create(uint32_t TimerProcessID, uint8_t* pTimerId,
                                  HW_TS_Mode_t TimerMode, HW_TS_pTimerCb_t pftimeout_handler) {
...
void HW_TS_Stop(uint8_t timer_id) {
    ...
    if (aTimerContext[timer_id].TimerIDStatus == TimerID_Running) {
        ...
        if (localcurrentrunningtimerid == CFG_HW_TS_MAX_NBR_CONCURRENT_TIMER) {
             ...
             * It takes 2 RTCCLK between the time the WUTE bit is disabled and the
             * time the timer is disabled. The WUTWF bit somehow guarantee the system is stable
             * Otherwise, when the timer is periodic with 1 Tick, it may generate an extra interrupt
             * in between due to the autoreload feature
             */
            __HAL_RTC_WAKEUPTIMER_CLEAR_FLAG(&hrtc,
                                             RTC_FLAG_WUTF); /**<  Clear flag in RTC module */
            __HAL_RTC_WAKEUPTIMER_EXTI_CLEAR_FLAG();         /**<  Clear flag in EXTI module */
            HAL_NVIC_ClearPendingIRQ(
                CFG_HW_TS_RTC_WAKEUP_HANDLER_ID); /**<  Clear pending bit in NVIC */
        }
...
__weak void HW_TS_RTC_Int_AppNot(uint32_t TimerProcessID, uint8_t TimerID,
                                 HW_TS_pTimerCb_t pTimerCallBack) {
    pTimerCallBack();
...
uint8_t HW_TS_CountUsed() {
    ...
    while ((loop < CFG_HW_TS_MAX_NBR_CONCURRENT_TIMER)) {
        if (aTimerContext[loop].TimerIDStatus != TimerID_Free)
            ret++;
        loop++;
    }
#if (CFG_HW_TS_USE_PRIMASK_AS_CRITICAL_SECTION == 1)
