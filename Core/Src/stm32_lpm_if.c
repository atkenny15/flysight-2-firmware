...
void PWR_EnterOffMode(void) {
    ...
    /**
     * The systick should be disabled for the same reason than when the device enters stop mode
     * because at this time, the device may enter either OffMode or StopMode.
     */
    HAL_SuspendTick();
     ...
     ***********************************************************************************/
    /*
     * There is no risk to clear all the WUF here because in the current implementation, this API is
     * called in critical section. If an interrupt occurs while in that critical section before that
     * point, the flag is set and will be cleared here but the system will not enter Off Mode
...
void PWR_EnterStopMode(void) {
     ...
     *
     * When in production, the HAL_DBGMCU_EnableDBGStopMode() is not called so that the device can
     * reach best power consumption However, the systick should be disabled anyway to avoid the case
     * when it is about to expire at the same time the device enters stop mode (this will abort the
     * Stop Mode entry).
     */
    HAL_SuspendTick();
...
static void ExitLowPower(void) {
    ...
    if (LL_RCC_GetSysClkSource() == LL_RCC_SYS_CLKSOURCE_STATUS_HSI) {
        ...
        /* If the application is not running on HSE restore the clock configuration in this user
         * section */
