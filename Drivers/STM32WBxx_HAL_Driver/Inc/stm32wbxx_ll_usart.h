 ...
 * values */
static const uint32_t USART_PRESCALER_TAB[] = { 1UL,  2UL,  4UL,  6UL,  8UL,   10UL,
                                                12UL, 16UL, 32UL, 64UL, 128UL, 256UL };
/**
 * @}
 */
...
typedef struct {
    ...
    uint32_t
        TransferDirection; /*!< Specifies whether the Receive and/or Transmit mode is enabled or
                              disabled. This parameter can be a value of @ref USART_LL_EC_DIRECTION.

                                This feature can be modified afterwards using unitary
                                function @ref LL_USART_SetTransferDirection().*/

    uint32_t HardwareFlowControl; /*!< Specifies whether the hardware flow control mode is enabled
                                     or disabled. This parameter can be a value of @ref
                                     USART_LL_EC_HWCONTROL.
...
typedef struct {
    ...
    uint32_t LastBitClockPulse; /*!< Specifies whether the clock pulse corresponding to the last
                                   transmitted data bit (MSB) has to be output on the SCLK pin in
                                   synchronous mode. This parameter can be a value of @ref
...
#    define LL_USART_CR1_RXNEIE_RXFNEIE \
        USART_CR1_RXNEIE_RXFNEIE /*!< Read data register and RXFIFO not empty interrupt enable */
#    define LL_USART_CR1_TCIE USART_CR1_TCIE /*!< Transmission complete interrupt enable */
#    define LL_USART_CR1_TXEIE_TXFNFIE                                                           \
        USART_CR1_TXEIE_TXFNFIE /*!< Transmit data register empty and TX FIFO not full interrupt \
                                   enable */
...
#    define LL_USART_CR3_TCBGTIE \
        USART_CR3_TCBGTIE /*!< Transmission complete before guard time interrupt enable */
...
#    define LL_USART_PARITY_ODD \
        (USART_CR1_PCE | USART_CR1_PS) /*!< Parity control enabled and Odd Parity is selected */
/**
 * @}
 */
...
#    define LL_USART_PRESCALER_DIV6 \
        (USART_PRESC_PRESCALER_1 | USART_PRESC_PRESCALER_0)   /*!< Input clock divided by 6  */
...
#    define LL_USART_BINARY_LOGIC_POSITIVE                                                        \
        0x00000000U /*!< Logical data from the data register are send/received in positive/direct \
                       logic. (1=H, 0=L) */
#    define LL_USART_BINARY_LOGIC_NEGATIVE                                                        \
        USART_CR2_DATAINV /*!< Logical data from the data register are send/received in           \
                             negative/inverse logic. (1=L, 0=H). The parity bit is also inverted. \
                           */
/**
 * @}
 */
...
#    define LL_USART_BITORDER_LSBFIRST                                                           \
        0x00000000U /*!< data is transmitted/received with data bit 0 first, following the start \
                       bit */
#    define LL_USART_BITORDER_MSBFIRST                                                         \
        USART_CR2_MSBFIRST /*!< data is transmitted/received with the MSB first, following the \
                              start bit */
/**
 * @}
 */
...
#    define LL_USART_AUTOBAUD_DETECT_ON_FALLINGEDGE                                            \
        USART_CR2_ABRMODE_0 /*!< Falling edge to falling edge measurement. Received frame must \
                               start with a single bit = 1 -> Frame = Start10xxxxxx */
...
#    define LL_USART_HWCONTROL_RTS                                                                \
        USART_CR3_RTSE /*!< RTS output enabled, data is only requested when there is space in the \
                          receive buffer */
#    define LL_USART_HWCONTROL_CTS                                                            \
        USART_CR3_CTSE /*!< CTS mode enabled, data is only transmitted when the nCTS input is \
                          asserted (tied to 0) */
...
#    define __LL_USART_DIV_SAMPLING16(__PERIPHCLK__, __PRESCALER__, __BAUDRATE__)               \
        ((((__PERIPHCLK__) / (USART_PRESCALER_TAB[(__PRESCALER__)])) + ((__BAUDRATE__) / 2U)) / \
         (__BAUDRATE__))
 ...
 *         and current operations are discarded. The configuration of the USART is kept, but all the
 * status flags, in the USARTx_ISR are set to their default values.
 ...
 * @note   When this function is enabled, USART is able to wake up the MCU from Stop mode, provided
 * that USART clock selection is HSI or LSE in RCC.
 ...
 * @note   This function selects if hardware parity control (generation and detection) is enabled or
 * disabled. When the parity control is enabled (Odd or Even), computed parity bit is inserted at
 * the MSB position (9th or 8th bit depending on data width) and parity is checked on the received
 * data.
...
/**
 * @brief  Configure Clock signal format (Phase Polarity and choice about output of last bit clock
 * pulse)
 * @note   Macro IS_USART_INSTANCE(USARTx) can be used to check whether or not
 *         Synchronous mode is supported by the USARTx instance.
 ...
 *         LSB First means data is transmitted/received with data bit 0 first, following the start
 * bit.
 ...
 * @note   Macro IS_USART_AUTOBAUDRATE_DETECTION_INSTANCE(USARTx) can be used to check whether or
 * not Auto Baud Rate detection feature is supported by the USARTx instance.
 ...
 * @retval None
 */
__STATIC_INLINE void LL_USART_ConfigNodeAddress(USART_TypeDef* USARTx, uint32_t AddressLen,
...
__STATIC_INLINE void LL_USART_SetBaudRate(USART_TypeDef* USARTx, uint32_t PeriphClk,
    ...
    if (PrescalerValue > LL_USART_PRESCALER_DIV256) {
        ...
        usartdiv =
            (uint16_t)(__LL_USART_DIV_SAMPLING8(PeriphClk, (uint8_t)PrescalerValue, BaudRate));
        ...
        USARTx->BRR =
            (uint16_t)(__LL_USART_DIV_SAMPLING16(PeriphClk, (uint8_t)PrescalerValue, BaudRate));
    }
}
 ...
 * @note   In case of non-initialized or invalid value stored in BRR register, value 0 will be
 * returned.
 * @note   In case of oversampling by 16 and 8, BRR content must be greater than or equal to 16d.
...
/** @defgroup USART_LL_EF_Configuration_Smartcard Configuration functions related to Smartcard
 * feature
 * @{
 */
 ...
 * @note   Macro IS_SMARTCARD_INSTANCE(USARTx) can be used to check whether or not
 *         Smartcard feature is supported by the USARTx instance.
 * @note   This bit-field specifies the number of retries in transmit and receive, in Smartcard
 * mode. In transmission mode, it specifies the number of automatic retransmission retries, before
 *         generating a transmission error (FE bit set).
 *         In reception mode, it specifies the number or erroneous reception trials, before
...
/** @defgroup USART_LL_EF_Configuration_HalfDuplex Configuration functions related to Half Duplex
 * feature
 * @{
 */
...
/** @defgroup USART_LL_EF_Configuration_SPI_SLAVE Configuration functions related to SPI Slave
 * feature
 * @{
 */
/**
...
/**
 * @brief  Check if the USART Transmit Data Register Empty or USART TX FIFO Not Full Flag is set or
 * not
 * @note   Macro IS_UART_FIFO_INSTANCE(USARTx) can be used to check whether or not
 *         FIFO mode feature is supported by the USARTx instance.
 ...
 * @note   When set, Error Interrupt Enable Bit is enabling interrupt generation in case of a
 * framing error, overrun error or noise flag (FE=1 or ORE=1 or NF=1 in the USARTx_ISR register). 0:
 * Interrupt is inhibited 1: An interrupt is generated when FE=1 or ORE=1 or NF=1 in the USARTx_ISR
 * register.
...
/**
 * @brief  Check if the USART RX Not Empty and USART RX FIFO Not Empty Interrupt is enabled or
 * disabled.
 * @note   Macro IS_UART_FIFO_INSTANCE(USARTx) can be used to check whether or not
 *         FIFO mode feature is supported by the USARTx instance.
...
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_RXNE_RXFNE(const USART_TypeDef* USARTx) {
    return ((READ_BIT(USARTx->CR1, USART_CR1_RXNEIE_RXFNEIE) == (USART_CR1_RXNEIE_RXFNEIE)) ? 1UL
                                                                                            : 0UL);
}
...
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_TXE_TXFNF(const USART_TypeDef* USARTx) {
    return ((READ_BIT(USARTx->CR1, USART_CR1_TXEIE_TXFNFIE) == (USART_CR1_TXEIE_TXFNFIE)) ? 1UL
                                                                                          : 0UL);
}
...
/**
 * @brief  Check if the Smartcard Transmission Complete Before Guard Time Interrupt is enabled or
 * disabled.
 * @note   Macro IS_SMARTCARD_INSTANCE(USARTx) can be used to check whether or not
 *         Smartcard feature is supported by the USARTx instance.
