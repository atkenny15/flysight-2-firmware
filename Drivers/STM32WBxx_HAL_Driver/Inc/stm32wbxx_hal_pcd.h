...
#    if (USE_HAL_PCD_REGISTER_CALLBACKS == 1U)
    ...
    void (*SetupStageCallback)(
        struct __PCD_HandleTypeDef* hpcd); /*!< USB OTG PCD Setup Stage callback        */
...
#    if (USE_HAL_PCD_REGISTER_CALLBACKS == 1U)
/** @defgroup HAL_PCD_Callback_ID_enumeration_definition HAL USB OTG PCD Callback ID enumeration
 * definition
...
#    define PCD_SET_EP_TX_ADDRESS(USBx, bEpNum, wAddr)                                         \
        do {                                                                                   \
            ...
            _wRegVal =                                                                         \
                (__IO uint16_t*)(_wRegBase + 0x400U + (((uint32_t)(bEpNum)*8U) * PMA_ACCESS)); \
            *_wRegVal = ((wAddr) >> 1) << 1;                                                   \
