...
typedef struct {
    ...
    uint32_t
        MicPairsNbr; /*!< Specifies the number of microphone pairs used.
                          This parameter must be a number between Min_Data = 1 and Max_Data = 3. */
    uint32_t
        ClockEnable; /*!< Specifies which clock must be enabled.
...
typedef struct {
    ...
    uint32_t NoDivider; /*!< Specifies whether master clock will be divided or not.
                             ...
                             @note If bit NODIV in the SAI_xCR1 register is cleared, the frame
                           length should be aligned to a number equal to a power of 2, from 8 to
                           256. If bit NODIV in the SAI_xCR1 register is set, the frame length can
    ...
    uint32_t
        MckOverSampling; /*!< Specifies the master clock oversampling.
    ...
    /* This part of the structure is automatically filled if your are using the high level
       initialisation function HAL_SAI_InitProtocol */
...
typedef struct {
    ...
    uint32_t
        FrameLength; /*!< Specifies the Frame length, the number of SCK clocks for each audio frame.
                          This parameter must be a number between Min_Data = 8 and Max_Data = 256.
                          @note If master clock MCLK_x pin is declared as an output, the frame
                        length should be aligned to a number equal to power of 2 in order to keep in
                        an audio frame, an integer number of MCLK pulses by bit Clock. */

    uint32_t
        ActiveFrameLength; /*!< Specifies the Frame synchronization active level length.
...
typedef struct {
    ...
    uint32_t
        SlotNumber; /*!< Specifies the number of slot in the audio frame.
...
typedef struct __SAI_HandleTypeDef {
    ...
    void (*InterruptServiceRoutine)(
        struct __SAI_HandleTypeDef* hsai); /* function pointer for IRQ handler */
...
#    if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
    ...
    void (*RxHalfCpltCallback)(
        struct __SAI_HandleTypeDef* hsai); /*!< SAI receive half complete callback */
    void (*TxCpltCallback)(struct __SAI_HandleTypeDef* hsai); /*!< SAI transmit complete callback */
    void (*TxHalfCpltCallback)(
        struct __SAI_HandleTypeDef* hsai); /*!< SAI transmit half complete callback */
 ...
 * @{
 */
HAL_StatusTypeDef HAL_SAI_InitProtocol(SAI_HandleTypeDef* hsai, uint32_t protocol,
                                       uint32_t datasize, uint32_t nbslot);
...
#    define IS_SAI_SUPPORTED_PROTOCOL(PROTOCOL)                                      \
        (((PROTOCOL) == SAI_I2S_STANDARD) || ((PROTOCOL) == SAI_I2S_MSBJUSTIFIED) || \
         ((PROTOCOL) == SAI_I2S_LSBJUSTIFIED) || ((PROTOCOL) == SAI_PCM_LONG) ||     \
