 ...
 * InjectedNbrOfConversion, InjectedDiscontinuousConvMode, AutoInjectedConv, QueueInjectedContext,
 * ExternalTrigInjecConv, ExternalTrigInjecConvEdge, InjecOversamplingMode, InjecOversampling.
 ...
 * 'ExternalTrigInjecConv', 'ExternalTrigInjecConvEdge': ADC enabled and while conversion on going
 *            on ADC groups regular and injected.
 *         If ADC is not in the appropriate state to modify some parameters, these parameters
 * setting is bypassed without error reporting (as it can be the expected behavior in case of
 * intended action to update another parameter (which fulfills the ADC state condition) on the fly).
 */
typedef struct {
    uint32_t InjectedChannel; /*!< Specifies the channel to configure into ADC group injected.
                                   This parameter can be a value of @ref ADC_HAL_EC_CHANNEL
                                   Note: Depending on devices and ADC instances, some channels may
                                 not be available on device package pins. Refer to device datasheet
    ...
    uint32_t InjectedRank; /*!< Specifies the rank in the ADC group injected sequencer.
                              ...
                              rank containing a previous channel setting can be overwritten by the
                              new channel setting (or parameter number of conversions adjusted) */
    ...
    uint32_t
        InjectedSamplingTime; /*!< Sampling time value to be set for the selected channel.
                                 ...
                                 at 10 bits, 8.5 cycles at 8 bits, 6.5 cycles at 6 bits). This
                                 parameter can be a value of @ref ADC_HAL_EC_CHANNEL_SAMPLINGTIME.
                                   Caution: This parameter applies to a channel that can be used in
                                 a regular and/or injected group. It overwrites the last setting.
    ...
    uint32_t InjectedSingleDiff; /*!< Selection of single-ended or differential input.
                                    ...
                                    selected channel 'i' (positive input) and channel 'i+1'
                                    (negative input). Only channel 'i' has to be configured, channel
                                    'i+1' is configured automatically. This parameter must be a
                                    value of @ref ADC_HAL_EC_CHANNEL_SINGLE_DIFF_ENDING. Caution:
                                    This parameter applies to a channel that can be used in a
                                    regular and/or injected group. It overwrites the last setting.
                                      Note: Refer to Reference Manual to ensure the selected channel
                                    is available in differential mode. Note: When configuring a
                                    channel 'i' in differential mode, the channel 'i+1' is not
                                    usable separately. Note: This parameter must be modified when
                                    ADC is disabled (before ADC start conversion or after ADC stop
                                    conversion). If ADC is enabled, this parameter setting is
                                    bypassed without error reporting (as it can be the expected
    ...
    uint32_t InjectedOffset; /*!< Defines the offset to be subtracted from the raw converted data.
                                ...
                                0xFFF,  0x3FF, 0xFF or 0x3F respectively. Note: This parameter must
                                be modified when no conversion is on going on both regular and
                                injected groups (ADC disabled, or ADC enabled without continuous
                                mode or external trigger that could launch a conversion). */

    uint32_t InjectedNbrOfConversion; /*!< Specifies the number of ranks that will be converted
                                         within the ADC group injected sequencer. To use the
                                         injected group sequencer and convert several ranks,
                                         parameter 'ScanConvMode' must be enabled. This parameter
                                         must be a number between Min_Data = 1 and Max_Data = 4.
    ...
    FunctionalState
        InjectedDiscontinuousConvMode; /*!< Specifies whether the conversions sequence of ADC group
                                     ...
                                     disabled. This parameter can be set to ENABLE or DISABLE. Note:
                                     This parameter must be modified when ADC is disabled (before
                                     ADC start conversion or after ADC stop conversion). Note: For
                                     injected group, discontinuous mode converts the sequence
    ...
    FunctionalState
        AutoInjectedConv; /*!< Enables or disables the selected ADC group injected automatic
                             ...
                             DISABLE. Note: To use Automatic injected conversion, discontinuous mode
                             must be disabled ('DiscontinuousConvMode' and
                             'InjectedDiscontinuousConvMode' set to DISABLE) Note: To use Automatic
                             injected conversion, injected group external triggers must be disabled
                             ('ExternalTrigInjecConv' set to ADC_INJECTED_SOFTWARE_START) Note: In
                             case of DMA used with regular group: if DMA configured in normal mode
                             (single shot) JAUTO will be stopped upon DMA transfer complete. To
                             maintain JAUTO always enabled, DMA must be configured in circular mode.
    ...
    FunctionalState
        QueueInjectedContext; /*!< Specifies whether the context queue feature is enabled.
                                 ...
                                 channels with as many calls to HAL_ADCEx_InjectedConfigChannel() as
                                 the 'InjectedNbrOfConversion' parameter. Caution: this setting
    ...
    uint32_t ExternalTrigInjecConv; /*!< Selects the external event used to trigger the conversion
                                       start of injected group. If set to
                                       ADC_INJECTED_SOFTWARE_START, external triggers are disabled
                                       and software trigger is used instead. This parameter can be a
                                       value of @ref ADC_injected_external_trigger_source. Caution:
                                       this setting impacts the entire injected group. Therefore,
...
#    define ADC_EXTERNALTRIGINJEC_T1_TRGO                                                         \
        (LL_ADC_INJ_TRIG_EXT_TIM1_TRGO) /*!< ADC group injected conversion trigger from external  \
                                           peripheral: TIM1 TRGO. Trigger edge set to rising edge \
                                           (default setting). */
#    define ADC_EXTERNALTRIGINJEC_T1_TRGO2                                                        \
        (LL_ADC_INJ_TRIG_EXT_TIM1_TRGO2) /*!< ADC group injected conversion trigger from external \
                                            peripheral: TIM1 TRGO2. Trigger edge set to rising    \
                                            edge (default setting). */
#    define ADC_EXTERNALTRIGINJEC_T1_CC4                                                           \
        (LL_ADC_INJ_TRIG_EXT_TIM1_CH4) /*!< ADC group injected conversion trigger from external    \
...
#    define ADC_EXTERNALTRIGINJEC_T2_TRGO                                                         \
        (LL_ADC_INJ_TRIG_EXT_TIM2_TRGO) /*!< ADC group injected conversion trigger from external  \
                                           peripheral: TIM2 TRGO. Trigger edge set to rising edge \
                                           (default setting). */
#    define ADC_EXTERNALTRIGINJEC_T2_CC1                                                           \
        (LL_ADC_INJ_TRIG_EXT_TIM2_CH1) /*!< ADC group injected conversion trigger from external    \
                                          peripheral: TIM2 channel 1 event (capture compare: input \
                                          capture or output capture). Trigger edge set to rising   \
                                          edge (default setting). */
#    define ADC_EXTERNALTRIGINJEC_EXT_IT15                                                         \
        (LL_ADC_INJ_TRIG_EXT_EXTI_LINE15) /*!< ADC group injected conversion trigger from external \
                                             peripheral: external interrupt line 15. Trigger edge  \
                                             set to rising edge (default setting). */
/**
 * @}
 */

/** @defgroup ADCEx_injected_external_trigger_edge ADC group injected trigger edge (when external
 * trigger is selected)
 * @{
 */
...
#    define ADC_EXTERNALTRIGINJECCONV_EDGE_RISING                                              \
        (ADC_JSQR_JEXTEN_0) /*!< Injected conversions hardware trigger detection on the rising \
                               edge                   */
#    define ADC_EXTERNALTRIGINJECCONV_EDGE_FALLING                                              \
        (ADC_JSQR_JEXTEN_1) /*!< Injected conversions hardware trigger detection on the falling \
                               edge                  */
#    define ADC_EXTERNALTRIGINJECCONV_EDGE_RISINGFALLING                                          \
        (ADC_JSQR_JEXTEN) /*!< Injected conversions hardware trigger detection on both the rising \
                             and falling edges */
/**
 * @}
 */
...
#define ADC_SINGLE_ENDED                                                                        \
    (LL_ADC_SINGLE_ENDED) /*!< ADC channel ending set to single ended (literal also used to set \
                             calibration mode) */
#if !defined(ADC_SUPPORT_2_5_MSPS)
#    define ADC_DIFFERENTIAL_ENDED                                                                 \
        (LL_ADC_DIFFERENTIAL_ENDED) /*!< ADC channel ending set to differential (literal also used \
...
#define ADC_OFFSET_NONE \
    (ADC_OFFSET_4 +     \
     1U) /*!< ADC offset disabled: no offset correction for the selected ADC channel */
#define ADC_OFFSET_1                                                                              \
    (LL_ADC_OFFSET_1) /*!< ADC offset number 1: ADC channel and offset level to which the offset  \
                         programmed will be applied (independently of channel mapped on ADC group \
                         regular or group injected) */
#define ADC_OFFSET_2                                                                              \
    (LL_ADC_OFFSET_2) /*!< ADC offset number 2: ADC channel and offset level to which the offset  \
                         programmed will be applied (independently of channel mapped on ADC group \
                         regular or group injected) */
#define ADC_OFFSET_3                                                                              \
    (LL_ADC_OFFSET_3) /*!< ADC offset number 3: ADC channel and offset level to which the offset  \
                         programmed will be applied (independently of channel mapped on ADC group \
                         regular or group injected) */
#define ADC_OFFSET_4                                                                              \
    (LL_ADC_OFFSET_4) /*!< ADC offset number 4: ADC channel and offset level to which the offset  \
                         programmed will be applied (independently of channel mapped on ADC group \
                         regular or group injected) */
/**
 * @}
...
#define ADC_CFGR_FIELDS                                                                         \
    (ADC_CFGR_AWD1CH | ADC_CFGR_JAUTO | ADC_CFGR_JAWD1EN | ADC_CFGR_AWD1EN | ADC_CFGR_AWD1SGL | \
     ADC_CFGR_JQM | ADC_CFGR_JDISCEN | ADC_CFGR_DISCNUM | ADC_CFGR_DISCEN | ADC_CFGR_AUTDLY |   \
     ADC_CFGR_CONT | ADC_CFGR_OVRMOD | ADC_CFGR_EXTEN | ADC_CFGR_EXTSEL | ADC_CFGR_ALIGN |      \
...
#define ADC_SMPR1_FIELDS                                                                  \
    (ADC_SMPR1_SMP9 | ADC_SMPR1_SMP8 | ADC_SMPR1_SMP7 | ADC_SMPR1_SMP6 | ADC_SMPR1_SMP5 | \
     ADC_SMPR1_SMP4 | ADC_SMPR1_SMP3 | ADC_SMPR1_SMP2 | ADC_SMPR1_SMP1 | ADC_SMPR1_SMP0)
/**
 * @}
 */
 ...
 * @retval None
 */
#    define ADC_CFGR_AUTOOFF(_AUTOOFF_) ((_AUTOOFF_) << 15UL)
...
#    define ADC_AWD23THRESHOLD_SHIFT_RESOLUTION(__HANDLE__, __THRESHOLD__)                        \
        ((((__HANDLE__)->Instance->CFGR1 & ADC_CFGR1_RES) != (ADC_CFGR1_RES_1 | ADC_CFGR1_RES_0)) \
             ? ((__THRESHOLD__) >>                                                                \
                ((4UL - ((((__HANDLE__)->Instance->CFGR1 & ADC_CFGR1_RES) >> 3UL) * 2UL)) &       \
                 0x1FUL))                                                                         \
             : ((__THRESHOLD__) << 2UL))
#else
#    define ADC_AWD23THRESHOLD_SHIFT_RESOLUTION(__HANDLE__, __THRESHOLD__)                         \
        ((((__HANDLE__)->Instance->CFGR & ADC_CFGR_RES) != (ADC_CFGR_RES_1 | ADC_CFGR_RES_0))      \
             ? ((__THRESHOLD__) >>                                                                 \
                ((4UL - ((((__HANDLE__)->Instance->CFGR & ADC_CFGR_RES) >> 3UL) * 2UL)) & 0x1FUL)) \
             : ((__THRESHOLD__) << 2UL))
#endif /* ADC_SUPPORT_2_5_MSPS */
...
#    define ADC_CLEAR_COMMON_CONTROL_REGISTER(__HANDLE__)                \
        CLEAR_BIT(__LL_ADC_COMMON_INSTANCE((__HANDLE__)->Instance)->CCR, \
                  ADC_CCR_PRESC | ADC_CCR_VBATEN | ADC_CCR_TSEN | ADC_CCR_VREFEN)
#else
#    define ADC_CLEAR_COMMON_CONTROL_REGISTER(__HANDLE__)                \
        CLEAR_BIT(__LL_ADC_COMMON_INSTANCE((__HANDLE__)->Instance)->CCR, \
 ...
 * @retval SET (__LENGTH__ is within the maximum number of possible programmable injected
...
#define IS_ADC_CHANNEL(__HANDLE__, __CHANNEL__)                                      \
    ((((__HANDLE__)->Instance) == ADC1) &&                                           \
     (((__CHANNEL__) == ADC_CHANNEL_0) || ((__CHANNEL__) == ADC_CHANNEL_1) ||        \
      ...
      ((__CHANNEL__) == ADC_CHANNEL_18) || ((__CHANNEL__) == ADC_CHANNEL_VREFINT) || \
...
#define IS_ADC_DIFF_CHANNEL(__HANDLE__, __CHANNEL__)                           \
    (((__CHANNEL__) == ADC_CHANNEL_1) || ((__CHANNEL__) == ADC_CHANNEL_2) ||   \
     ...
     ((__CHANNEL__) == ADC_CHANNEL_13) || ((__CHANNEL__) == ADC_CHANNEL_14) || \
...
#    define IS_ADC_TRIGGER_FREQ(TRIGGER_FREQ)            \
        (((TRIGGER_FREQ) == LL_ADC_TRIGGER_FREQ_HIGH) || \
...
#    define IS_ADC_EVENT_TYPE(__EVENT__)                                        \
        (((__EVENT__) == ADC_EOSMP_EVENT) || ((__EVENT__) == ADC_AWD1_EVENT) || \
 ...
 * @param __HANDLE__ ADC handle.
 * @note When DMSDFM configuration is not supported, the macro systematically reports 0x0 (i.e
 * disabled). For this reason, the input parameter is the ADC handle and not the configuration
 * parameter directly.
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef* hadc,
                                                  const ADC_InjectionConfTypeDef* sConfigInjected);
#endif /* ADC_SUPPORT_2_5_MSPS */
#if defined(ADC_SUPPORT_2_5_MSPS)
/* Feature "ADC group injected" not available on ADC peripheral of this STM32WB device */
#else
