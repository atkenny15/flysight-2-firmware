 ...
 */
typedef struct {
    uint32_t WakeUpEvent; /*!< Specifies which event will activate the Wakeup from Stop mode flag
                             (WUF). This parameter can be a value of @ref
                             UART_WakeUp_from_Stop_Selection. If set to UART_WAKEUP_ON_ADDRESS, the
                             two other fields below must be filled up. */

    uint16_t
        AddressLength; /*!< Specifies whether the address is 4 or 7-bit long.
...
#define UART_TXFIFO_THRESHOLD_3_4 \
    (USART_CR3_TXFTCFG_0 | USART_CR3_TXFTCFG_1)       /*!< TX FIFO reaches 3/4 of its depth */
...
#define UART_RXFIFO_THRESHOLD_3_4 \
    (USART_CR3_RXFTCFG_0 | USART_CR3_RXFTCFG_1)       /*!< RX FIFO reaches 3/4 of its depth */
