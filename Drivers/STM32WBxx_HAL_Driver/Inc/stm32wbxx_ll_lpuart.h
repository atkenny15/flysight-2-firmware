 ...
 * @{
 */
/* Array used to get the LPUART prescaler division decimal values versus @ref LPUART_LL_EC_PRESCALER
 * values */
static const uint16_t LPUART_PRESCALER_TAB[] = { (uint16_t)1,  (uint16_t)2,   (uint16_t)4,
                                                 (uint16_t)6,  (uint16_t)8,   (uint16_t)10,
                                                 (uint16_t)12, (uint16_t)16,  (uint16_t)32,
                                                 (uint16_t)64, (uint16_t)128, (uint16_t)256 };
/**
 * @}
 */
...
typedef struct {
    ...
    uint32_t TransferDirection; /*!< Specifies whether the Receive and/or Transmit mode is enabled
                                   or disabled. This parameter can be a value of @ref
                                   LPUART_LL_EC_DIRECTION.
...
#    define __LL_LPUART_DIV(__PERIPHCLK__, __PRESCALER__, __BAUDRATE__)               \
        (uint32_t)((((((uint64_t)(__PERIPHCLK__) /                                    \
 ...
 *         and current operations are discarded. The configuration of the LPUART is kept, but all
 * the status flags, in the LPUARTx_ISR are set to their default values.
 ...
 * @note   When this function is enabled, LPUART is able to wake up the MCU from Stop mode, provided
 * that LPUART clock selection is HSI or LSE in RCC.
 ...
 * the MSB position (depending on data width) and parity is checked on the received data.
 ...
 * @retval None
 */
__STATIC_INLINE void LL_LPUART_ConfigNodeAddress(USART_TypeDef* LPUARTx, uint32_t AddressLen,
...
/** @defgroup LPUART_LL_EF_Configuration_HalfDuplex Configuration functions related to Half Duplex
 * feature
 * @{
 */
 ...
 * @param  LPUARTx LPUART Instance
 * @retval Time value expressed on 5 bits ([4:0] bits) : Time Value between Min_Data=0 and
 * Max_Data=31
 */
...
/**
 * @brief  Check if the LPUART Transmit Data Register Empty or LPUART TX FIFO Not Full Flag is set
 * or not
 ...
 * framing error, overrun error or noise flag (FE=1 or ORE=1 or NF=1 in the LPUARTx_ISR register).
...
/**
 * @brief  Check if the LPUART RX Not Empty and LPUART RX FIFO Not Empty Interrupt is enabled or
 * disabled.
...
__STATIC_INLINE uint32_t LL_LPUART_IsEnabledIT_RXNE_RXFNE(const USART_TypeDef* LPUARTx) {
    return ((READ_BIT(LPUARTx->CR1, USART_CR1_RXNEIE_RXFNEIE) == (USART_CR1_RXNEIE_RXFNEIE)) ? 1UL
                                                                                             : 0UL);
}
...
__STATIC_INLINE uint32_t LL_LPUART_IsEnabledIT_TXE_TXFNF(const USART_TypeDef* LPUARTx) {
    return ((READ_BIT(LPUARTx->CR1, USART_CR1_TXEIE_TXFNFIE) == (USART_CR1_TXEIE_TXFNFIE)) ? 1UL
                                                                                           : 0UL);
}
