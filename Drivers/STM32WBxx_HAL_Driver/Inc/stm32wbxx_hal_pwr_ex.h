 ...
 */
typedef struct {
    uint32_t PVMType; /*!< PVMType: Specifies which voltage is monitored and against which
                         threshold. This parameter can be a value of @ref PWREx_PVM_Type.
                           @arg @ref PWR_PVM_1 Peripheral Voltage Monitoring 1 enable: VDDUSB
                         versus 1.2 V (applicable when USB feature is supported).
                           @arg @ref PWR_PVM_3 Peripheral Voltage Monitoring 3 enable: VDDA
                         versus 1.62 V.
                           */
...
typedef struct {
    uint32_t
        StartupCurrent; /*!< SMPS step down converter supply startup current selection.
                             This parameter can be a value of @ref PWREx_SMPS_STARTUP_CURRENT. */

    uint32_t
        OutputVoltage; /*!< SMPS step down converter output voltage scaling voltage level.
...
/** @defgroup PWREx_WUP_Polarity Shift to apply to retrieve polarity information from
...
#    define PWR_PVM_1                                                                              \
        PWR_CR2_PVME1 /*!< Peripheral Voltage Monitoring 1 enable: VDDUSB versus 1.2 V (applicable \
                         when USB feature is supported) */
#endif                /* PWR_CR2_PVME1 */
...
#define PWR_PVM_MODE_IT_RISING \
    (PVM_MODE_IT |             \
     PVM_RISING_EDGE) /*!< External Interrupt Mode with Rising edge trigger detection */
#define PWR_PVM_MODE_IT_FALLING \
    (PVM_MODE_IT |              \
     PVM_FALLING_EDGE) /*!< External Interrupt Mode with Falling edge trigger detection */
#define PWR_PVM_MODE_IT_RISING_FALLING                                                            \
    (PVM_MODE_IT | PVM_RISING_FALLING_EDGE) /*!< External Interrupt Mode with Rising/Falling edge \
                                               trigger detection */

#define PWR_PVM_MODE_EVENT_RISING \
    (PVM_MODE_EVT | PVM_RISING_EDGE) /*!< Event Mode with Rising edge trigger detection */
#define PWR_PVM_MODE_EVENT_FALLING \
    (PVM_MODE_EVT | PVM_FALLING_EDGE) /*!< Event Mode with Falling edge trigger detection */
#define PWR_PVM_MODE_EVENT_RISING_FALLING \
    (PVM_MODE_EVT |                       \
     PVM_RISING_FALLING_EDGE) /*!< Event Mode with Rising/Falling edge trigger detection */
/**
 * @}
 */
...
#    define PWR_REGULATOR_VOLTAGE_SCALE1                                                           \
        PWR_CR1_VOS_0 /*!< Regulator voltage output range 1 mode, typical output voltage at 1.2 V, \
                         system frequency up to 64 MHz */
#    define PWR_REGULATOR_VOLTAGE_SCALE2                                                           \
        PWR_CR1_VOS_1 /*!< Regulator voltage output range 2 mode, typical output voltage at 1.0 V, \
                         system frequency up to 16 MHz */
#else
#    define PWR_REGULATOR_VOLTAGE_SCALE1                                                         \
        (0x00000200UL) /*!< Regulator voltage output range 1 mode, typical output voltage at 1.2 \
                          V, system frequency up to 64 MHz */
#endif                 /* PWR_CR1_VOS */
/**
 * @}
 */
...
#    define PWR_BOR_SYSTEM_RESET (LL_PWR_BOR_SYSTEM_RESET) /*!< BOR will generate a system reset \
                                                            */
#    define PWR_BOR_SMPS_FORCE_BYPASS                                                            \
        (LL_PWR_BOR_SMPS_FORCE_BYPASS) /*!< BOR will for SMPS step down converter in bypass mode \
                                        */
/**
 * @}
 */
...
#    define PWR_SMPS_STEP_DOWN                                                                     \
        (PWR_SR2_SMPSF) /*!< SMPS step down in step down mode if system low power mode is run, LP  \
                           run or stop0. If system low power mode is stop1, stop2, standby,        \
                           shutdown, then SMPS is forced in mode open to preserve energy stored in \
                           decoupling capacitor as long as possible. */
/**
 * @}
 */
 ...
 * @{
 */
#    define PWR_SMPS_STARTUP_CURRENT_80MA \
        (0x00000000U) /*!< SMPS step down converter supply startup current 80mA */
#    define PWR_SMPS_STARTUP_CURRENT_100MA \
        (PWR_CR5_SMPSSC_0) /*!< SMPS step down converter supply startup current 100mA */
#    define PWR_SMPS_STARTUP_CURRENT_120MA \
        (PWR_CR5_SMPSSC_1) /*!< SMPS step down converter supply startup current 120mA */
#    define PWR_SMPS_STARTUP_CURRENT_140MA \
        (PWR_CR5_SMPSSC_1 |                \
         PWR_CR5_SMPSSC_0) /*!< SMPS step down converter supply startup current 140mA */
#    define PWR_SMPS_STARTUP_CURRENT_160MA \
        (PWR_CR5_SMPSSC_2) /*!< SMPS step down converter supply startup current 160mA */
#    define PWR_SMPS_STARTUP_CURRENT_180MA \
        (PWR_CR5_SMPSSC_2 |                \
         PWR_CR5_SMPSSC_0) /*!< SMPS step down converter supply startup current 180mA */
#    define PWR_SMPS_STARTUP_CURRENT_200MA \
        (PWR_CR5_SMPSSC_2 |                \
         PWR_CR5_SMPSSC_1) /*!< SMPS step down converter supply startup current 200mA */
#    define PWR_SMPS_STARTUP_CURRENT_220MA     \
        (PWR_CR5_SMPSSC_2 | PWR_CR5_SMPSSC_1 | \
         PWR_CR5_SMPSSC_0) /*!< SMPS step down converter supply startup current 220mA */
/**
 * @}
 */

/** @defgroup PWREx_SMPS_OUTPUT_VOLTAGE_LEVEL SMPS step down converter output voltage scaling
 * voltage level
 * @{
 */
/* Note: SMPS voltage is trimmed during device production to control
         the actual voltage level variation from device to device. */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V20 \
        (0x00000000U) /*!< SMPS step down converter supply output voltage 1.20V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V25 \
        (PWR_CR5_SMPSVOS_0) /*!< SMPS step down converter supply output voltage 1.25V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V30 \
        (PWR_CR5_SMPSVOS_1) /*!< SMPS step down converter supply output voltage 1.30V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V35 \
        (PWR_CR5_SMPSVOS_1 |             \
         PWR_CR5_SMPSVOS_0) /*!< SMPS step down converter supply output voltage 1.35V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V40 \
        (PWR_CR5_SMPSVOS_2) /*!< SMPS step down converter supply output voltage 1.40V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V45 \
        (PWR_CR5_SMPSVOS_2 |             \
         PWR_CR5_SMPSVOS_0) /*!< SMPS step down converter supply output voltage 1.45V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V50 \
        (PWR_CR5_SMPSVOS_2 |             \
         PWR_CR5_SMPSVOS_1) /*!< SMPS step down converter supply output voltage 1.50V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V55         \
        (PWR_CR5_SMPSVOS_2 | PWR_CR5_SMPSVOS_1 | \
         PWR_CR5_SMPSVOS_0) /*!< SMPS step down converter supply output voltage 1.55V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V60 \
        (PWR_CR5_SMPSVOS_3) /*!< SMPS step down converter supply output voltage 1.60V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V65 \
        (PWR_CR5_SMPSVOS_3 |             \
         PWR_CR5_SMPSVOS_0) /*!< SMPS step down converter supply output voltage 1.65V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V70 \
        (PWR_CR5_SMPSVOS_3 |             \
         PWR_CR5_SMPSVOS_1) /*!< SMPS step down converter supply output voltage 1.70V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V75         \
        (PWR_CR5_SMPSVOS_3 | PWR_CR5_SMPSVOS_1 | \
         PWR_CR5_SMPSVOS_0) /*!< SMPS step down converter supply output voltage 1.75V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V80 \
        (PWR_CR5_SMPSVOS_3 |             \
         PWR_CR5_SMPSVOS_2) /*!< SMPS step down converter supply output voltage 1.80V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V85         \
        (PWR_CR5_SMPSVOS_3 | PWR_CR5_SMPSVOS_2 | \
         PWR_CR5_SMPSVOS_0) /*!< SMPS step down converter supply output voltage 1.85V */
#    define PWR_SMPS_OUTPUT_VOLTAGE_1V90         \
        (PWR_CR5_SMPSVOS_3 | PWR_CR5_SMPSVOS_2 | \
         PWR_CR5_SMPSVOS_1) /*!< SMPS step down converter supply output voltage 1.90V */
/**
 * @}
 */
#endif /* PWR_CR5_SMPSEN */
...
#endif                                                          /* PWR_CR3_EWUP5 */
#define PWR_FLAG_WU \
    (PWR_FLAG_REG_SR1 | PWR_SR1_WUF) /*!< Encompass wakeup event on all wakeup pins */

#if defined(PWR_CR5_SMPSEN)
#    define PWR_FLAG_FRCBYPI \
        (PWR_FLAG_REG_SR1 | PWR_SR1_SMPSFBF_Pos) /*!< SMPS Forced in Bypass Interrupt Flag */
#endif                                           /* PWR_CR5_SMPSEN */
...
#define PWR_FLAG_BLEACTI (PWR_FLAG_REG_SR1 | PWR_SR1_BLEAF_Pos)  /*!< BLE Activity Interrupt Flag \
                                                                  */
#define PWR_FLAG_802ACTI \
    (PWR_FLAG_REG_SR1 | PWR_SR1_802AF_Pos) /*!< 802.15.4 Activity Interrupt Flag */
...
#define PWR_FLAG_REGLPS \
    (PWR_FLAG_REG_SR2 | PWR_SR2_REGLPS_Pos) /*!< Low-power regulator start flag */
...
#define PWR_FLAG_PVDO \
    (PWR_FLAG_REG_SR2 | PWR_SR2_PVDO_Pos) /*!< Power Voltage Detector output flag */

#if defined(PWR_CR2_PVME1)
#    define PWR_FLAG_PVMO1 \
        (PWR_FLAG_REG_SR2 | PWR_SR2_PVMO1_Pos) /*!< Power Voltage Monitoring 1 output flag */
#endif                                         /* PWR_CR2_PVME1 */
#define PWR_FLAG_PVMO3 \
    (PWR_FLAG_REG_SR2 | PWR_SR2_PVMO3_Pos) /*!< Power Voltage Monitoring 3 output flag */

/*------------------------------EXTSCR---------------------------*/
#define PWR_FLAG_SB                               \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C1SBF_Pos | \
     (PWR_EXTSCR_C1CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Standby flag for CPU1 */
#define PWR_FLAG_STOP                               \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C1STOPF_Pos | \
     (PWR_EXTSCR_C1CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Stop flag for CPU1 */

#define PWR_FLAG_C2SB                             \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C2SBF_Pos | \
     (PWR_EXTSCR_C2CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Standby flag for CPU2 */
#define PWR_FLAG_C2STOP                             \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C2STOPF_Pos | \
     (PWR_EXTSCR_C2CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Stop flag for CPU2 */

#define PWR_FLAG_CRITICAL_RF_PHASE               \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_CRPF_Pos | \
     (PWR_EXTSCR_CCRPF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< Critical radio system phase flag */
...
#    define PWR_EXTI_LINE_PVM1 \
        (LL_EXTI_LINE_31)                    /*!< External interrupt line 31 Connected to PVM1 */
#endif                                       /* PWR_CR2_PVME1 */
...
#define PVM_RISING_FALLING_EDGE \
    (0x00000003U) /*!< Mask for rising and falling edges set as PVM trigger    */
/**
 * @}
 */
...
#define PWR_FLAG_REG_EXTSCR \
    (0x60UL) /* Bitfield to indicate PWR flag located in register PWR_EXTSCR */
#define PWR_FLAG_REG_MASK                  \
    (PWR_FLAG_REG_SR1 | PWR_FLAG_REG_SR2 | \
     PWR_FLAG_REG_EXTSCR) /* Bitfield mask to indicate PWR flag location in PWR register */
#define PWR_FLAG_EXTSCR_CLR_POS                                                                  \
    (16UL) /* Bitfield for register PWR_EXTSCR clearable bits positions: position of bitfield in \
              flag literals */
#define PWR_FLAG_EXTSCR_CLR_MASK                                                               \
    ((PWR_EXTSCR_C1CSSF_Pos | PWR_EXTSCR_C2CSSF_Pos | PWR_EXTSCR_CCRPF_Pos)                    \
     << PWR_FLAG_EXTSCR_CLR_POS) /* Bitfield for register PWR_EXTSCR clearable bits positions: \
                                    mask of bitfield in flag literals */
/**
 * @}
 */
...
#    define __HAL_PWR_PVM1_EXTI_ENABLE_RISING_EDGE() \
        LL_EXTI_EnableRisingTrig_0_31(PWR_EXTI_LINE_PVM1)
...
#    define __HAL_PWR_PVM1_EXTI_DISABLE_RISING_EDGE() \
        LL_EXTI_DisableRisingTrig_0_31(PWR_EXTI_LINE_PVM1)
...
#    define __HAL_PWR_PVM1_EXTI_ENABLE_FALLING_EDGE() \
        LL_EXTI_EnableFallingTrig_0_31(PWR_EXTI_LINE_PVM1)
...
#    define __HAL_PWR_PVM1_EXTI_DISABLE_FALLING_EDGE() \
        LL_EXTI_DisableFallingTrig_0_31(PWR_EXTI_LINE_PVM1)
...
#define __HAL_PWR_PVM3_EXTI_DISABLE_RISING_EDGE() \
    LL_EXTI_DisableRisingTrig_32_63(PWR_EXTI_LINE_PVM3)
...
#define __HAL_PWR_PVM3_EXTI_ENABLE_FALLING_EDGE() \
    LL_EXTI_EnableFallingTrig_32_63(PWR_EXTI_LINE_PVM3)
...
#define __HAL_PWR_PVM3_EXTI_DISABLE_FALLING_EDGE() \
    LL_EXTI_DisableFallingTrig_32_63(PWR_EXTI_LINE_PVM3)
...
#    define IS_PWR_WAKEUP_PIN(PIN)                                             \
        (((PIN) == PWR_WAKEUP_PIN1_HIGH) || ((PIN) == PWR_WAKEUP_PIN2_HIGH) || \
         ...
         ((PIN) == PWR_WAKEUP_PIN5_HIGH) || ((PIN) == PWR_WAKEUP_PIN1_LOW) ||  \
         ((PIN) == PWR_WAKEUP_PIN2_LOW) || ((PIN) == PWR_WAKEUP_PIN3_LOW) ||   \
         ((PIN) == PWR_WAKEUP_PIN4_LOW) || ((PIN) == PWR_WAKEUP_PIN5_LOW))
#else
#    define IS_PWR_WAKEUP_PIN(PIN)                                             \
        (((PIN) == PWR_WAKEUP_PIN1_HIGH) || ((PIN) == PWR_WAKEUP_PIN4_HIGH) || \
         ((PIN) == PWR_WAKEUP_PIN1_LOW) || ((PIN) == PWR_WAKEUP_PIN4_LOW))
#endif /* PWR_CR3_EWUP2 */

#define IS_PWR_WAKEUP_PIN_POLARITY(POLARITY) \
    (((POLARITY) == PWR_PIN_POLARITY_HIGH) || ((POLARITY) == PWR_PIN_POLARITY_LOW))

#if defined(PWR_CR2_PVME1)
#    define IS_PWR_PVM_TYPE(TYPE) (((TYPE) == PWR_PVM_1) || ((TYPE) == PWR_PVM_3))
#else
...
#    define IS_PWR_GPIO(GPIO)                                                          \
        (((GPIO) == PWR_GPIO_A) || ((GPIO) == PWR_GPIO_B) || ((GPIO) == PWR_GPIO_C) || \
         ((GPIO) == PWR_GPIO_D) || ((GPIO) == PWR_GPIO_E) || ((GPIO) == PWR_GPIO_H))
#else
#    define IS_PWR_GPIO(GPIO)                                                          \
        (((GPIO) == PWR_GPIO_A) || ((GPIO) == PWR_GPIO_B) || ((GPIO) == PWR_GPIO_C) || \
...
#if defined(PWR_CR5_SMPSEN)
#    define IS_PWR_SMPS_MODE(SMPS_MODE) \
        (((SMPS_MODE) == PWR_SMPS_BYPASS) || ((SMPS_MODE) == PWR_SMPS_STEP_DOWN))

#    define IS_PWR_SMPS_STARTUP_CURRENT(SMPS_STARTUP_CURRENT)          \
        (((SMPS_STARTUP_CURRENT) == PWR_SMPS_STARTUP_CURRENT_80MA) ||  \
...
#    define IS_PWR_SMPS_OUTPUT_VOLTAGE(SMPS_OUTPUT_VOLTAGE)         \
        (((SMPS_OUTPUT_VOLTAGE) == PWR_SMPS_OUTPUT_VOLTAGE_1V20) || \
