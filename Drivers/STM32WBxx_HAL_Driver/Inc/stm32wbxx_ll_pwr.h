 ...
 * @{
 */
#        define SMPS_VOLTAGE_CAL_ADDR                                                             \
            ((uint32_t*)(0x1FFF7558UL)) /* SMPS output voltage calibration level corresponding to \
                                           voltage "SMPS_VOLTAGE_CAL_VOLTAGE_MV" */
#        define SMPS_VOLTAGE_CAL_POS \
            (8UL) /* SMPS output voltage calibration level bitfield position */
#        define SMPS_VOLTAGE_CAL \
            (0xFUL               \
             << SMPS_VOLTAGE_CAL_POS) /* SMPS output voltage calibration level bitfield mask */
#        define SMPS_VOLTAGE_CAL_VOLTAGE_MV \
            (1500UL)                          /* SMPS output voltage calibration value (unit: mV) */
...
#        define LL_PWR_FLAG_SMPSB PWR_SR2_SMPSBF /* SMPS step down converter in bypass mode flag \
                                                  */
#        define LL_PWR_FLAG_SMPSFB \
            PWR_SR1_SMPSFB /* SMPS step down converter forced in bypass mode interrupt flag */
#    endif                 /* PWR_CR5_SMPSEN */
...
#        define LL_PWR_REGU_VOLTAGE_SCALE1                                                   \
            (PWR_CR1_VOS_0) /* Regulator voltage output range 1 mode, typical output voltage \
                               at 1.2 V, system frequency up to 64 MHz. */
#        define LL_PWR_REGU_VOLTAGE_SCALE2                                                   \
            (PWR_CR1_VOS_1) /* Regulator voltage output range 2 mode, typical output voltage \
                               at 1.0 V, system frequency up to 16 MHz. */
/**
 * @}
 */
#    endif /* PWR_CR1_VOS */
...
#    define LL_PWR_PVDLEVEL_7            \
        (PWR_CR2_PLS_2 | PWR_CR2_PLS_1 | \
         PWR_CR2_PLS_0) /* External input analog voltage   (Compare internally to VREFINT) */
/**
 * @}
 */
...
#        define LL_PWR_BOR_SMPS_FORCE_BYPASS \
            (PWR_CR5_BORHC) /*!< BOR will for SMPS step down converter in bypass mode */
/**
 * @}
 */
...
#        define LL_PWR_SMPS_STEP_DOWN                                                              \
            (PWR_SR2_SMPSF) /*!< SMPS step down in step down mode if system low power mode is run, \
                               LP run or stop0. If system low power mode is stop1, stop2, standby, \
                               shutdown, then SMPS is forced in mode open to preserve energy       \
                               stored in decoupling capacitor as long as possible. */
/**
 * @}
 */

/** @defgroup PWR_LL_EC_SMPS_STARTUP_CURRENT SMPS step down converter supply startup current
 * selection
 * @{
 */
#        define LL_PWR_SMPS_STARTUP_CURRENT_80MA \
            (0x00000000U) /*!< SMPS step down converter supply startup current 80mA */
#        define LL_PWR_SMPS_STARTUP_CURRENT_100MA \
            (PWR_CR5_SMPSSC_0) /*!< SMPS step down converter supply startup current 100mA */
#        define LL_PWR_SMPS_STARTUP_CURRENT_120MA \
            (PWR_CR5_SMPSSC_1) /*!< SMPS step down converter supply startup current 120mA */
#        define LL_PWR_SMPS_STARTUP_CURRENT_140MA \
            (PWR_CR5_SMPSSC_1 |                   \
             PWR_CR5_SMPSSC_0) /*!< SMPS step down converter supply startup current 140mA */
#        define LL_PWR_SMPS_STARTUP_CURRENT_160MA \
            (PWR_CR5_SMPSSC_2) /*!< SMPS step down converter supply startup current 160mA */
#        define LL_PWR_SMPS_STARTUP_CURRENT_180MA \
            (PWR_CR5_SMPSSC_2 |                   \
             PWR_CR5_SMPSSC_0) /*!< SMPS step down converter supply startup current 180mA */
#        define LL_PWR_SMPS_STARTUP_CURRENT_200MA \
            (PWR_CR5_SMPSSC_2 |                   \
             PWR_CR5_SMPSSC_1) /*!< SMPS step down converter supply startup current 200mA */
#        define LL_PWR_SMPS_STARTUP_CURRENT_220MA  \
 ...
 *
 *         (*) Not available on devices STM32WB50xx, STM32WB35xx, STM32WB30xx, STM32WB15xx,
...
__STATIC_INLINE void LL_PWR_SMPS_SetMode(uint32_t OperatingMode) {
    ...
    /*       "LL_PWR_SMPS_SetMode()", "LL_PWR_SMPS_GetMode()"                   */
    /*       and "LL_PWR_SMPS_GetEffectiveMode()".                              */
    MODIFY_REG(PWR->CR5, PWR_CR5_SMPSEN,
               (OperatingMode & PWR_SR2_SMPSF) << (PWR_CR5_SMPSEN_Pos - PWR_SR2_SMPSF_Pos));
}
...
__STATIC_INLINE uint32_t LL_PWR_SMPS_GetMode(void) {
    ...
    uint32_t OperatingMode =
        (READ_BIT(PWR->CR5, PWR_CR5_SMPSEN) >> (PWR_CR5_SMPSEN_Pos - PWR_SR2_SMPSF_Pos));
...
__STATIC_INLINE void LL_PWR_SMPS_SetOutputVoltageLevel(uint32_t OutputVoltageLevel) {
    __IO const uint32_t OutputVoltageLevel_calibration =
        (((*SMPS_VOLTAGE_CAL_ADDR) & SMPS_VOLTAGE_CAL) >>
         SMPS_VOLTAGE_CAL_POS); /* SMPS output voltage level calibrated in production */
    int32_t TrimmingSteps;      /* Trimming steps between theoretical output voltage and calibrated
                                   output voltage */
    int32_t OutputVoltageLevelTrimmed; /* SMPS output voltage level after calibration: trimming
                                          value added to required level */

    if (OutputVoltageLevel_calibration == 0UL) {
        /* Device with SMPS output voltage not calibrated in production: Apply output voltage value
         * directly */
        ...
        /* Device with SMPS output voltage calibrated in production: Apply output voltage value
         * after correction by calibration value */

        TrimmingSteps = ((int32_t)OutputVoltageLevel_calibration -
                         (int32_t)(LL_PWR_SMPS_OUTPUT_VOLTAGE_1V50 >> PWR_CR5_SMPSVOS_Pos));
        OutputVoltageLevelTrimmed =
            ((int32_t)((uint32_t)(OutputVoltageLevel >> PWR_CR5_SMPSVOS_Pos)) +
...
__STATIC_INLINE uint32_t LL_PWR_SMPS_GetOutputVoltageLevel(void) {
    ...
    if (OutputVoltageLevel_calibration == 0UL) {
        /* Device with SMPS output voltage not calibrated in production: Return output voltage value
         * directly */

        return (uint32_t)(READ_BIT(PWR->CR5, PWR_CR5_SMPSVOS));
    }
    else {
        /* Device with SMPS output voltage calibrated in production: Return output voltage value
         * after correction by calibration value */

        TrimmingSteps =
            ((int32_t)OutputVoltageLevel_calibration -
             (int32_t)(LL_PWR_SMPS_OUTPUT_VOLTAGE_1V50 >>
                       PWR_CR5_SMPSVOS_Pos)); /* Trimming steps between theoretical output voltage
                                                 and calibrated output voltage */

        OutputVoltageLevelTrimmed =
            ((int32_t)((uint32_t)READ_BIT(PWR->CR5, PWR_CR5_SMPSVOS)) - TrimmingSteps);
...
/** @defgroup PWR_LL_EF_Configuration_Multicore Configuration of multicore, intended to be executed
 * by CPU1
 * @{
 */
...
/** @defgroup PWR_LL_EF_Configuration_CPU2_Radio Configuration of radio (BLE or 802.15.4) of CPU2,
 * intended to be executed by CPU2
 * @{
 */
...
#    if defined(PWR_CR1_VOS)
/**
 * @brief  Indicate whether the regulator is ready in the selected voltage range or if its output
 * voltage is still changing to the required voltage level
 ...
 * @note   Take care, return value "0" means the regulator is ready. Return value "1" means the
 * output voltage range is still changing.
...
/** @defgroup PWR_LL_EF_IT_Management_CPU2 PWR IT management of CPU2, intended to be executed by
 * CPU2
 * @{
 */
