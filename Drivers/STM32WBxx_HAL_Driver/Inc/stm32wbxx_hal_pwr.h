...
#define PWR_PVDLEVEL_7               \
    (PWR_CR2_PLS_2 | PWR_CR2_PLS_1 | \
     PWR_CR2_PLS_0) /*!< External input analog voltage (compared internally to VREFINT) */
/**
 * @}
 */
...
#define PWR_PVD_MODE_IT_RISING \
    (PVD_MODE_IT |             \
     PVD_RISING_EDGE) /*!< External Interrupt Mode with Rising edge trigger detection */
#define PWR_PVD_MODE_IT_FALLING \
    (PVD_MODE_IT |              \
     PVD_FALLING_EDGE) /*!< External Interrupt Mode with Falling edge trigger detection */
#define PWR_PVD_MODE_IT_RISING_FALLING                                                            \
    (PVD_MODE_IT | PVD_RISING_FALLING_EDGE) /*!< External Interrupt Mode with Rising/Falling edge \
                                               trigger detection */
/**
 * @}
 */
...
#    define PWR_LOWPOWERMODE_STOP2                                                                \
        (PWR_CR1_LPMS_1) /*!< Stop 2: stop mode with low power regulator and VDD12I interruptible \
                            digital core domain supply OFF (less peripherals activated than low   \
...
#define PWR_EXTI_LINE_PVD \
    (LL_EXTI_LINE_16) /*!< External interrupt line 16 Connected to the PWR PVD */
/**
 * @}
 */
...
#define PVD_RISING_EDGE \
    (0x00000001U) /*!< Mask for rising edge set as PVD trigger                 */
#define PVD_FALLING_EDGE \
    (0x00000002U) /*!< Mask for falling edge set as PVD trigger                */
#define PVD_RISING_FALLING_EDGE \
    (0x00000003U) /*!< Mask for rising and falling edges set as PVD trigger    */
/**
 * @}
 */
 ...
 *
 *            @arg @ref PWR_FLAG_VOSF   Voltage Scaling Flag. Indicates whether the regulator is
 * ready in the selected voltage range or is still changing to the required voltage level.
 *            @arg @ref PWR_FLAG_PVDO   Power Voltage Detector Output. Indicates whether VDD voltage
 * is below or above the selected PVD threshold.
 *
 *            @arg @ref PWR_FLAG_PVMO1 Peripheral Voltage Monitoring Output 1. Indicates whether
 * VDDUSB voltage is is below or above PVM1 threshold (applicable when USB feature is supported).
 *            @arg @ref PWR_FLAG_PVMO3 Peripheral Voltage Monitoring Output 3. Indicates whether
 * VDDA voltage is is below or above PVM3 threshold.
 *
 *           /----------------------------EXTSCR--------------------------/
...
#define __HAL_PWR_GET_FLAG(__FLAG__)                             \
    ((((__FLAG__)&PWR_FLAG_REG_MASK) == PWR_FLAG_REG_SR1)        \
...
#define __HAL_PWR_CLEAR_FLAG(__FLAG__)                                                       \
    ((((__FLAG__)&PWR_FLAG_REG_MASK) == PWR_FLAG_REG_EXTSCR)                                 \
         ? (PWR->EXTSCR =                                                                    \
                (1UL << (((__FLAG__)&PWR_FLAG_EXTSCR_CLR_MASK) >> PWR_FLAG_EXTSCR_CLR_POS))) \
         : ((((__FLAG__)) == PWR_FLAG_WU) ? (PWR->SCR = PWR_SCR_CWUF)                        \
