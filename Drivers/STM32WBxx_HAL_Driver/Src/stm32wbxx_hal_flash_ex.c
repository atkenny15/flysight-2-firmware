 ...
 ==============================================================================
  [..] This driver provides functions to configure and program the FLASH memory
      ...
      (#) Flash Suspend, Allow functions:
           (++) Suspend or Allow new program or erase operation request using
 HAL_FLASHEx_SuspendOperation() and HAL_FLASHEx_AllowOperation() functions
...
static void FLASH_OB_GetPCROP(uint32_t* PCROPConfig, uint32_t* PCROP1AStartAddr,
                              uint32_t* PCROP1AEndAddr, uint32_t* PCROP1BStartAddr,
...
static void FLASH_OB_GetSecureMemoryConfig(uint32_t* SecureFlashStartAddr,
                                           uint32_t* SecureRAM2aStartAddr,
                                           uint32_t* SecureRAM2bStartAddr, uint32_t* SecureMode);
static void FLASH_OB_GetC2BootResetConfig(uint32_t* C2BootResetVectAddr,
                                          uint32_t* C2BootResetRegion);
    ...
    if (status == HAL_OK) {
        if (pEraseInit->TypeErase == FLASH_TYPEERASE_PAGES) {
            ...
            *PageError = 0xFFFFFFFFU;
            ...
            for (index = pEraseInit->Page; index < (pEraseInit->Page + pEraseInit->NbPages);
                 index++) {
                /* Start erase page */
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef* pOBInit) {
    ...
    if ((pOBInit->OptionType & OPTIONBYTE_PCROP) != 0U) {
        ...
        if ((pOBInit->PCROPConfig & (OB_PCROP_ZONE_A | OB_PCROP_RDP_ERASE)) != 0U) {
            ...
            FLASH_OB_PCROP1AConfig(pOBInit->PCROPConfig, pOBInit->PCROP1AStartAddr,
...
void HAL_FLASHEx_OBGetConfig(FLASH_OBProgramInitTypeDef* pOBInit) {
    ...
    FLASH_OB_GetPCROP(&(pOBInit->PCROPConfig), &(pOBInit->PCROP1AStartAddr),
                      &(pOBInit->PCROP1AEndAddr), &(pOBInit->PCROP1BStartAddr),
    ...
    /* Get the Secure Flash start address, Secure Backup RAM2a start address, Secure non-Backup
     * RAM2b start address and the Security Mode, */
    FLASH_OB_GetSecureMemoryConfig(&(pOBInit->SecureFlashStartAddr),
                                   &(pOBInit->SecureRAM2aStartAddr),
 ...
 *         until one of this bit or the same bit in Flash CPU2 access control register (FLASH_C2ACR)
 * is set. The PESD bit in both the Flash status register (FLASH_SR) and Flash CPU2 status register
 * (FLASH_C2SR) register will be cleared when both PES bit in FLASH_ACR and FLASH_C2ACR are cleared.
 * @retval Status
 ...
 *          This parameter can be page number between WRPStartOffset and (max number of pages in the
 * Flash - 1)
 * @retval None
 */
static void FLASH_OB_WRPConfig(uint32_t WRPArea, uint32_t WRPStartOffset, uint32_t WRDPEndOffset) {
 ...
 *         @arg @ref OB_RESET_MODE_INPUT_ONLY or @ref OB_RESET_MODE_GPIO or @ref
 ...
 *         through the call of the @ref HAL_FLASH_OB_Launch() function.
 * @param  PCROPConfig: specifies the erase configuration (OB_PCROP_RDP_NOT_ERASE or
 * OB_PCROP_RDP_ERASE) on RDP level 1 regression.
 * @param  PCROP1AStartAddr Specifies the Zone 1A Start address of the Proprietary code readout
 * protection This parameter can be an address between begin and end of the flash
 * @param  PCROP1AEndAddr Specifies the Zone 1A end address of the Proprietary code readout
 * protection This parameter can be an address between PCROP1AStartAddr and end of the flash
 * @retval None
 */
static void FLASH_OB_PCROP1AConfig(uint32_t PCROPConfig, uint32_t PCROP1AStartAddr,
                                   uint32_t PCROP1AEndAddr) {
    ...
    if ((PCROPConfig & OB_PCROP_ZONE_A) != 0U) {
        ...
        startoffset =
            ((PCROP1AStartAddr - FLASH_BASE) >> FLASH_PCROP_GRANULARITY_OFFSET); /* 2K pages */
        endoffset =
            ((PCROP1AEndAddr - FLASH_BASE) >> FLASH_PCROP_GRANULARITY_OFFSET); /* 2K pages */
 ...
 * @note   To validate the PCROP options, the option bytes must be reloaded
 *         through the call of the @ref HAL_FLASH_OB_Launch() function.
 * @param  PCROP1BStartAddr Specifies the Zone 1BStart address of the Proprietary code readout
 * protection This parameter can be an address between begin and end of the flash
 * @param  PCROP1BEndAddr Specifies the Zone 1B end address of the Proprietary code readout
 * protection This parameter can be an address between PCROP1BStartAddr and end of the flash
 * @retval None
 */
static void FLASH_OB_PCROP1BConfig(uint32_t PCROP1BStartAddr, uint32_t PCROP1BEndAddr) {
    uint32_t startoffset;
    ...
    /* Compute offset depending on pcrop granularity */
    startoffset =
        ((PCROP1BStartAddr - FLASH_BASE) >> FLASH_PCROP_GRANULARITY_OFFSET);       /* 2K pages */
...
static void FLASH_OB_SecureConfig(FLASH_OBProgramInitTypeDef* pOBParam) {
    ...
    if ((pOBParam->OptionType & OPTIONBYTE_SECURE_MODE) != 0U) {
        ...
        MODIFY_REG(sfr_reg_val, FLASH_SFR_SFSA,
                   (((pOBParam->SecureFlashStartAddr - FLASH_BASE) / FLASH_PAGE_SIZE)
                    << FLASH_SFR_SFSA_Pos));

        /* Configure SRRVR register */
#    if defined(FLASH_SRRVR_SBRSA_A)
        MODIFY_REG(srrvr_reg_val, (FLASH_SRRVR_SBRSA_A | FLASH_SRRVR_SBRSA_B),
                   (((((pOBParam->SecureRAM2aStartAddr - SRAM2A_BASE) >>
                       SRAM_SECURE_PAGE_GRANULARITY_OFFSET)
                      << FLASH_SRRVR_SBRSA_A_Pos)) |
                    ((((pOBParam->SecureRAM2bStartAddr - SRAM2B_BASE) >>
                       SRAM_SECURE_PAGE_GRANULARITY_OFFSET)
                      << FLASH_SRRVR_SBRSA_B_Pos))));
#    else
        MODIFY_REG(srrvr_reg_val, (FLASH_SRRVR_SBRSA | FLASH_SRRVR_SNBRSA),
                   (((((pOBParam->SecureRAM2aStartAddr - SRAM2A_BASE) >>
                       SRAM_SECURE_PAGE_GRANULARITY_OFFSET)
                      << FLASH_SRRVR_SBRSA_Pos)) |
                    ((((pOBParam->SecureRAM2bStartAddr - SRAM2B_BASE) >>
                       SRAM_SECURE_PAGE_GRANULARITY_OFFSET)
                      << FLASH_SRRVR_SNBRSA_Pos))));
...
#    endif /* FLASH_SRRVR_BRSD_A */
    ...
    if ((pOBParam->OptionType & OPTIONBYTE_C2_BOOT_VECT) != 0U) {
        ...
        if (pOBParam->C2BootRegion == OB_C2_BOOT_FROM_FLASH) {
            MODIFY_REG(
                srrvr_reg_val, (FLASH_SRRVR_SBRV | FLASH_SRRVR_C2OPT),
...
static void FLASH_OB_GetWRP(uint32_t WRPArea, uint32_t* WRPStartOffset, uint32_t* WRDPEndOffset) {
    ...
    if (WRPArea == OB_WRPAREA_BANK1_AREAA) {
        ...
        *WRDPEndOffset =
            (READ_BIT(FLASH->WRP1AR, FLASH_WRP1AR_WRP1A_END) >> FLASH_WRP1AR_WRP1A_END_Pos);
    }
    else /* OB_WRPAREA_BANK1_AREAB */
    {
        *WRPStartOffset = READ_BIT(FLASH->WRP1BR, FLASH_WRP1BR_WRP1B_STRT);
        *WRDPEndOffset =
            (READ_BIT(FLASH->WRP1BR, FLASH_WRP1BR_WRP1B_END) >> FLASH_WRP1BR_WRP1B_END_Pos);
    }
}
...
/**
 * @brief  Return the FLASH Write Protection Option Bytes value.
 * @param PCROPConfig [out] Specifies the address where to copied the configuration of PCROP_RDP
 * option
...
/**
 * @brief  Return the Secure Flash start address, Secure Backup RAM2a start address, Secure
 * non-Backup RAM2b start address and the SecureMode
 * @param  SecureFlashStartAddr Specifies the address where to copied the Secure Flash start address
 * @param  SecureRAM2aStartAddr Specifies the address where to copied the Secure Backup RAM2a start
 * address
 * @param  SecureRAM2bStartAddr Specifies the address where to copied the Secure non-Backup RAM2b
 * start address
 ...
 * @param  C2BootResetVectAddr Specifies the address where to copied the CPU2 Secure Boot reset
 * vector address
...
static void FLASH_OB_GetC2BootResetConfig(uint32_t* C2BootResetVectAddr,
    ...
    *C2BootResetRegion = (READ_BIT(FLASH->SRRVR, FLASH_SRRVR_C2OPT));
    ...
    if (*C2BootResetRegion == OB_C2_BOOT_FROM_FLASH) {
        *C2BootResetVectAddr =
            (uint32_t)((READ_BIT(FLASH->SRRVR, FLASH_SRRVR_SBRV) << 2) + FLASH_BASE);
    }
    else {
        *C2BootResetVectAddr =
            (uint32_t)((READ_BIT(FLASH->SRRVR, FLASH_SRRVR_SBRV) << 2) + SRAM1_BASE);
    }
}
