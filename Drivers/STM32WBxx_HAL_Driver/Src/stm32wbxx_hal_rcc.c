  ...
  ==============================================================================
    ...
    [..]
      ...
      (+) Configure the clock source(s) for peripherals which clocks are not
          derived from the System clock (SAI1, RTC, ADC, USB/RNG, USART1, LPUART1, LPTIMx, I2Cx,
  SMPS)
...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization functions
 ...
 ===============================================================================
    [..]
      ...
      (HSE, HSI, LSE, MSI, LSI1, LSI2, PLL, CSS and MCO) and the System buses clocks (SYSCLK, HCLK1,
 HCLK2, HCLK4, PCLK1 and PCLK2).

    [..] Internal/external clock and PLL configuration
         (+) HSI (high-speed internal): 16 MHz factory-trimmed RC used directly or through
             the PLL as System clock source.

         (+) MSI (Multiple Speed Internal): Its frequency is software trimmable from 100KHZ to
 48MHZ. It can be used to generate the clock for the USB FS (48 MHz). The number of flash wait
 states is automatically adjusted when MSI range is updated with HAL_RCC_OscConfig() and the MSI is
 used as System clock source.
         ...
         (+) HSE (high-speed external): 32 MHz crystal oscillator used directly or
             through the PLL as System clock source. Can be used also optionally as RTC clock
 source.
         ...
         (+) CSS (Clock security system): once enabled, if a HSE clock failure occurs
            (HSE used directly or through PLL as System clock source), the System clock
             ...
             The interrupt is linked to the CPU1 and CPU2 NMI (Non-Maskable Interrupt) exception
 vector.

         (+) LSECSS: once enabled, if a LSE clock failure occurs, the LSE
             clock is no longer supplied to the RTC but no hardware action is made to the registers.
 If the MSI was in PLL-mode, this mode is disabled. In Standby mode a wakeup is generated. In other
 modes an interrupt can be sent to wakeup the software

         (+) MCO (microcontroller clock output): used to output MSI, LSI1, LSI2, HSI, LSE, HSE
 (before and after stabilization), SYSCLK, HSI48 or main PLL clock (through a configurable
 prescaler) on PA8, PB6 & PA15 pins.
    ...
    [..] System, AHB and APB buses clocks configuration
         ...
         -@- All the peripheral clocks are derived from the System clock (SYSCLK) except:
           ...
           (+@) SAI: the SAI clock can be derived either from a specific PLL (PLLSAI1) or (PLLSYS)
 or from an external clock mapped on the SAI_CKIN pin. You have to use HAL_RCCEx_PeriphCLKConfig()
 function to configure this clock.
         ...
         (+) The maximum frequency of the SYSCLK, HCLK1, HCLK4, PCLK1 and PCLK2 is 64 MHz.
             ...
             as listed in the Reference Manual "Clock source frequency versus voltage scaling"
 chapter.
...
#    if defined(SAI1)
    CLEAR_BIT(RCC->CR, RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS | RCC_CR_HSEON |
                           RCC_CR_HSEPRE | RCC_CR_PLLON | RCC_CR_PLLSAI1ON);
#    else
    CLEAR_BIT(RCC->CR, RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS | RCC_CR_HSEON |
                           RCC_CR_HSEPRE | RCC_CR_PLLON);
#    endif /* SAI1 */
    ...
    WRITE_REG(RCC->PLLCFGR,
              RCC_PLLCFGR_PLLR_0 | RCC_PLLCFGR_PLLQ_0 | RCC_PLLCFGR_PLLP_1 | RCC_PLLCFGR_PLLN_0);
...
HAL_StatusTypeDef HAL_RCC_OscConfig(RCC_OscInitTypeDef* RCC_OscInitStruct) {
    ...
    if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSE) == RCC_OSCILLATORTYPE_HSE) {
        ...
        /* When the HSE is used as system clock or clock source for PLL in these cases it is not
         * allowed to be disabled */
        const uint32_t temp_sysclksrc = __HAL_RCC_GET_SYSCLK_SOURCE();
        const uint32_t temp_plloscsrc = __HAL_RCC_GET_PLL_OSCSOURCE();
        if ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSE) ||
            ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK) &&
             (temp_plloscsrc == RCC_PLLSOURCE_HSE))) {
    ...
    if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSI) == RCC_OSCILLATORTYPE_HSI) {
        ...
        if ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSI) ||
            ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK) &&
             (temp_plloscsrc == RCC_PLLSOURCE_HSI))) {
    ...
    if ((((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_LSI1) ==
         RCC_OSCILLATORTYPE_LSI1) ||
        (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_LSI2) ==
        ...
        if (RCC_OscInitStruct->LSIState != RCC_LSI_OFF) {
             ...
             * -------------------------*/
            if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_LSI2) ==
                RCC_OSCILLATORTYPE_LSI2) {
                assert_param(
                    IS_RCC_LSI2_CALIBRATION_VALUE(RCC_OscInitStruct->LSI2CalibrationValue));
...
#    endif /* RCC_HSI48_SUPPORT */
    ...
    if (RCC_OscInitStruct->PLL.PLLState != RCC_PLL_NONE) {
        ...
        if (RCC_OscInitStruct->PLL.PLLState == RCC_PLL_ON) {
            ...
            if ((READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLSRC) !=
                 RCC_OscInitStruct->PLL.PLLSource) ||
                (READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLM) != RCC_OscInitStruct->PLL.PLLM) ||
                ((READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos) !=
...
#    if defined(SAI1) && defined(USB)
                CLEAR_BIT(RCC->PLLCFGR, (RCC_PLLCFGR_PLLSRC | RCC_PLLCFGR_PLLPEN |
 ...
 * HSI48
 *            @arg @ref RCC_MCO1SOURCE_HSE_BEFORE_STAB  HSE clock before stabilization selected as
 * MCO source
...
uint32_t HAL_RCC_GetHCLKFreq(void) {
    ...
    return (
        (uint32_t)(__LL_RCC_CALC_HCLK1_FREQ(HAL_RCC_GetSysClockFreq(), LL_RCC_GetAHBPrescaler())));
}
...
uint32_t HAL_RCC_GetHCLK2Freq(void) {
    ...
    return ((uint32_t)(__LL_RCC_CALC_HCLK2_FREQ(HAL_RCC_GetSysClockFreq(),
                                                LL_C2_RCC_GetAHBPrescaler())));
}
...
uint32_t HAL_RCC_GetHCLK4Freq(void) {
    ...
    return (
        (uint32_t)(__LL_RCC_CALC_HCLK4_FREQ(HAL_RCC_GetSysClockFreq(), LL_RCC_GetAHB4Prescaler())));
}
 ...
 * @param  VCORE_Voltage     Current Vcore voltage (PWR_REGULATOR_VOLTAGE_SCALE1 or
