/**
 ...
 HAL_MultiProcessor_Init(), also configure the low level Hardware GPIO, CLOCK, CORTEX...etc) by
...
#    define USART_CR1_FIELDS                                                                   \
        ((uint32_t)(USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE | USART_CR1_RE | \
                    USART_CR1_OVER8 |                                                          \
                    USART_CR1_FIFOEN)) /*!< UART or USART CR1 fields of parameters set by      \
                                          UART_SetConfig API */

#    define USART_CR3_FIELDS                                                                 \
        ((uint32_t)(USART_CR3_RTSE | USART_CR3_CTSE | USART_CR3_ONEBIT | USART_CR3_TXFTCFG | \
                    USART_CR3_RXFTCFG)) /*!< UART or USART CR3 fields of parameters set by   \
                                           UART_SetConfig API */
 ...
 * HAL_HalfDuplex_Init(), HAL_LIN_Init(), HAL_MultiProcessor_Init() or HAL_RS485Ex_Init() in
 * HAL_UART_STATE_RESET to register callbacks for HAL_UART_MSPINIT_CB_ID and
 ...
 * @note   The HAL_UART_UnRegisterCallback() may be called before HAL_UART_Init(),
 * HAL_HalfDuplex_Init(), HAL_LIN_Init(), HAL_MultiProcessor_Init() or HAL_RS485Ex_Init() in
 * HAL_UART_STATE_RESET to un-register callbacks for HAL_UART_MSPINIT_CB_ID and
 * HAL_UART_MSPDEINIT_CB_ID
 * @param  huart uart handle
 ...
 *         UART Rx Event Callback is redirected to the weak HAL_UARTEx_RxEventCallback() predefined
 * callback
 * @param  huart     Uart handle
 * @retval HAL status
 */
 ...
 ===============================================================================
    ...
    (#) There are two mode of transfer:
       ...
       (+) Non-Blocking mode: The communication is performed using Interrupts
           ...
           The HAL_UART_ErrorCallback()user callback will be executed when a communication error is
detected
    ...
    (#) For Abort services based on interrupts (HAL_UART_Abortxxx_IT), a set of Abort Complete
Callbacks are provided:
        (+) HAL_UART_AbortCpltCallback()
        (+) HAL_UART_AbortTransmitCpltCallback()
        (+) HAL_UART_AbortReceiveCpltCallback()

    (#) A Rx Event Reception Callback (Rx event notification) is available for Non_Blocking modes of
enhanced reception services:
        (+) HAL_UARTEx_RxEventCallback()

    (#) In Non-Blocking mode transfers, possible errors are split into 2 categories.
        Errors are handled as follows :
       (+) Error is considered as Recoverable and non blocking : Transfer could go till end, but
error severity is to be evaluated by user : this concerns Frame Error, Parity Error or Noise Error
           in Interrupt mode reception .
           Received character is then retrieved and stored in Rx buffer, Error code is set to allow
user to identify error type, and HAL_UART_ErrorCallback() user callback is executed. Transfer is
kept ongoing on UART side. If user wants to abort it, Abort services should be called by user.
       (+) Error is considered as Blocking : Transfer could not be completed properly and is
aborted. This concerns Overrun Error In Interrupt mode reception and all errors in DMA mode. Error
code is set to allow user to identify error type, and HAL_UART_ErrorCallback() user callback is
executed.
 ...
 * @note   When UART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits (M1-M0
 * = 01), the sent data is handled as a set of u16. In this case, Size must indicate the number of
 * u16 provided through pData.
...
HAL_StatusTypeDef HAL_UART_Transmit(UART_HandleTypeDef* huart, const uint8_t* pData, uint16_t Size,
    ...
    if (huart->gState == HAL_UART_STATE_READY) {
        ...
        while (huart->TxXferCount > 0U) {
            if (UART_WaitOnFlagUntilTimeout(huart, UART_FLAG_TXE, RESET, tickstart, Timeout) !=
                HAL_OK) {
 ...
 * @note   When UART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits (M1-M0
 * = 01), the received data is handled as a set of u16. In this case, Size must indicate the number
...
HAL_StatusTypeDef HAL_UART_DMAStop(UART_HandleTypeDef* huart) {
    /* The Lock is not implemented on this API to allow the user application
       ...
       HAL_UART_RxCpltCallback() / HAL_UART_TxHalfCpltCallback / HAL_UART_RxHalfCpltCallback:
 ...
 * @param  huart UART handle.
 * @note   This procedure could be used for aborting any ongoing transfer started in Interrupt or
 * DMA mode. This procedure performs following operations :
 *           - Disable UART Interrupts (Tx and Rx)
 *           - Disable the DMA transfer in the peripheral register (if enabled)
 *           - Abort DMA transfer by calling HAL_DMA_Abort (in case of transfer in DMA mode)
 *           - Set handle State to READY
 * @note   This procedure is executed in blocking mode : when exiting function, Abort is considered
 * as completed.
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_UART_Abort(UART_HandleTypeDef* huart) {
    /* Disable TXE, TC, RXNE, PE, RXFT, TXFT and ERR (Frame error, noise error, overrun error)
     * interrupts */
 ...
 * @param  huart UART handle.
 * @note   This procedure could be used for aborting any ongoing Tx transfer started in Interrupt or
 * DMA mode. This procedure performs following operations :
 ...
 * @param  huart UART handle.
 * @note   This procedure could be used for aborting any ongoing Rx transfer started in Interrupt or
 * DMA mode. This procedure performs following operations :
 ...
 *         considered as completed only when user abort complete callback is executed (not when
 * exiting function).
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_UART_Abort_IT(UART_HandleTypeDef* huart) {
    ...
    /* Disable interrupts */
    ATOMIC_CLEAR_BIT(huart->Instance->CR1, (USART_CR1_PEIE | USART_CR1_TCIE |
    ...
    /* If DMA Tx and/or DMA Rx Handles are associated to UART Handle, DMA Abort complete callbacks
       should be initialised before any call to DMA Abort functions */
...
#    endif /* USE_HAL_UART_REGISTER_CALLBACKS */
    ...
    if ((huart->ReceptionType == HAL_UART_RECEPTION_TOIDLE) &&
        ((isrflags & USART_ISR_IDLE) != 0U) && ((cr1its & USART_ISR_IDLE) != 0U)) {
        ...
        if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) {
            ...
            /* Check received length : If all expected data are received, do nothing,
               ...
               Otherwise, if at least one data has already been received, IDLE event is to be
               notified to user */
...
#    endif /* (USE_HAL_UART_REGISTER_CALLBACKS) */
            ...
            if ((huart->RxXferCount > 0U) && (nb_rx_data > 0U)) {
                ...
                /* Disable the UART Error Interrupt:(Frame error, noise error, overrun error) and RX
...
#    endif /* (USE_HAL_UART_REGISTER_CALLBACKS) */
    ...
    if (((isrflags & USART_ISR_WUF) != 0U) && ((cr3its & USART_CR3_WUFIE) != 0U)) {
        ...
        /* UART Rx state is not reset as a reception process might be ongoing.
           If UART handle state fields need to be reset to READY, this could be done in Wakeup
           callback */
...
@verbatim
 ...
 ===============================================================================
    ...
    This subsection provides a set of functions allowing to control the UART.
     (+) HAL_UART_ReceiverTimeout_Config() API allows to configure the receiver timeout value on the
fly
...
static void UART_DMATxAbortCallback(DMA_HandleTypeDef* hdma) {
    ...
    /* No Abort process still ongoing : All DMA channels are aborted, call user Abort Complete
     * callback */
    huart->TxXferCount = 0U;
    huart->RxXferCount = 0U;
