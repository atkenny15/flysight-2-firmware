/**
  ...
  ******************************************************************************
  ...
  [..]
   ...
   (#) Use HAL_DMA_GetState() function to return the DMA state and HAL_DMA_GetError() in case of
  error detection.
     ...
     *** DMA HAL driver macros list ***
     =============================================
      [..]
       ...
       (+) __HAL_DMA_GET_IT_SOURCE: Check whether the specified DMA Channel interrupt is enabled or
  not.
...
#    endif /* DMA2 */
    ...
    tmp &= ((uint32_t) ~(DMA_CCR_PL | DMA_CCR_MSIZE | DMA_CCR_PSIZE | DMA_CCR_MINC | DMA_CCR_PINC |
...
/**
 * @brief  Updates the DMA handle with the DMAMUX  channel and status mask depending on channel
 * number
 * @param  hdma Pointer to a DMA_HandleTypeDef structure that contains
 *                     the configuration information for the specified DMA Channel.
 * @retval None
...
#    endif /* DMA2 */
    ...
    /* Initialize the field DMAmuxChannelStatusMask with the corresponding index of the DMAMUX
     * channel selected for the current ChannelIndex */
