...
#    endif /* RCC_SMPS_SUPPORT */
    ...
    if (PeriphClk == RCC_PERIPHCLK_RTC) {
        ...
        else if (rtcClockSource ==
                 LL_RCC_RTC_CLKSOURCE_LSI) /* LSI clock used as RTC clock source */
        {
            ...
            if ((temp_lsi1ready == 1U) || (temp_lsi2ready == 1U)) {
                ...
                /* Nothing to do as frequency already initialized to 0U */
                 ...
                 LL_RCC_RTC_CLKSOURCE_HSE_DIV32) /* HSE clock used as RTC clock source */
        {
...
#    endif /* SAI1 */
        ...
        else if (rngClockSource ==
                 RCC_RNGCLKSOURCE_PLL) /* PLL clock divided by 3 used as RNG clock source */
        {
            if (LL_RCC_PLL_IsReady() == 1U) {
                ...
                /* Nothing to do as frequency already initialized to 0U */
                 ...
                 RCC_RNGCLKSOURCE_MSI) /* MSI clock divided by 3 used as RNG clock source */
        {
...
#    if defined(SAI1)
                 ...
                 RCC_RNGCLKSOURCE_PLLSAI1) /* PLLSAI1 clock used as SAI1 clock source */
        {
            if (LL_RCC_PLLSAI1_IsReady() == 1U) {
...
#    endif /* USB */
        ...
        switch (LL_RCC_GetUSARTClockSource(LL_RCC_USART1_CLKSOURCE)) {
            ...
            default: /* USART1 Clock is PCLK2 */
                frequency = __LL_RCC_CALC_PCLK2_FREQ(
                    __LL_RCC_CALC_HCLK1_FREQ(HAL_RCC_GetSysClockFreq(), LL_RCC_GetAHBPrescaler()),
...
#    if defined(LPUART1)
        ...
        switch (LL_RCC_GetLPUARTClockSource(LL_RCC_LPUART1_CLKSOURCE)) {
            ...
            default: /* LPUART1 Clock is PCLK1 */
                frequency = __LL_RCC_CALC_PCLK1_FREQ(
                    __LL_RCC_CALC_HCLK1_FREQ(HAL_RCC_GetSysClockFreq(), LL_RCC_GetAHBPrescaler()),
...
#    endif /* I2C3 */
        ...
        if (rfwkpClockSource ==
            LL_RCC_RFWKP_CLKSOURCE_LSE) /* LSE clock used as RF Wakeup clock source */
        {
            if (LL_RCC_LSE_IsReady() == 1U) {
                frequency = LSE_VALUE;
            }
        ...
        else if (rfwkpClockSource ==
                 LL_RCC_RFWKP_CLKSOURCE_HSE_DIV1024) /* HSE clock used as RF Wakeup clock source */
        {
...
#    if defined(RCC_SMPS_SUPPORT)
        ...
        else if (smpsClockSource ==
                 LL_RCC_SMPS_CLKSOURCE_STATUS_HSE) /* SMPS Clock source is HSE Osc. */
        {
            if (LL_RCC_HSE_IsReady() == 1U) {
                ...
                /* Nothing to do as frequency already initialized to 0U */
                 ...
                 LL_RCC_SMPS_CLKSOURCE_STATUS_MSI) /* SMPS Clock source is MSI Osc. */
        {
            switch (LL_RCC_MSI_GetRange()) {
                case LL_RCC_MSIRANGE_8:
                    frequency = __LL_RCC_CALC_MSI_FREQ(LL_RCC_MSIRANGE_8) /
                                SmpsPrescalerTable[smps_prescaler_index][4];
                    break;
                case LL_RCC_MSIRANGE_9:
                    frequency = __LL_RCC_CALC_MSI_FREQ(LL_RCC_MSIRANGE_9) /
                                SmpsPrescalerTable[smps_prescaler_index][3];
                    break;
                case LL_RCC_MSIRANGE_10:
                    frequency = __LL_RCC_CALC_MSI_FREQ(LL_RCC_MSIRANGE_10) /
                                SmpsPrescalerTable[smps_prescaler_index][2];
                    break;
                case LL_RCC_MSIRANGE_11:
                    frequency = __LL_RCC_CALC_MSI_FREQ(LL_RCC_MSIRANGE_11) /
                                SmpsPrescalerTable[smps_prescaler_index][1];
                    break;
                default:
                    break;
            }
...
HAL_StatusTypeDef HAL_RCCEx_EnablePLLSAI1(RCC_PLLSAI1InitTypeDef* PLLSAI1Init) {
    ...
    if (status == HAL_OK) {
        ...
        __HAL_RCC_PLLSAI1_CONFIG(PLLSAI1Init->PLLN, PLLSAI1Init->PLLP, PLLSAI1Init->PLLQ,
                                 PLLSAI1Init->PLLR);
...
/**
 * @brief  Select the clock source to output on LSCO1 pin(PA2) or LSC02 pin (PH3) or LSCO3 pin
 * (PC12).
 ...
 * @note The LSI2 oscillator must be disabled before calling this trimming function through @ref
 * HAL_RCC_OscConfig
 * @retval HAL status
 */
...
@verbatim
 ...
 ===============================================================================
    [..]
      For devices with Clock Recovery System feature (CRS), RCC Extended HAL driver can be used as
follows:
      ...
      (#) Call CRS functions as follows:
          (##) Prepare synchronization configuration necessary for HSI48 calibration
              (+++) Default values can be set for frequency Error Measurement (reload and error
limit) and also HSI48 oscillator smooth trimming.
...
calibration in changing synchronization configuration and call again HAL_RCCEx_CRSConfig. Note: When
the SYNC event is detected during the downcounting phase (before reaching the zero value), it means
that the actual frequency is lower than the target (and so, that the TRIM value should be
...
actual frequency is higher (and that the TRIM value should be decremented).
      ...
      (#) In interrupt mode, user can resort to the available macros (__HAL_RCC_CRS_XXX_IT).
Interrupts will go through CRS Handler (CRS_IRQn/CRS_IRQHandler)
...
HAL_RCCEx_CRSSoftwareSynchronizationGenerate(). This function can be called before calling
HAL_RCCEx_CRSConfig (for instance in Systick handler)
