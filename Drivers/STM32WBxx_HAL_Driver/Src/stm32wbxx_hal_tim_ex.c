...
#        define TIM_GET_OR_MASK(__INSTANCE__)                                                     \
            (((__INSTANCE__) == TIM1)    ? (TIM1_OR_ETR_ADC1_RMP | TIM1_OR_TI1_RMP)               \
             ...
             : ((__INSTANCE__) == TIM16) ? TIM16_OR_TI1_RMP                                       \
                                         : TIM17_OR_TI1_RMP)
...
#    endif /* USE_HAL_TIM_REGISTER_CALLBACKS */
    ...
    /* Configure the Channel 1 as Input Channel to interface with the three Outputs of the  Hall
     * sensor */
 ...
 * @param  htim TIM handle
 * @param  InputTrigger the Internal trigger corresponding to the Timer Interfacing with the Hall
 * sensor This parameter can be one of the following values:
 ...
 *            @arg TIM_COMMUTATION_SOFTWARE:  Commutation source is set by software using the COMG
 * bit
 * @retval HAL status
 */
 ...
 *        commutation at its TRGO output (connected to Timer used in this function) each time
 *        the TI1 of the Interface Timer detect a commutation at its input TI1.
 * @note  The user should configure the DMA in his own software, in This function only the COMDE bit
 * is set
 * @param  htim TIM handle
 * @param  InputTrigger the Internal trigger corresponding to the Timer Interfacing with the Hall
 * sensor This parameter can be one of the following values:
 ...
 *            @arg TIM_TIM2_TI4_COMP1_COMP2:         TIM2 TI4 is connected to logical OR between
 * COMP1 and COMP2 output (*)
 *
 ...
 *            @arg TIM_TIM17_TI1_MSI:               TIM17 TI1 is connected to MSI  (constraint: MSI
 * clock < 1/4 TIM APB clock)
    ...
    switch (BreakInput) {
        case TIM_BREAKINPUT_BRK: {
            ...
            if (READ_BIT(htim->Instance->BDTR, TIM_BDTR_BKBID) == TIM_BDTR_BKBID) {
                /* Break input BRK is re-armed automatically by hardware. Poll to check whether
                 * fault condition disappeared */
        ...
        case TIM_BREAKINPUT_BRK2: {
            ...
            if (READ_BIT(htim->Instance->BDTR, TIM_BDTR_BK2BID) == TIM_BDTR_BK2BID) {
                /* Break input BRK2 is re-armed automatically by hardware. Poll to check whether
                 * fault condition disappeared */
                /* Init tickstart for timeout management */
                tickstart = HAL_GetTick();
