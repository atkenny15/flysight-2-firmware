 ...
 * @{
 */
#    define PWR_PORTE_AVAILABLE_PINS \
        (PWR_GPIO_BIT_4 | PWR_GPIO_BIT_3 | PWR_GPIO_BIT_2 | PWR_GPIO_BIT_1 | PWR_GPIO_BIT_0)
...
HAL_StatusTypeDef HAL_PWREx_ConfigSMPS(PWR_SMPSTypeDef* sConfigSMPS) {
    ...
    if (OutputVoltageLevel_calibration == 0UL) {
        ...
        /* Update register */
        MODIFY_REG(PWR->CR5, PWR_CR5_SMPSVOS,
                   (sConfigSMPS->StartupCurrent | sConfigSMPS->OutputVoltage));
    }
    else {
        ...
        OutputVoltageLevelTrimmed =
            ((int32_t)((uint32_t)(sConfigSMPS->OutputVoltage >> PWR_CR5_SMPSVOS_Pos)) +
...
void HAL_PWREx_EnableWakeUpPin(uint32_t WakeUpPinPolarity, uint32_t wakeupTarget) {
    ...
    MODIFY_REG(PWR->CR4, (PWR_C2CR3_EWUP & WakeUpPinPolarity),
 ...
 * preserved. The BOR is available.
 ...
 *         the HSI RC oscillator is selected as system clock if STOPWUCK bit in RCC_CFGR register
 *         is set; the MSI oscillator is selected if STOPWUCK is cleared.
 * @note  Due to low power mode, an additional startup delay is incurred when waking up from Stop 1
 * mode.
 * @note  According to system power policy, system entering in Stop mode
 *        is depending on other CPU power mode.
 * @param STOPEntry  specifies if Stop mode in entered with WFI or WFE instruction.
 ...
 *        (LCD, LPTIM1, I2C3 and LPUART) can switch on the HSI to receive a frame, and switch off
 * the HSI after receiving the frame if it is not a wakeup frame. In this case the HSI clock is
 * propagated only to the peripheral requesting it. SRAM1, SRAM2 and register contents are
 * preserved. The BOR is available. The voltage regulator is set in low-power mode but LPR bit must
 * be cleared to enter stop 2 mode. Otherwise, Stop 1 mode is entered.
 ...
 *        "With the JTAG debugger enabled on GPIO pins and after a wakeup from debug triggered by an
 * event on EXTI line 48 (CDBGPWRUPREQ), the device may enter in a state in which attempts to enter
 * Stop 2 mode are not fully effective ..." Workaround implementation example using LL driver:
 ...
 *        SRAM1, SRAM2, BKRAM and registers contents are lost except for registers in the Backup
 * domain. The BOR is not available.
