...
#    endif /* GPIOD */
    ...
    /* Clear all flags */
    LL_PWR_WriteReg(SCR, LL_PWR_SCR_CC2HF | LL_PWR_SCR_CBLEAF | LL_PWR_SCR_CCRPEF
...
#    endif /* PWR_CR5_SMPSEN */
    ...
    LL_PWR_WriteReg(EXTSCR, LL_PWR_EXTSCR_CCRPF | LL_PWR_EXTSCR_C2CSSF | LL_PWR_EXTSCR_C1CSSF);
}
 ...
 ===============================================================================
    ...
    *** Low Power modes configuration ***
    ...
    [..]
      ...
      (+) Low-power Run mode: core and peripherals are running, main regulator off, low power
regulator on.

      (+) Sleep mode: Cortex-M4 core stopped, peripherals kept running, main and low power
regulators on.
      (+) Low-power Sleep mode: Cortex-M4 core stopped, peripherals kept running, main regulator
off, low power regulator on.

      (+) Stop 0 mode: all clocks are stopped except LSI and LSE, main and low power regulators on.
      (+) Stop 1 mode: all clocks are stopped except LSI and LSE, main regulator off, low power
regulator on.
      (+) Stop 2 mode: all clocks are stopped except LSI and LSE, main regulator off, low power
regulator on, reduced set of waking up IPs compared to Stop 1 mode.

      (+) Standby mode with SRAM2a: all clocks are stopped except LSI and LSE, SRAM2a content
preserved, main regulator off, low power regulator on. Note: On devices STM32WB15xx, STM32WB10xx,
STM32WB1Mxx retention is extended to SRAM1, SRAM2a, SRAM2b.
      (+) Standby mode without SRAM2a: all clocks are stopped except LSI and LSE, main and low power
regulators off.
   ...
   ==========================
    [..]
      (+) Entry: (from main run mode)
          (++) set LPR bit with HAL_PWREx_EnableLowPowerRunMode() API after having decreased the
system clock below 2 MHz.
      (+) Exit:
          (++) clear LPR bit then wait for REGLP bit to be reset with
HAL_PWREx_DisableLowPowerRunMode() API. Only then can the system clock frequency be increased above
2 MHz.
   ...
   =========================================
    [..]
      (+) Entry:
          ...
          in specifying whether or not the regulator is forced to low-power mode and if exit is
interrupt or event-triggered.
          (++) PWR_MAINREGULATOR_ON: Sleep mode (regulator in main mode).
          (++) PWR_LOWPOWERREGULATOR_ON: Low-power sleep (regulator in low power mode).
          In the latter case, the system clock frequency must have been decreased below 2 MHz
beforehand.
   ...
   ===============================
    [..]
      (+) Entry:
          ...
          (++) HAL_PWREx_EnterSTOP0Mode() for mode 0, HAL_PWREx_EnterSTOP1Mode() for mode 1,
HAL_PWREx_EnterSTOP2Mode() for mode 2 or for porting reasons HAL_PWR_EnterSTOPMode(). Note: Low
power Stop2 mode is not available on devices STM32WB15xx, STM32WB10xx, STM32WB1Mxx.
      ...
      (+) WFE Exit:
       ...
       [..]
          When exiting Stop 0 and Stop 1 modes, the MCU is either in Run mode or in Low-power Run
mode depending on the LPR bit setting. When exiting Stop 2 mode, the MCU is in Run mode.

   *** Standby mode ***
   ====================
    [..] The Standby mode offers two options:
      (+) option a) all clocks off except LSI and LSE, RRS bit set (keeps voltage regulator in low
power mode). SRAM and registers contents are lost except for the SRAM2 content, the RTC registers,
RTC backup registers and Standby circuitry.
      (+) option b) all clocks off except LSI and LSE, RRS bit cleared (voltage regulator then
disabled). SRAM and register contents are lost except for the RTC registers, RTC backup registers
          and Standby circuitry.

      (++) Entry:
          (+++) The Standby mode is entered thru HAL_PWR_EnterSTANDBYMode() API.
                SRAM1 and register contents are lost except for registers in the Backup domain and
                Standby circuitry. SRAM2 content can be preserved if the bit RRS is set in PWR_CR3
register. To enable this feature, the user can resort to HAL_PWREx_EnableBKRAMContentRetention() API
                to set RRS bit.
      (++) Exit:
...
Reset.
   ...
   ======================
    ...
    [..] After waking up from Shutdown mode, program execution restarts in the same way as after a
Reset.
 ...
 *        after receiving the frame if it is not a wakeup frame. In this case, the HSI clock is
 * propagated only to the peripheral requesting it. SRAM1, SRAM2 and register contents are
 * preserved. The BOR is available. The voltage regulator can be configured either in normal (Stop
 * 0) or low-power mode (Stop 1).
 ...
 * @note  Case of Stop0 mode with SMPS: Before entering Stop 0 mode with SMPS Step Down converter
 * enabled, the HSI16 must be kept on by enabling HSI kernel clock (set HSIKERON register bit).
 ...
 * register. To enable this feature, the user can resort to HAL_PWREx_EnableBKRAMContentRetention()
 * API to set RRS bit. The BOR is available.
 * @note  The I/Os can be configured either with a pull-up or pull-down or can be kept in analog
 * state. HAL_PWREx_EnableGPIOPullUp() and HAL_PWREx_EnableGPIOPullDown() respectively enable Pull
 * Up and Pull Down state, HAL_PWREx_DisableGPIOPullUp() and HAL_PWREx_DisableGPIOPullDown() disable
 * the same. These states are effective in Standby mode only if APC bit is set through
