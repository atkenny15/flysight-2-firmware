/**
  ...
  *                using 2 possible modes: polling, interruption (not available on devices:
  STM32WB10xx, STM32WB15xx, STM32WB1Mxx ).
  *             ++ Calibration
  *               +++ ADC automatic self-calibration
  *               +++ Calibration factors get or set
  *           + Control functions
  *             ++ Channels configuration on ADC group injected (not available on devices:
  STM32WB10xx, STM32WB15xx, STM32WB1Mxx )
  *           + State functions
  *             ++ ADC group injected contexts queue management (not available on devices:
  STM32WB10xx, STM32WB15xx, STM32WB1Mxx )
...
#    define ADC_JSQR_FIELDS                                                                      \
        ((ADC_JSQR_JL | ADC_JSQR_JEXTSEL | ADC_JSQR_JEXTEN | ADC_JSQR_JSQ1 | ADC_JSQR_JSQ2 |     \
          ADC_JSQR_JSQ3 | ADC_JSQR_JSQ4)) /*!< ADC_JSQR fields of parameters that can be updated \
...
#    define ADC_CALIBRATION_TIMEOUT \
        (158379UL) /*!< ADC calibration time-out value (unit: CPU cycles) */
...
@verbatim
 ...
 ===============================================================================
    [..]  This section provides functions allowing to:
      ...
      (+) Start conversion of ADC group injected (not available on devices: STM32WB10xx,
STM32WB15xx, STM32WB1Mxx ).
      (+) Stop conversion of ADC group injected (not available on devices: STM32WB10xx, STM32WB15xx,
STM32WB1Mxx ).
      (+) Poll for conversion complete on ADC group injected (not available on devices: STM32WB10xx,
STM32WB15xx, STM32WB1Mxx ).
      (+) Get result of ADC group injected channel conversion (not available on devices:
STM32WB10xx, STM32WB15xx, STM32WB1Mxx ).
      (+) Start conversion of ADC group injected and enable interruptions (not available on devices:
STM32WB10xx, STM32WB15xx, STM32WB1Mxx ).
      (+) Stop conversion of ADC group injected and disable interruptions (not available on devices:
STM32WB10xx, STM32WB15xx, STM32WB1Mxx ).
...
#    if defined(ADC_SUPPORT_2_5_MSPS)
        ...
        /*       calibration.                                                     */
        backup_setting_cfgr1 =
            READ_BIT(hadc->Instance->CFGR1, ADC_CFGR1_DMAEN | ADC_CFGR1_DMACFG | ADC_CFGR1_AUTOFF);
...
#    if defined(ADC_SUPPORT_2_5_MSPS)
        ...
        /* Case of ADC clocked at low frequency: Delay required between ADC enable and disable
         * actions */
        if (LL_ADC_GetClock(hadc->Instance) == LL_ADC_CLOCK_ASYNC) {
            ...
            if (adc_clk_async_presc >= LL_ADC_CLOCK_ASYNC_DIV16) {
                ...
                /* Delay depends on ADC clock prescaler: Compute ADC clock asynchronous prescaler to
                 * decimal format */
...
HAL_StatusTypeDef HAL_ADCEx_InjectedStart(ADC_HandleTypeDef* hadc) {
    ...
    if (LL_ADC_INJ_IsConversionOngoing(hadc->Instance) != 0UL) {
        ...
        if (tmp_hal_status == HAL_OK) {
            ...
            ADC_STATE_CLR_SET(hadc->State, HAL_ADC_STATE_READY | HAL_ADC_STATE_INJ_EOC,
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef* hadc,
    ...
    if ((hadc->Init.ScanConvMode == ADC_SCAN_DISABLE) ||
        ...
        if (hadc->InjectionConfig.ChannelCount == 0U) {
            ...
            /* Handle hadc saves the context under build up over each
               HAL_ADCEx_InjectedConfigChannel() call, this context will be written in JSQR register
               at the last call. At this point, the context is merely reset  */
        ...
        /* 3. tmp_JSQR_ContextQueueBeingBuilt is fully built for this
           HAL_ADCEx_InjectedConfigChannel() call, aggregate the setting to those already built
           during the previous HAL_ADCEx_InjectedConfigChannel() calls (for the same context of
           course)  */
        hadc->InjectionConfig.ContextQueue |= tmp_JSQR_ContextQueueBeingBuilt;
    ...
    if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
        ...
        LL_ADC_SetChannelSingleDiff(hadc->Instance, sConfigInjected->InjectedChannel,
                                    sConfigInjected->InjectedSingleDiff);

        /* Configuration of differential mode */
        /* Note: ADC channel number masked with value "0x1F" to ensure shift value within 32 bits
         * range */
        if (sConfigInjected->InjectedSingleDiff == ADC_DIFFERENTIAL_ENDED) {
            /* Set sampling time of the selected ADC channel */
            LL_ADC_SetChannelSamplingTime(
                ...
                (uint32_t)(__LL_ADC_DECIMAL_NB_TO_CHANNEL(
                    (__LL_ADC_CHANNEL_TO_DECIMAL_NB((uint32_t)sConfigInjected->InjectedChannel) +
                     1UL) &
                    0x1FUL)),
                sConfigInjected->InjectedSamplingTime);
        }
