...
#    if (USE_HAL_RTC_REGISTER_CALLBACKS == 1)
    if (hrtc->State == HAL_RTC_STATE_RESET) {
        ...
        hrtc->AlarmAEventCallback =
            HAL_RTC_AlarmAEventCallback; /* Legacy weak AlarmAEventCallback      */
        hrtc->AlarmBEventCallback =
            HAL_RTCEx_AlarmBEventCallback; /* Legacy weak AlarmBEventCallback      */
        ...
        hrtc->WakeUpTimerEventCallback =
            HAL_RTCEx_WakeUpTimerEventCallback; /* Legacy weak WakeUpTimerEventCallback */
...
#        if defined(RTC_TAMPER3_SUPPORT)
        hrtc->Tamper3EventCallback =
            HAL_RTCEx_Tamper3EventCallback; /* Legacy weak Tamper3EventCallback     */
...
#    endif /* (USE_HAL_RTC_REGISTER_CALLBACKS) */
    ...
    if (__HAL_RTC_IS_CALENDAR_INITIALIZED(hrtc) == 0U) {
        ...
        if (RTC_EnterInitMode(hrtc) != HAL_OK) {
            ...
            /* Set RTC_CR register */
            hrtc->Instance->CR |=
                (uint32_t)(hrtc->Init.HourFormat | hrtc->Init.OutPut | hrtc->Init.OutPutPolarity);
...
HAL_StatusTypeDef HAL_RTC_SetTime(RTC_HandleTypeDef* hrtc, RTC_TimeTypeDef* sTime,
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        tmpreg = (((uint32_t)(sTime->Hours) << 16U) | ((uint32_t)(sTime->Minutes) << 8U) |
                  ((uint32_t)sTime->Seconds) | ((uint32_t)(sTime->TimeFormat) << 16U));
    }
    UNUSED(tmpreg);
    /* Disable the write protection for RTC registers */
    __HAL_RTC_WRITEPROTECTION_DISABLE(hrtc);
    ...
    if (RTC_EnterInitMode(hrtc) != HAL_OK) {
        ...
        /* This interface is deprecated. To manage Daylight Saving Time, please use HAL_RTC_DST_xxx
         * functions */
 ...
 *            @arg RTC_FORMAT_BIN: Binary data format
 *            @arg RTC_FORMAT_BCD: BCD data format
 * @note  You can use SubSeconds and SecondFraction (sTime structure fields returned) to convert
 * SubSeconds value in second fraction ratio with time unit following generic formula: Second
 * fraction ratio * time_unit= [(SecondFraction-SubSeconds)/(SecondFraction+1)] * time_unit This
 * conversion can be performed only if no shift operation is pending (ie. SHFP=0) when PREDIV_S >=
 * SS
 * @note  You must call HAL_RTC_GetDate() after HAL_RTC_GetTime() to unlock the values
 *        in the higher-order calendar shadow registers to ensure consistency between the time and
 * date values. Reading RTC current time locks the values in calendar shadow registers until Current
...
HAL_StatusTypeDef HAL_RTC_SetDate(RTC_HandleTypeDef* hrtc, RTC_DateTypeDef* sDate,
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        datetmpreg = (((uint32_t)RTC_ByteToBcd2(sDate->Year) << 16U) |
                      ...
                      ((uint32_t)RTC_ByteToBcd2(sDate->Date)) | ((uint32_t)sDate->WeekDay << 13U));
    }
    else {
        ...
        datetmpreg = ((((uint32_t)sDate->Year) << 16U) | (((uint32_t)sDate->Month) << 8U) |
                      ((uint32_t)sDate->Date) | (((uint32_t)sDate->WeekDay) << 13U));
    }
...
HAL_StatusTypeDef HAL_RTC_SetAlarm(RTC_HandleTypeDef* hrtc, RTC_AlarmTypeDef* sAlarm,
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        tmpreg = (((uint32_t)RTC_ByteToBcd2(sAlarm->AlarmTime.Hours) << 16U) |
                  ...
                  ((uint32_t)sAlarm->AlarmDateWeekDaySel) | ((uint32_t)sAlarm->AlarmMask));
    }
    else {
        if ((hrtc->Instance->CR & RTC_CR_FMT) != 0U) {
        ...
        if (sAlarm->AlarmDateWeekDaySel == RTC_ALARMDATEWEEKDAYSEL_DATE) {
            ...
            assert_param(
                IS_RTC_ALARM_DATE_WEEKDAY_WEEKDAY(RTC_Bcd2ToByte(sAlarm->AlarmDateWeekDay)));
        }

        tmpreg =
            (((uint32_t)(sAlarm->AlarmTime.Hours) << 16U) |
             ((uint32_t)(sAlarm->AlarmTime.Minutes) << 8U) | ((uint32_t)sAlarm->AlarmTime.Seconds) |
             ((uint32_t)(sAlarm->AlarmTime.TimeFormat) << 16U) |
