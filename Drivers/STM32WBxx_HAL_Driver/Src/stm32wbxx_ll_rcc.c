...
#            define IS_LL_RCC_I2C_CLKSOURCE(__VALUE__) \
                (((__VALUE__) == LL_RCC_I2C1_CLKSOURCE) || ((__VALUE__) == LL_RCC_I2C3_CLKSOURCE))
#        else
...
ErrorStatus LL_RCC_DeInit(void) {
    ...
    /* Reset CFGR register */
    LL_RCC_WriteReg(
        CFGR, 0x00070000U); /* MSI selected as System Clock and all prescaler to not divided */
...
#        endif /* SAI1 */
    ...
    vl_mask = RCC_CICR_LSI1RDYC | RCC_CICR_LSERDYC | RCC_CICR_MSIRDYC | RCC_CICR_HSIRDYC |
...
#        endif /* RCC_HSI48_SUPPORT */
    ...
    LL_RCC_WriteReg(
        HSECR,
        LL_RCC_HSE_CURRENTMAX_3); /* HSEGMC set to default value 011, current max limit 1.13 mA/V */
 ...
 * @brief  Return the frequencies of different on chip clocks;  System, AHB, APB1 and APB2 buses
 * clocks and different peripheral clocks available on the device.
 ...
 * @param  RCC_Clocks pointer to a @ref LL_RCC_ClocksTypeDef structure which will hold the clocks
 * frequencies
 * @retval None
 */
...
uint32_t LL_RCC_GetUSARTClockFreq(uint32_t USARTxSource) {
    ...
    switch (LL_RCC_GetUSARTClockSource(USARTxSource)) {
        ...
        default:
            usart_frequency =
                RCC_GetPCLK2ClockFreq(RCC_GetHCLK1ClockFreq(RCC_GetSystemClockFreq()));
            break;
    }
...
uint32_t LL_RCC_GetI2CClockFreq(uint32_t I2CxSource) {
    ...
    if (I2CxSource == LL_RCC_I2C1_CLKSOURCE) {
        ...
        switch (LL_RCC_GetI2CClockSource(I2CxSource)) {
            ...
            default:
                i2c_frequency =
                    RCC_GetPCLK1ClockFreq(RCC_GetHCLK1ClockFreq(RCC_GetSystemClockFreq()));
                break;
        }
    }
#        if defined(I2C3)
 ...
 *         - @ref  LL_RCC_PERIPH_FREQUENCY_NO indicates that oscillator (HSI, LSI or LSE) is not
 * ready
 */
 ...
 *         - @ref  LL_RCC_PERIPH_FREQUENCY_NO indicates that oscillator (MSI or HSI48) or PLLs (PLL
 * or PLLSAI1) is not ready
 */
 ...
 *         - @ref  LL_RCC_PERIPH_FREQUENCY_NO indicates that oscillators (LSI, LSE or HSE) are not
 * ready
 *         - @ref  LL_RCC_PERIPH_FREQUENCY_NA indicates that no clock source selected
 */
...
static uint32_t RCC_PLL_GetFreqDomain_SYS(void) {
    ...
    */
    ...
    return __LL_RCC_CALC_PLLCLK_FREQ(pllinputfreq, LL_RCC_PLL_GetDivider(), LL_RCC_PLL_GetN(),
                                     LL_RCC_PLL_GetR());
}
...
static uint32_t RCC_PLL_GetFreqDomain_SAI(void) {
    ...
    */
    ...
    return __LL_RCC_CALC_PLLCLK_SAI_FREQ(pllinputfreq, LL_RCC_PLL_GetDivider(), LL_RCC_PLL_GetN(),
                                         LL_RCC_PLL_GetP());
}
#        endif /* SAI1 */
