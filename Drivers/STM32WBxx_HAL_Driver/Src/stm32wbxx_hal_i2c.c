/**
  ...
  ******************************************************************************
  ...
  ==============================================================================
    ...
    (#) Configure the Communication Clock Timing, Own Address1, Master Addressing mode, Dual
  Addressing mode, Own Address2, Own Address2 Mask, General call and Nostretch mode in the hi2c Init
  structure.
  ...
  HAL_I2C_Slave_Receive_IT()
    ...
    *** Interrupt mode or DMA mode IO sequential operation ***
    ...
    [..]
      ...
      (++) I2C_FIRST_AND_LAST_FRAME: No sequential usage, functional is same as associated
  interfaces in no sequential mode
      (++) I2C_FIRST_FRAME: Sequential usage, this option allow to manage a sequence with start
  ...
  sequence with start condition, address and data to transfer without a final stop condition, an
  then permit a call the same master sequential interface several times (like
  ...
  condition, address and with new data to transfer if the direction change or manage only the new
  data to transfer if no direction change and without a final stop condition in both cases
      (++) I2C_LAST_FRAME: Sequential usage, this option allow to manage a sequance with a restart
  condition, address and with new data to transfer if the direction change or manage only the new
  data to transfer if no direction change and with a final stop condition in both cases
      (++) I2C_LAST_FRAME_NO_STOP: Sequential usage (Master only), this option allow to manage a
  restart condition after several call of the same master sequential interface several times (link
  with option I2C_FIRST_AND_NEXT_FRAME). Usage can, transfer several bytes one by one using
                             ...
                             Then usage of this option I2C_LAST_FRAME_NO_STOP at the last Transmit
  or Receive sequence permit to call the opposite interface Receive or Transmit without stopping the
  ...
  condition after each call of the same master sequential interface. Usage can, transfer several
  bytes one by one with a restart with slave address between each bytes using
                              HAL_I2C_Master_Seq_Transmit_IT
                              or HAL_I2C_Master_Seq_Receive_IT
                              or HAL_I2C_Master_Seq_Transmit_DMA
                              or HAL_I2C_Master_Seq_Receive_DMA
                              with option I2C_FIRST_FRAME then I2C_OTHER_FRAME.
                            Then usage of this option I2C_OTHER_AND_LAST_FRAME at the last frame to
  help automatic generation of STOP condition.
      ...
      (+++) At transmission end of current frame transfer, HAL_I2C_MasterTxCpltCallback() is
  executed and users can add their own code by customization of function pointer
  HAL_I2C_MasterTxCpltCallback()
      (++) Sequential receive in master I2C mode an amount of data in non-blocking mode using
            HAL_I2C_Master_Seq_Receive_IT() or using HAL_I2C_Master_Seq_Receive_DMA()
      (+++) At reception end of current frame transfer, HAL_I2C_MasterRxCpltCallback() is executed
  and users can add their own code by customization of function pointer
  HAL_I2C_MasterRxCpltCallback()
      (++) Abort a master IT or DMA I2C process communication with Interrupt using
  HAL_I2C_Master_Abort_IT()
      ...
      (+++) When address slave I2C match, HAL_I2C_AddrCallback() is executed and users can
           add their own code to check the Address Match Code and the transmission direction request
  by master (Write/Read).
      ...
      (+++) At transmission end of current frame transfer, HAL_I2C_SlaveTxCpltCallback() is executed
  and users can add their own code by customization of function pointer
  HAL_I2C_SlaveTxCpltCallback()
      ...
      (+++) At reception end of current frame transfer, HAL_I2C_SlaveRxCpltCallback() is executed
  and users can add their own code by customization of function pointer
  ...
  using HAL_I2C_Mem_Read_IT()
    ...
    *** DMA mode IO MEM operation ***
    ...
    [..]
     For specific callback AddrCallback use dedicated register callbacks :
...
#    define I2C_XFER_TX_IT                                        \
        (uint16_t)(0x0001U) /*!< Bit field can be combinated with \
                               @ref I2C_XFER_LISTEN_IT */
#    define I2C_XFER_RX_IT                                        \
        (uint16_t)(0x0002U) /*!< Bit field can be combinated with \
                               @ref I2C_XFER_LISTEN_IT */
#    define I2C_XFER_LISTEN_IT                                                        \
        (uint16_t)(0x8000U) /*!< Bit field can be combinated with @ref I2C_XFER_TX_IT \
                               and @ref I2C_XFER_RX_IT */

#    define I2C_XFER_ERROR_IT                                                      \
        (uint16_t)(0x0010U) /*!< Bit definition to manage addition of global Error \
                               and NACK treatment */
#    define I2C_XFER_CPLT_IT \
        (uint16_t)(0x0020U) /*!< Bit definition to manage only STOP evenement */
#    define I2C_XFER_RELOAD_IT \
        (uint16_t)(0x0040U) /*!< Bit definition to manage only Reload of NBYTE */
...
#    if (USE_HAL_I2C_REGISTER_CALLBACKS == 1)
        ...
        hi2c->MasterTxCpltCallback =
            HAL_I2C_MasterTxCpltCallback; /* Legacy weak MasterTxCpltCallback */
        hi2c->MasterRxCpltCallback =
            HAL_I2C_MasterRxCpltCallback; /* Legacy weak MasterRxCpltCallback */
        hi2c->SlaveTxCpltCallback =
            HAL_I2C_SlaveTxCpltCallback; /* Legacy weak SlaveTxCpltCallback  */
        hi2c->SlaveRxCpltCallback =
            HAL_I2C_SlaveRxCpltCallback; /* Legacy weak SlaveRxCpltCallback  */
...
#    endif /* USE_HAL_I2C_REGISTER_CALLBACKS */
    ...
    hi2c->Instance->OAR2 =
        (hi2c->Init.DualAddressMode | hi2c->Init.OwnAddress2 | (hi2c->Init.OwnAddress2Masks << 8));
 ...
 * @note   The HAL_I2C_UnRegisterCallback() may be called before HAL_I2C_Init() in
 * HAL_I2C_STATE_RESET to un-register callbacks for HAL_I2C_MSPINIT_CB_ID and
        ...
        switch (CallbackID) {
            ...
            case HAL_I2C_LISTEN_COMPLETE_CB_ID:
                hi2c->ListenCpltCallback =
                    HAL_I2C_ListenCpltCallback; /* Legacy weak ListenCpltCallback   */
                break;

            case HAL_I2C_MEM_TX_COMPLETE_CB_ID:
                hi2c->MemTxCpltCallback =
                    HAL_I2C_MemTxCpltCallback; /* Legacy weak MemTxCpltCallback    */
                break;

            case HAL_I2C_MEM_RX_COMPLETE_CB_ID:
                hi2c->MemRxCpltCallback =
                    HAL_I2C_MemRxCpltCallback; /* Legacy weak MemRxCpltCallback    */
                break;
            ...
            case HAL_I2C_ABORT_CB_ID:
                hi2c->AbortCpltCallback =
                    HAL_I2C_AbortCpltCallback; /* Legacy weak AbortCpltCallback    */
                break;
 ...
 *         Info Ready I2C Callback is redirected to the weak HAL_I2C_AddrCallback() predefined
 * callback
 * @param  hi2c Pointer to a I2C_HandleTypeDef structure that contains
 *                the configuration information for the specified I2C.
 * @retval HAL status
...
HAL_StatusTypeDef HAL_I2C_Master_Transmit(I2C_HandleTypeDef* hi2c, uint16_t DevAddress,
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_BUSY, SET, I2C_TIMEOUT_BUSY, tickstart) !=
            HAL_OK) {
            return HAL_ERROR;
        }
        ...
        if (hi2c->XferSize > 0U) {
            ...
            /* Send Slave Address */
            /* Set NBYTES to write and reload if hi2c->XferCount > MAX_NBYTE_SIZE and generate
             * RESTART */
        ...
        while (hi2c->XferCount > 0U) {
            ...
            if ((hi2c->XferCount != 0U) && (hi2c->XferSize == 0U)) {
                ...
                if (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_TCR, RESET, Timeout, tickstart) !=
                    HAL_OK) {
                    return HAL_ERROR;
                }
...
HAL_StatusTypeDef HAL_I2C_Slave_Transmit(I2C_HandleTypeDef* hi2c, uint8_t* pData, uint16_t Size,
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->Init.AddressingMode == I2C_ADDRESSINGMODE_10BIT) {
            /* Wait until ADDR flag is set */
            if (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_ADDR, RESET, Timeout, tickstart) !=
                HAL_OK) {
                /* Disable Address Acknowledge */
                hi2c->Instance->CR2 |= I2C_CR2_NACK;
                return HAL_ERROR;
...
HAL_StatusTypeDef HAL_I2C_Master_Receive_DMA(I2C_HandleTypeDef* hi2c, uint16_t DevAddress,
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
            ...
            if (dmaxferstatus == HAL_OK) {
                /* Send Slave Address */
                /* Set NBYTES to read and reload if hi2c->XferCount > MAX_NBYTE_SIZE and generate
                 * RESTART */
                I2C_TransferConfig(hi2c, DevAddress, (uint8_t)hi2c->XferSize, xfermode,
                                   I2C_GENERATE_START_READ);
...
HAL_StatusTypeDef HAL_I2C_Mem_Read(I2C_HandleTypeDef* hi2c, uint16_t DevAddress,
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        do {
            /* Wait until RXNE flag is set */
            if (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_RXNE, RESET, Timeout, tickstart) !=
                HAL_OK) {
                return HAL_ERROR;
            }
...
HAL_StatusTypeDef HAL_I2C_Mem_Write_DMA(I2C_HandleTypeDef* hi2c, uint16_t DevAddress,
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->hdmatx != NULL) {
            ...
            /* Enable the DMA channel */
            dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmatx, (uint32_t)pData,
                                             (uint32_t)&hi2c->Instance->TXDR, hi2c->XferSize);
        }
        else {
            /* Update I2C state */
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        do {
            ...
            if (__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_AF) == RESET) {
                /* Wait until STOPF flag is reset */
                if (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_STOPF, RESET, Timeout, tickstart) !=
                    HAL_OK) {
                    return HAL_ERROR;
                }
...
/**
 * @brief  Sequential transmit in master I2C mode an amount of data in non-blocking mode with
 * Interrupt.
 * @note   This interface allow to manage repeated start condition when a direction change during
 * transfer
 * @param  hi2c Pointer to a I2C_HandleTypeDef structure that contains
 *                the configuration information for the specified I2C.
 * @param  DevAddress Target device address: The device 7 bits address value
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_I2C_Master_Seq_Transmit_IT(I2C_HandleTypeDef* hi2c, uint16_t DevAddress,
                                                 uint8_t* pData, uint16_t Size,
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if ((hi2c->XferSize > 0U) &&
            ((XferOptions == I2C_FIRST_FRAME) || (XferOptions == I2C_FIRST_AND_LAST_FRAME))) {
            /* Preload TX register */
            /* Write data to TXDR */
            hi2c->Instance->TXDR = *hi2c->pBuffPtr;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_I2C_Master_Seq_Transmit_DMA(I2C_HandleTypeDef* hi2c, uint16_t DevAddress,
                                                  uint8_t* pData, uint16_t Size,
                                                  uint32_t XferOptions) {
    uint32_t xfermode;
    uint32_t xferrequest = I2C_GENERATE_START_WRITE;
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
            ...
            if (dmaxferstatus == HAL_OK) {
                /* Send Slave Address and set NBYTES to write */
                if ((XferOptions == I2C_FIRST_FRAME) || (XferOptions == I2C_FIRST_AND_LAST_FRAME)) {
                    I2C_TransferConfig(hi2c, DevAddress, (uint8_t)sizetoxfer, xfermode,
                                       xferrequest);
                }
                else {
                    I2C_TransferConfig(hi2c, DevAddress, (uint8_t)hi2c->XferSize, xfermode,
...
/**
 * @brief  Sequential receive in master I2C mode an amount of data in non-blocking mode with
 * Interrupt
 * @note   This interface allow to manage repeated start condition when a direction change during
 * transfer
 * @param  hi2c Pointer to a I2C_HandleTypeDef structure that contains
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_I2C_Master_Seq_Receive_DMA(I2C_HandleTypeDef* hi2c, uint16_t DevAddress,
                                                 uint8_t* pData, uint16_t Size,
                                                 uint32_t XferOptions) {
    uint32_t xfermode;
    uint32_t xferrequest = I2C_GENERATE_START_READ;
...
/**
 * @brief  Sequential transmit in slave/device I2C mode an amount of data in non-blocking mode with
 * Interrupt
 * @note   This interface allow to manage repeated start condition when a direction change during
 * transfer
 * @param  hi2c Pointer to a I2C_HandleTypeDef structure that contains
...
/**
 * @brief  Sequential receive in slave/device I2C mode an amount of data in non-blocking mode with
 * Interrupt
 * @note   This interface allow to manage repeated start condition when a direction change during
 * transfer
 * @param  hi2c Pointer to a I2C_HandleTypeDef structure that contains
...
HAL_StatusTypeDef HAL_I2C_Master_Abort_IT(I2C_HandleTypeDef* hi2c, uint16_t DevAddress) {
    if (hi2c->Mode == HAL_I2C_MODE_MASTER) {
        ...
        /* Set AUTOEND mode, this will generate a NACK then STOP condition to abort the current
         * transfer */
...
void HAL_I2C_ER_IRQHandler(I2C_HandleTypeDef* hi2c) {
    ...
    if ((tmperror & (HAL_I2C_ERROR_BERR | HAL_I2C_ERROR_OVR | HAL_I2C_ERROR_ARLO)) !=
        HAL_I2C_ERROR_NONE) {
 ...
 * @param  hi2c Pointer to a I2C_HandleTypeDef structure that contains
 *                the configuration information for the specified I2C.
 * @param  TransferDirection Master request Transfer Direction (Write/Read), value of @ref
 * I2C_XFERDIRECTION
 * @param  AddrMatchCode Address Match Code
 * @retval None
 */
__weak void HAL_I2C_AddrCallback(I2C_HandleTypeDef* hi2c, uint8_t TransferDirection,
        ...
        if ((hi2c->XferCount != 0U) && (hi2c->XferSize == 0U)) {
            ...
            if (hi2c->XferCount > MAX_NBYTE_SIZE) {
                ...
                if (hi2c->XferOptions != I2C_NO_OPTION_FRAME) {
                    I2C_TransferConfig(hi2c, devaddress, (uint8_t)hi2c->XferSize, hi2c->XferOptions,
                                       I2C_NO_STARTSTOP);
                }
                else {
                    I2C_TransferConfig(hi2c, devaddress, (uint8_t)hi2c->XferSize, I2C_AUTOEND_MODE,
...
static HAL_StatusTypeDef I2C_Slave_ISR_DMA(struct __I2C_HandleTypeDef* hi2c, uint32_t ITFlags,
    ...
    if ((I2C_CHECK_FLAG(ITFlags, I2C_FLAG_AF) != RESET) &&
        ...
        if ((I2C_CHECK_IT_SOURCE(ITSources, I2C_CR1_TXDMAEN) != RESET) ||
            ...
            if (treatdmanack == 1U) {
                if ((hi2c->State == HAL_I2C_STATE_LISTEN) &&
                    (tmpoptions == I2C_FIRST_AND_LAST_FRAME))
                /* Same action must be done for (tmpoptions == I2C_LAST_FRAME) which removed for
                   Warning[Pa134]: left and right operands are identical */
                {
                    /* Call I2C Listen complete process */
                ...
                /* if no, error use case, a Non-Acknowledge of last Data is generated by the
                 * MASTER*/
                /* Clear NACK Flag */
                __HAL_I2C_CLEAR_FLAG(hi2c, I2C_FLAG_AF);
                ...
                if ((tmpoptions == I2C_FIRST_FRAME) || (tmpoptions == I2C_NEXT_FRAME)) {
                    if ((tmpstate == HAL_I2C_STATE_BUSY_TX) ||
                        (tmpstate == HAL_I2C_STATE_BUSY_TX_LISTEN)) {
                        hi2c->PreviousState = I2C_STATE_SLAVE_BUSY_TX;
                    }
                    else if ((tmpstate == HAL_I2C_STATE_BUSY_RX) ||
...
static void I2C_ITAddrCplt(I2C_HandleTypeDef* hi2c, uint32_t ITFlags) {
    ...
    if (((uint32_t)hi2c->State & (uint32_t)HAL_I2C_STATE_LISTEN) ==
        ...
        if (hi2c->Init.AddressingMode == I2C_ADDRESSINGMODE_10BIT) {
            if ((slaveaddrcode & SLAVE_ADDR_MSK) ==
                ((ownadd1code >> SLAVE_ADDR_SHIFT) & SLAVE_ADDR_MSK)) {
...
static void I2C_ITMasterCplt(I2C_HandleTypeDef* hi2c, uint32_t ITFlags) {
    ...
    if ((hi2c->State == HAL_I2C_STATE_ABORT) &&
        (I2C_CHECK_FLAG(tmpITFlags, I2C_FLAG_RXNE) != RESET)) {
        /* Read data from RXDR */
...
#    endif /* HAL_DMA_MODULE_ENABLED */
    ...
    /* Disable Interrupts */
    if ((tmpstate == HAL_I2C_STATE_LISTEN) || (tmpstate == HAL_I2C_STATE_BUSY_TX_LISTEN) ||
        (tmpstate == HAL_I2C_STATE_BUSY_RX_LISTEN)) {
...
static void I2C_DMAMasterTransmitCplt(DMA_HandleTypeDef* hdma) {
    ...
    /* else prepare a new DMA transfer and enable TCReload interrupt */
        ...
        /* Enable the DMA channel */
        if (HAL_DMA_Start_IT(hi2c->hdmatx, (uint32_t)hi2c->pBuffPtr,
                             (uint32_t)&hi2c->Instance->TXDR, hi2c->XferSize) != HAL_OK) {
            /* Call the corresponding callback to inform upper layer of End of Transfer */
...
static void I2C_DMAMasterReceiveCplt(DMA_HandleTypeDef* hdma) {
    ...
    /* else prepare a new DMA transfer and enable TCReload interrupt */
        ...
        /* Enable the DMA channel */
        if (HAL_DMA_Start_IT(hi2c->hdmarx, (uint32_t)&hi2c->Instance->RXDR,
                             (uint32_t)hi2c->pBuffPtr, hi2c->XferSize) != HAL_OK) {
            /* Call the corresponding callback to inform upper layer of End of Transfer */
            I2C_ITError(hi2c, HAL_I2C_ERROR_DMA);
        }
        else {
            ...
            if (Timeout != HAL_MAX_DELAY) {
                if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) {
                    ...
                    if ((__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_BUSY) != RESET) &&
                        (tmp1 != I2C_CR2_STOP) && (tmp2 != HAL_I2C_MODE_SLAVE)) {
                        /* Generate Stop */
                        hi2c->Instance->CR2 |= I2C_CR2_STOP;
...
/**
 * @brief  Handles I2Cx communication when starting transfer or during transfer (TC or TCR flag are
 * set).
 * @param  hi2c I2C handle.
