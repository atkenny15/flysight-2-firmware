/**
  ...
  ******************************************************************************
  ...
  the DMA Tx/Rx Stream.

    (#) The initialization can be done by two ways
        (##) Expert mode : Initialize the structures Init, FrameInit and SlotInit and call
  HAL_SAI_Init().
        (##) Simplified mode : Initialize the high part of Init Structure and call
  ...
  [..]
    (@) Make sure that either:
        ...
        (+@) External clock source is configured after setting correctly
             the define constant EXTERNAL_SAI1_CLOCK_VALUE or EXTERNAL_SAI2_CLOCK_VALUE in the
  stm32wbxx_hal_conf.h file.
...
#        endif
    ...
    if ((hsai->Init.AudioMode == SAI_MODEMASTER_TX) || (hsai->Init.AudioMode == SAI_MODESLAVE_TX)) {
        /* Transmit */
        ckstr_bits =
            (hsai->Init.ClockStrobing == SAI_CLOCKSTROBING_RISINGEDGE) ? 0U : SAI_xCR1_CKSTR;
    }
    else {
        /* Receive */
        ckstr_bits =
            (hsai->Init.ClockStrobing == SAI_CLOCKSTROBING_RISINGEDGE) ? SAI_xCR1_CKSTR : 0U;
    }
    ...
    hsai->Instance->CR1 |=
        (hsai->Init.AudioMode | hsai->Init.Protocol | hsai->Init.DataSize | hsai->Init.FirstBit |
    ...
    hsai->Instance->CR2 |=
        (hsai->Init.FIFOThreshold | hsai->Init.CompandingMode | hsai->Init.TriState);
        ...
        while (hsai->XferCount > 0U) {
            ...
            if ((hsai->Instance->SR & SAI_xSR_FLVL) != SAI_FIFOSTATUS_FULL) {
                ...
                if ((((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) &&
                    ...
                    /* No need to check return value because state update, unlock and error return
...
HAL_StatusTypeDef HAL_SAI_DMAStop(SAI_HandleTypeDef* hsai) {
    ...
    if ((hsai->State == HAL_SAI_STATE_BUSY_TX) && (hsai->hdmatx != NULL)) {
        ...
        /* Only HAL_DMA_ERROR_NO_XFER can be returned in case of error and it's not an error for
         * SAI. */
...
void HAL_SAI_IRQHandler(SAI_HandleTypeDef* hsai) {
    if (hsai->State != HAL_SAI_STATE_RESET) {
        ...
        /* SAI Overrun error interrupt occurred ----------------------------------*/
            ...
            tmperror =
                ((hsai->State == HAL_SAI_STATE_BUSY_RX) ? HAL_SAI_ERROR_OVR : HAL_SAI_ERROR_UDR);
...
static void SAI_DMAError(DMA_HandleTypeDef* hdma) {
    ...
    /* Disable SAI peripheral */
    /* No need to check return value because state will be updated and HAL_SAI_ErrorCallback will be
     * called later */
    (void)SAI_Disable(hsai);
...
static void SAI_DMAAbort(DMA_HandleTypeDef* hdma) {
    ...
    if (hsai->ErrorCode != HAL_SAI_ERROR_WCKCFG) {
        /* Disable SAI peripheral */
        /* No need to check return value because state will be updated and HAL_SAI_ErrorCallback
