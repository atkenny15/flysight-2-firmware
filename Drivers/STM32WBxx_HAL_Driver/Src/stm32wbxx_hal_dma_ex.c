/**
  ...
  ******************************************************************************
  ...
  function. Functions HAL_DMAEx_EnableMuxRequestGenerator and HAL_DMAEx_DisableMuxRequestGenerator
  ...
  HAL_DMAEx_MUX_IRQHandler should be called with, as parameter, the appropriate DMA handle as many
  as used DMAs in the user project (exception done if a given DMA is not using the DMAMUX SYNC block
  neither a request generator)
...
@verbatim
 ...
 ===============================================================================
    ...
    (+) Configure the DMAMUX Request Generator Block using HAL_DMAEx_ConfigMuxRequestGenerator
function. Functions HAL_DMAEx_EnableMuxRequestGenerator and HAL_DMAEx_DisableMuxRequestGenerator can
 ...
 * @param  pSyncConfig Pointer to HAL_DMA_MuxSyncConfigTypeDef : contains the DMAMUX synchronization
 * parameters
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxSync(DMA_HandleTypeDef* hdma,
    ...
    if (hdma->State == HAL_DMA_STATE_READY) {
        ...
        MODIFY_REG(hdma->DMAmuxChannel->CCR,
                   (DMAMUX_CxCR_SYNC_ID | DMAMUX_CxCR_NBREQ | DMAMUX_CxCR_SPOL | DMAMUX_CxCR_SE |
                    DMAMUX_CxCR_EGE),
...
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxRequestGenerator(
    ...
    */
    if ((hdma->State == HAL_DMA_STATE_READY) && (hdma->DMAmuxRequestGen != 0U)) {
        ...
        WRITE_REG(hdma->DMAmuxRequestGen->RGCR,
                  (pRequestGeneratorConfig->SignalID | pRequestGeneratorConfig->Polarity |
