    ...
    (#) Enable the ADC interface
        ...
        (++) Two clock settings are mandatory:
             ...
             (+++) ADC clock (conversions clock).
                   ...
                   or asynchronous clock derived from system clock, PLLSAI1 (output divider R) or
  the PLL system (output divider P) running up to 64MHz.
             ...
             (+++) Example:
               ...
               RCC_ADCCLKSOURCE_PLL enable:                       (optional: if asynchronous clock
  selected)
...
#    define ADC_CFGR_FIELDS_1                                                                 \
        ((ADC_CFGR_RES | ADC_CFGR_ALIGN | ADC_CFGR_CONT | ADC_CFGR_OVRMOD | ADC_CFGR_DISCEN | \
...
#        define ADC_CHSELR_SQ2_TO_SQ8                                                             \
            (ADC_CHSELR_SQ2 | ADC_CHSELR_SQ3 | ADC_CHSELR_SQ4 | ADC_CHSELR_SQ5 | ADC_CHSELR_SQ6 | \
             ADC_CHSELR_SQ7 | ADC_CHSELR_SQ8)
...
#    if defined(ADC_SUPPORT_2_5_MSPS)
        ...
        if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
            ...
            tmpCFGR2 |=
                ((hadc->Init.ClockPrescaler & ADC_CFGR2_CKMODE) | hadc->Init.TriggerFrequencyMode);

            MODIFY_REG(hadc->Instance->CFGR2, ADC_CFGR2_CKMODE | ADC_CFGR2_LFTRIG, tmpCFGR2);
        ...
        tmpCFGR |= (ADC_CFGR_AUTOWAIT((uint32_t)hadc->Init.LowPowerAutoWait) |
                    ...
                    ADC_CFGR_OVERRUN(hadc->Init.Overrun) | hadc->Init.DataAlign |
        ...
        MODIFY_REG(hadc->Instance->CFGR1,
                   ADC_CFGR1_DISCEN | ADC_CFGR1_AUTOFF | ADC_CFGR1_CHSELRMOD | ADC_CFGR1_WAIT |
                       ADC_CFGR1_CONT | ADC_CFGR1_OVRMOD | ADC_CFGR1_EXTSEL | ADC_CFGR1_EXTEN |
                       ADC_CFGR1_ALIGN | ADC_CFGR1_SCANDIR | ADC_CFGR1_DMACFG,
                   tmpCFGR);
        ...
        if (hadc->Init.ScanConvMode == ADC_SCAN_DISABLE) {
            ...
            MODIFY_REG(hadc->Instance->CHSELR, ADC_CHSELR_SQ_ALL,
                       ((ADC_CHSELR_SQ2_TO_SQ8
                         << (((hadc->Init.NbrOfConversion - 1UL) * ADC_REGULAR_RANK_2) & 0x1FUL)) |
        ...
        if ((hadc->Instance->CFGR1 & ~(ADC_CFGR1_AWD1CH | ADC_CFGR1_AWD1EN | ADC_CFGR1_AWD1SGL |
                                       ADC_CFGR1_RES)) == tmpCFGR) {
            /* Set ADC error code to none */
            ADC_CLEAR_ERRORCODE(hadc);
...
#    else
        ...
        /*  - clock configuration                                                 */
        if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
            if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(__LL_ADC_COMMON_INSTANCE(hadc->Instance)) ==
                0UL) {
        ...
        if (hadc->Init.ExternalTrigConv != ADC_SOFTWARE_START) {
            tmpCFGR |=
                ((hadc->Init.ExternalTrigConv & ADC_CFGR_EXTSEL) | hadc->Init.ExternalTrigConvEdge);
        }
        ...
        if ((tmp_adc_is_conversion_on_going_regular == 0UL) &&
            ...
            tmpCFGR =
                (ADC_CFGR_DFSDM(hadc) | ADC_CFGR_AUTOWAIT((uint32_t)hadc->Init.LowPowerAutoWait) |
                 ADC_CFGR_DMACONTREQ((uint32_t)hadc->Init.DMAContinuousRequests));
...
#    if defined(ADC_SUPPORT_2_5_MSPS)
    /* Reset register IER */
    __HAL_ADC_DISABLE_IT(hadc, (ADC_IT_AWD3 | ADC_IT_AWD2 | ADC_IT_AWD1 | ADC_IT_OVR | ADC_IT_EOS |
                                ADC_IT_EOC | ADC_IT_EOSMP | ADC_IT_RDY));

    /* Reset register ISR */
    __HAL_ADC_CLEAR_FLAG(hadc, (ADC_FLAG_AWD3 | ADC_FLAG_AWD2 | ADC_FLAG_AWD1 | ADC_FLAG_OVR |
    ...
    hadc->Instance->CFGR1 &=
        ~(ADC_CFGR1_AWD1CH | ADC_CFGR1_AWD1EN | ADC_CFGR1_AWD1SGL | ADC_CFGR1_DISCEN |
          ADC_CFGR1_AUTOFF | ADC_CFGR1_WAIT | ADC_CFGR1_CONT | ADC_CFGR1_OVRMOD | ADC_CFGR1_EXTEN |
          ADC_CFGR1_EXTSEL | ADC_CFGR1_ALIGN | ADC_CFGR1_RES | ADC_CFGR1_SCANDIR |
          ADC_CFGR1_DMACFG | ADC_CFGR1_DMAEN | ADC_CFGR1_CHSELRMOD);
...
#    else
    ...
    __HAL_ADC_DISABLE_IT(hadc,
                         (
#        if defined(ADC_SUPPORT_2_5_MSPS)
                             ADC_IT_AWD3 | ADC_IT_AWD2 | ADC_IT_AWD1 | ADC_IT_EOCAL | ADC_IT_CCRDY |
#        else
                             ADC_IT_AWD3 | ADC_IT_AWD2 | ADC_IT_AWD1 | ADC_IT_JEOS | ADC_IT_JEOC |
                             ADC_IT_JQOVF |
#        endif /* ADC_SUPPORT_2_5_MSPS */
                             ADC_IT_EOS | ADC_IT_EOC | ADC_IT_OVR | ADC_IT_EOSMP | ADC_IT_RDY));

    /* Reset register ISR */
    __HAL_ADC_CLEAR_FLAG(
        hadc, (
#        if defined(ADC_SUPPORT_2_5_MSPS)
                  ADC_FLAG_AWD3 | ADC_FLAG_AWD2 | ADC_FLAG_AWD1 | ADC_FLAG_EOCAL | ADC_FLAG_CCRDY |
#        else
                  ADC_FLAG_AWD3 | ADC_FLAG_AWD2 | ADC_FLAG_AWD1 | ADC_FLAG_JEOS | ADC_FLAG_JEOC |
                  ADC_FLAG_JQOVF |
#        endif /* ADC_SUPPORT_2_5_MSPS */
                  ADC_FLAG_EOS | ADC_FLAG_EOC | ADC_FLAG_OVR | ADC_FLAG_EOSMP | ADC_FLAG_RDY));
 ...
 *          @arg @ref HAL_ADC_CONVERSION_HALF_CB_ID          ADC conversion DMA half-transfer
 * callback ID
 *          @arg @ref HAL_ADC_LEVEL_OUT_OF_WINDOW_1_CB_ID    ADC analog watchdog 1 callback ID
 *          @arg @ref HAL_ADC_ERROR_CB_ID                    ADC error callback ID
 *          @arg @ref HAL_ADC_INJ_CONVERSION_COMPLETE_CB_ID  ADC group injected conversion complete
 * callback ID
 *          @arg @ref HAL_ADC_INJ_QUEUE_OVEFLOW_CB_ID        ADC group injected context queue
 * overflow callback ID
...
#    else
            ADC_STATE_CLR_SET(hadc->State, HAL_ADC_STATE_REG_BUSY | HAL_ADC_STATE_INJ_BUSY,
                              HAL_ADC_STATE_READY);
#    endif /* ADC_SUPPORT_2_5_MSPS */
        }
    }
 ...
 *            @arg @ref ADC_AWD1_EVENT   ADC Analog watchdog 1 event (main analog watchdog, present
 ...
 *         mode), the user must:
 *          1. first clear the EOSMP flag if set with macro __HAL_ADC_CLEAR_FLAG(hadc,
 * ADC_FLAG_EOSMP)
...
#    endif /* ADC_SUPPORT_2_5_MSPS */
            ...
            tmp_hal_status = HAL_DMA_Start_IT(hadc->DMA_Handle, (uint32_t)&hadc->Instance->DR,
    ...
    if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL) {
        ...
        if (sConfig->Rank != ADC_RANK_NONE) {
            ...
            if ((hadc->Init.ScanConvMode == ADC_SCAN_SEQ_FIXED) ||
                ...
                MODIFY_REG(
                    hadc->ADCGroupRegularSequencerRanks, ADC_CHSELR_SQ1 << (sConfig->Rank & 0x1FUL),
...
#        endif /* ADC_SUPPORT_2_5_MSPS */
        {
            ...
            if (sConfig->OffsetNumber != ADC_OFFSET_NONE) {
                ...
                LL_ADC_SetOffset(hadc->Instance, sConfig->OffsetNumber, sConfig->Channel,
                                 tmpOffsetShifted);
            }
            else {
        ...
        if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
            ...
            if (sConfig->SingleDiff == ADC_DIFFERENTIAL_ENDED) {
                /* Set sampling time of the selected ADC channel */
                /* Note: ADC channel number masked with value "0x1F" to ensure shift value within 32
                 * bits range */
                LL_ADC_SetChannelSamplingTime(
                    hadc->Instance,
                    (uint32_t)(__LL_ADC_DECIMAL_NB_TO_CHANNEL(
                        (__LL_ADC_CHANNEL_TO_DECIMAL_NB((uint32_t)sConfig->Channel) + 1UL) &
                        0x1FUL)),
...
#    endif /* ADC_SUPPORT_2_5_MSPS */
            ...
            tmpAWDHighThresholdShifted =
                ADC_AWD1THRESHOLD_SHIFT_RESOLUTION(hadc, AnalogWDGConfig->HighThreshold);
            tmpAWDLowThresholdShifted =
                ADC_AWD1THRESHOLD_SHIFT_RESOLUTION(hadc, AnalogWDGConfig->LowThreshold);

            /* Set ADC analog watchdog thresholds value of both thresholds high and low */
            LL_ADC_ConfigAnalogWDThresholds(hadc->Instance, AnalogWDGConfig->WatchdogNumber,
...
#    if defined(ADC_SUPPORT_2_5_MSPS)
        /* Feature "ADC analog watchdog 2 and 3" not available on ADC peripheral of this STM32WB
         * device */
#    else
...
#        endif /* !ADC_SUPPORT_2_5_MSPS */
                    ...
                    if (AnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_2) {
                        SET_BIT(hadc->Instance->AWD2CR,
                                (1UL << (__LL_ADC_CHANNEL_TO_DECIMAL_NB(AnalogWDGConfig->Channel) &
                                         0x1FUL)));
                    }
                    else {
...
#        endif /* ADC_SUPPORT_2_5_MSPS */
            ...
            tmpAWDHighThresholdShifted =
                ADC_AWD23THRESHOLD_SHIFT_RESOLUTION(hadc, AnalogWDGConfig->HighThreshold);
            tmpAWDLowThresholdShifted =
                ADC_AWD23THRESHOLD_SHIFT_RESOLUTION(hadc, AnalogWDGConfig->LowThreshold);
...
#    else
    ...
    if ((tmp_adc_is_conversion_on_going_regular != 0UL) ||
        ...
        if (((hadc->Instance->CFGR & ADC_CFGR_JAUTO) != 0UL) &&
            (hadc->Init.ContinuousConvMode == ENABLE) && (hadc->Init.LowPowerAutoWait == ENABLE)) {
...
#    if defined(ADC_SUPPORT_2_5_MSPS)
        if ((hadc->Instance->CR &
             (ADC_CR_ADCAL | ADC_CR_ADSTP | ADC_CR_ADSTART | ADC_CR_ADDIS | ADC_CR_ADEN)) != 0UL)
#    else
        if ((hadc->Instance->CR & (ADC_CR_ADCAL | ADC_CR_JADSTP | ADC_CR_ADSTP | ADC_CR_JADSTART |
