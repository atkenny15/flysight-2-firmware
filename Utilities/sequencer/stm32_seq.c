...
/**
 * @brief default number of task is default 32 (maximum), can be reduced by redefining in
 * utilities_conf.h
 */
...
void UTIL_SEQ_Run(UTIL_SEQ_bm_t Mask_bm) {
    ...
    while (((local_taskset & local_taskmask & SuperMask) != 0U) &&
           ((local_evtset & local_evtwaited) == 0U)) {
        counter = 0U;
        /*
         * When a flag is set, the associated bit is set in TaskPrio[counter].priority mask
         * depending on the priority parameter given from UTIL_SEQ_SetTask() The while loop is
         * looking for a flag set from the highest priority maskr to the lower
         */
         ...
         * The concept is to make sure that on each round on UTIL_SEQ_Run(), if two same flags are
         * always set, the sequencer does not run always only the first one. When a task has been
         * executed, The flag is removed from the round_robin mask. If on the next UTIL_SEQ_RUN(),
         * the two same flags are set again, the round_robin mask will mask out the first flag so
         * that the second one can be executed. Note that the first flag is not removed from the
         * list of pending task but just masked by the round_robin mask
         *
         * In the check below, the round_robin mask is reinitialize in case all pending tasks haven
         ...
         * Once the index is read, the associated task will be executed even though a higher
         * priority stack is requested before task execution.
         */
    ...
    /* the set of CurrentTaskIdx to no task running allows to call WaitEvt in the Pre/Post ilde
     * context */
...
void UTIL_SEQ_WaitEvt(UTIL_SEQ_bm_t EvtId_bm) {
    ...
    /*
     * Restore the CurrentTaskIdx that may have been modified by call of UTIL_SEQ_Run() from
     * UTIL_SEQ_EvtIdle() This is required so that a second call of UTIL_SEQ_WaitEvt() in the same
     * process pass the correct current_task_id_bm in the call of UTIL_SEQ_EvtIdle()
     */
...
#if (__CORTEX_M == 0)
const uint8_t SEQ_clz_table_4bit[16U] = { 4U, 3U, 2U, 2U, 1U, 1U, 1U, 1U,
