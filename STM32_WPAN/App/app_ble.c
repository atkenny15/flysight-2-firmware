...
void APP_BLE_Init(void) {
    ...
    if (status != SHCI_Success) {
        ...
        /* if you are here, maybe CPU2 doesn't contain STM32WB_Copro_Wireless_Binaries, see
         * Release_Notes.html */
        Error_Handler();
    }
    else {
    ...
    HW_TS_Create(CFG_TIM_PROC_ID_ISR, &(BleApplicationContext.Advertising_mgr_timer_Id),
                 hw_ts_SingleShot, Adv_Update_Req);
#if 0
        ...
        case HCI_LE_META_EVT_CODE: {
            ...
            switch (p_meta_evt->subevent) {
                ...
                case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE: {
                    ...
                    APP_DBG_MSG(">>== HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE - Connection "
                                "handle: 0x%x\n",
            ...
            switch (p_blecore_evt->ecode) {
                ...
                case ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE:
                    ...
                    ret = aci_gap_numeric_comparison_value_confirm_yesno(
                        BleApplicationContext.BleApplicationContext_legacy.connectionHandle, YES);
                    if (ret != BLE_STATUS_SUCCESS) {
                        APP_DBG_MSG("==>> aci_gap_numeric_comparison_value_confirm_yesno-->YES : "
                                    "Fail, reason: 0x%x\n",
                                    ret);
                    }
                    else {
...
#    endif /* CFG_STATIC_RANDOM_ADDRESS */
    ...
    ret = aci_hal_write_config_data(CONFIG_DATA_RANDOM_ADDRESS_OFFSET,
                                    CONFIG_DATA_RANDOM_ADDRESS_LEN, (uint8_t*)a_srd_bd_addr);
    if (ret != BLE_STATUS_SUCCESS) {
        ...
        APP_DBG_MSG(
            "  Success: aci_hal_write_config_data command - CONFIG_DATA_RANDOM_ADDRESS_OFFSET\n");
        APP_DBG_MSG("  Random Bluetooth Address: %02x:%02x:%02x:%02x:%02x:%02x\n",
                    (uint8_t)(a_srd_bd_addr[1] >> 8), (uint8_t)(a_srd_bd_addr[1]),
                    (uint8_t)(a_srd_bd_addr[0] >> 24), (uint8_t)(a_srd_bd_addr[0] >> 16),
                    (uint8_t)(a_srd_bd_addr[0] >> 8), (uint8_t)(a_srd_bd_addr[0]));
    }
    ...
    if (ret != BLE_STATUS_SUCCESS) {
        APP_DBG_MSG(
            "  Fail   : aci_hal_write_config_data command - CONFIG_DATA_IR_OFFSET, result: 0x%x \n",
            ret);
    }
    else {
    ...
    if (ret != BLE_STATUS_SUCCESS) {
        APP_DBG_MSG(
            "  Fail   : aci_hal_write_config_data command - CONFIG_DATA_ER_OFFSET, result: 0x%x \n",
            ret);
    }
    else {
    ...
    if (role > 0) {
        ...
        ret = aci_gap_init(role, CFG_PRIVACY, CFG_GAP_DEVICE_NAME_LENGTH, &gap_service_handle,
    ...
    BleApplicationContext.BleApplicationContext_legacy.appearanceCharHandle =
...
static void Adv_Request(APP_BLE_ConnStatus_t NewStatus) {
    ...
    ret = aci_gap_set_discoverable(ADV_TYPE, CFG_FAST_CONN_ADV_INTERVAL_MIN,
                                   CFG_FAST_CONN_ADV_INTERVAL_MAX, CFG_BLE_ADDRESS_TYPE, ADV_FILTER,
                                   0, 0, 0, 0, 0, 0);
    if (ret != BLE_STATUS_SUCCESS) {
...
void BLE_SVC_L2CAP_Conn_Update(uint16_t ConnectionHandle) {
    ...
    if (mutex == 1) {
        ...
        ret = aci_l2cap_connection_parameter_update_req(
            BleApplicationContext.BleApplicationContext_legacy.connectionHandle, interval_min,
            interval_max, peripheral_latency, timeout_multiplier);
        if (ret != BLE_STATUS_SUCCESS) {
...
static void LinkConfiguration(void) {
    ...
    status = hci_le_set_data_length(
        BleApplicationContext.BleApplicationContext_legacy.connectionHandle, 251, 2120);
...
static void FS_Adv_Request(APP_BLE_ConnStatus_t NewStatus) {
    ...
    if ((BleApplicationContext.Device_Connection_Status == APP_BLE_FAST_ADV) ||
        ...
        if (ret != BLE_STATUS_SUCCESS) {
            APP_DBG_MSG(
                "==>> aci_gap_set_non_discoverable - Stop Advertising Failed , result: %d \n", ret);
        }
        else {
    ...
    if (request_pairing) {
        /* Start Fast or Low Power Advertising */
        ret = aci_gap_set_limited_discoverable(ADV_IND, Min_Inter, Max_Inter, CFG_BLE_ADDRESS_TYPE,
        ...
        /* Start Fast or Low Power Advertising */
        ret = aci_gap_set_undirected_connectable(Min_Inter, Max_Inter, CFG_BLE_ADDRESS_TYPE,
...
void APP_BLE_UpdateDeviceName(void) {
    ...
    if (BleApplicationContext.BleApplicationContext_legacy.gapServiceHandle) {
        ...
        ret = aci_gatt_update_char_value(
            ...
            BleApplicationContext.BleApplicationContext_legacy.devNameCharHandle, 0, char_length,
            (uint8_t*)name);
        if (ret != BLE_STATUS_SUCCESS) {
            BLE_DBG_SVCCTL_MSG("  Fail   : aci_gatt_update_char_value - Device Name\n");
        }
    ...
    if (BLE_STATUS_SUCCESS == ret) {
        if (total == 0) {
            ...
            for (uint8_t k = 0; k < total; k++) {
                APP_DBG_MSG(
                    "Bonded device %d / %d - %s BLE address %02X:%02X:%02X:%02X:%02X:%02X\r\n", k,
                    total, devices[k].Address_Type == 0 ? "Public" : "Random",
