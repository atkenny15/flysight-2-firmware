...
#define COPY_CRS_UUID(uuid_struct)                                                               \
    COPY_UUID_128(uuid_struct, 0x00, 0x00, 0x00, 0x00, 0xcc, 0x7a, 0x48, 0x2a, 0x98, 0x4a, 0x7f, \
                  0x2e, 0xd5, 0xb3, 0xe5, 0x8f)
#define COPY_CRS_TX_UUID(uuid_struct)                                                            \
    COPY_UUID_128(uuid_struct, 0x00, 0x00, 0x00, 0x01, 0x8e, 0x22, 0x45, 0x41, 0x9d, 0x4c, 0x21, \
                  0xed, 0xae, 0x82, 0xed, 0x19)
#define COPY_CRS_RX_UUID(uuid_struct)                                                            \
    COPY_UUID_128(uuid_struct, 0x00, 0x00, 0x00, 0x02, 0x8e, 0x22, 0x45, 0x41, 0x9d, 0x4c, 0x21, \
                  0xed, 0xae, 0x82, 0xed, 0x19)
#define COPY_GNSS_UUID(uuid_struct)                                                              \
    COPY_UUID_128(uuid_struct, 0x00, 0x00, 0x00, 0x00, 0xcc, 0x7a, 0x48, 0x2a, 0x98, 0x4a, 0x7f, \
                  0x2e, 0xd5, 0xb3, 0xe5, 0x8f)
#define COPY_GNSS_PV_UUID(uuid_struct)                                                           \
    COPY_UUID_128(uuid_struct, 0x00, 0x00, 0x00, 0x00, 0x8e, 0x22, 0x45, 0x41, 0x9d, 0x4c, 0x21, \
                  0xed, 0xae, 0x82, 0xed, 0x19)
                ...
                case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
                    ...
                    if (attribute_modified->Attr_Handle ==
                        ...
                        switch (attribute_modified->Attr_Data[0]) {
                            ...
                            case COMSVC_Notification:
                                /* USER CODE BEGIN
                                 * CUSTOM_STM_Service_1_Char_1_COMSVC_Notification_BEGIN */
                                ...
                                Custom_STM_App_Notification(&Notification);
                                /* USER CODE BEGIN
                                 * CUSTOM_STM_Service_1_Char_1_COMSVC_Notification_END */
                    ...
                    } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomCrs_TxHdle +
                         CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/
                        ...
                        switch (attribute_modified->Attr_Data[0]) {
                            ...
                            case COMSVC_Notification:
                                /* USER CODE BEGIN
                                 * CUSTOM_STM_Service_2_Char_1_COMSVC_Notification_BEGIN */
                                ...
                                Custom_STM_App_Notification(&Notification);
                                /* USER CODE BEGIN
                                 * CUSTOM_STM_Service_2_Char_1_COMSVC_Notification_END */
                    ...
                    } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomGnss_PvHdle +
                         CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/
                        ...
                        return_value = SVCCTL_EvtAckFlowEnable;
                        /* USER CODE BEGIN
                         * CUSTOM_STM_Service_1_Char_2_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
...
void SVCCTL_InitCustomSvc(void) {
     ...
     * 1*no_of_char_with_broadcast_property service_max_attribute_record = 1 for CRS + 2 for CRS_TX
     * + 2 for CRS_RX + 1 for CRS_TX configuration descriptor + = 6
     *
     * This value doesn't take into account number of descriptors manually added
     * In case of descriptors added, please update the max_attr_record value accordingly in the next
     * SVCCTL_InitService User Section
     */
    max_attr_record = 6;
    ...
    ret = aci_gatt_add_service(UUID_TYPE_128, (Service_UUID_t*)&uuid, PRIMARY_SERVICE,
                               max_attr_record, &(CustomContext.CustomCrsHdle));
    ...
    ret = aci_gatt_add_char(CustomContext.CustomCrsHdle, UUID_TYPE_128, &uuid, SizeCrs_Tx,
                            CHAR_PROP_NOTIFY, ATTR_PERMISSION_ENCRY_READ, GATT_DONT_NOTIFY_EVENTS,
                            0x10, CHAR_VALUE_LEN_VARIABLE, &(CustomContext.CustomCrs_TxHdle));
    if (ret != BLE_STATUS_SUCCESS) {
    ...
    ret = aci_gatt_add_char(CustomContext.CustomCrsHdle, UUID_TYPE_128, &uuid, SizeCrs_Rx,
                            CHAR_PROP_READ | CHAR_PROP_WRITE_WITHOUT_RESP,
                            ATTR_PERMISSION_ENCRY_READ | ATTR_PERMISSION_ENCRY_WRITE,
                            GATT_NOTIFY_ATTRIBUTE_WRITE, 0x10, CHAR_VALUE_LEN_VARIABLE,
                            &(CustomContext.CustomCrs_RxHdle));
    if (ret != BLE_STATUS_SUCCESS) {
     ...
     * 1*no_of_char_with_broadcast_property service_max_attribute_record = 1 for GNSS + 2 for
     * GNSS_PV + 1 for GNSS_PV configuration descriptor + = 4
     *
     * This value doesn't take into account number of descriptors manually added
     * In case of descriptors added, please update the max_attr_record value accordingly in the next
     * SVCCTL_InitService User Section
    ...
    ret = aci_gatt_add_service(UUID_TYPE_128, (Service_UUID_t*)&uuid, PRIMARY_SERVICE,
                               max_attr_record, &(CustomContext.CustomGnssHdle));
    if (ret != BLE_STATUS_SUCCESS) {
    ...
    ret = aci_gatt_add_char(CustomContext.CustomGnssHdle, UUID_TYPE_128, &uuid, SizeGnss_Pv,
                            CHAR_PROP_READ | CHAR_PROP_NOTIFY, ATTR_PERMISSION_ENCRY_READ,
                            GATT_DONT_NOTIFY_EVENTS, 0x10, CHAR_VALUE_LEN_CONSTANT,
                            &(CustomContext.CustomGnss_PvHdle));
    if (ret != BLE_STATUS_SUCCESS) {
    ...
    switch (CharOpcode) {
        ...
        case CUSTOM_STM_CRS_TX:
            ret = aci_gatt_update_char_value(CustomContext.CustomCrsHdle,
                                             CustomContext.CustomCrs_TxHdle, 0, /* charValOffset */
                                             SizeCrs_Tx,                        /* charValueLen */
                                             (uint8_t*)pPayload);
            if (ret != BLE_STATUS_SUCCESS) {
                APP_DBG_MSG(
                    "  Fail   : aci_gatt_update_char_value CRS_TX command, result : 0x%x \n\r",
                    ret);
            }
            else {
        ...
        case CUSTOM_STM_CRS_RX:
            ret = aci_gatt_update_char_value(CustomContext.CustomCrsHdle,
                                             CustomContext.CustomCrs_RxHdle, 0, /* charValOffset */
                                             SizeCrs_Rx,                        /* charValueLen */
                                             (uint8_t*)pPayload);
            if (ret != BLE_STATUS_SUCCESS) {
                APP_DBG_MSG(
                    "  Fail   : aci_gatt_update_char_value CRS_RX command, result : 0x%x \n\r",
                    ret);
            }
            else {
        ...
        case CUSTOM_STM_GNSS_PV:
            ret = aci_gatt_update_char_value(CustomContext.CustomGnssHdle,
                                             CustomContext.CustomGnss_PvHdle, 0, /* charValOffset */
                                             SizeGnss_Pv,                        /* charValueLen */
                                             (uint8_t*)pPayload);
            if (ret != BLE_STATUS_SUCCESS) {
                APP_DBG_MSG(
                    "  Fail   : aci_gatt_update_char_value GNSS_PV command, result : 0x%x \n\r",
                    ret);
            }
            else {
